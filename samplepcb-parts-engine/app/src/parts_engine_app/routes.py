...
from .jobs import Job, JobError, JobService
...
async def start_supplier_search(request: Request, job_id: str) -> dict[str, Any]:
    _job(request, job_id)
    try:
        job = _svc(request).submit_supplier(job_id)
    except JobError as error:
