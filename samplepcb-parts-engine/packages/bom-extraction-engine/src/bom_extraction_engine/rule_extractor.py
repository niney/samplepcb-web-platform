...
_VALUE_PAT = re.compile(r"^value$|^comment$|comp[\s_]*value|^규격$"
                        ...
                        r"|^designation$|^용량$|^형\s*명$")  # KiCad export의 값 열
...
def classify_columns(labels: List[str]) -> Dict[str, List[int]]:
    ...
    for i, raw in enumerate(labels):
        ...
        if _DIST_PAT.search(lab):
            ...
            role = "manufacturer"
        elif _FOOTPRINT_PAT.search(lab):
            # Protel의 PCB DECAL은 원시 풋프린트다. 실제 package와 분리해
            # 보존하고, 검증 가능한 물리 표기만 뒤에서 package로 승격한다.
            role = "footprint"
...
_RAW_NUMBER = r"(?:\d+(?:[.,]\d+)?|\.\d+)"
_RE_CAP = re.compile(
    rf"(?<![A-Za-z0-9]){_RAW_NUMBER}\s*(?:\[\s*)?[uµμnp]F(?:\s*\])?"
    r"(?![A-Za-z])",
    re.I,
)
...
_RE_PASSIVE_COMPOSITE = re.compile(
    r"^(?P<prefix>[CRL])(?P<size>01005|0201|0402|0603|0805|1005|1206|1210|"
    r"1608|1808|1812|2010|2012|2220|2312|2512|3216|3225|3528|4520|4532|"
    r"5025|5750|6032|6332|7343)(?:_|,\s*)(?P<value>.+)$",
    re.I,
)
_RE_PASSIVE_VALUE_TOLERANCE = re.compile(
    r"^(?P<resistance>\d+(?:[.,]\d+)?(?:[KM]\d*|m?R\d*|kR)?)"
    r"\s*[,/]\s*(?P<tolerance>[BCDFGJKM])$",
    re.I,
)
_RE_ELECTROLYTIC_PREFIX = re.compile(
    r"^(?:E\s*/\s*C|E[-_ ]?CAP|ECAP)[_:/\s-]+(?P<body>.+)$",
    re.I,
)
...
_RE_TOL = re.compile(
    ...
    r"|\b\d+(?:\.\d+)?\s*(?:[%％]|ppm)", re.I)
_RE_VOLT = re.compile(  # 범위와 소수 쉼표를 보존하고 Y5V 같은 유전체는 제외
    rf"(?<![A-Za-z0-9])[-+]?{_RAW_NUMBER}\s*(?:k?V(?:DC|AC)?)?\s*"
    rf"(?:~|to|[-–—])\s*[-+]?{_RAW_NUMBER}\s*k?V(?:DC|AC)?\b"
    rf"|(?<![A-Za-z0-9])[-+]?{_RAW_NUMBER}\s*k?V(?:DC|AC)?\b",
    re.I,
)
...
_RE_PKG_IMP = re.compile(r"\b(0201|0402|0603|0606|0805|1005|1206|1210|1218"
                         ...
                         r"|2520|3215|3216|3225|3528|4532|5025|5032|7050|7343)\b")
_RE_PKG_NAMED = re.compile(  # 꼬리 -?\d*는 절단 매치("DIP-")를 만들므로 금지
    ...
    r"|SOIC(?:-?\d+)?|DFN(?:-?\d+)?|(?:[BLU]?FCSP|WLCSP)(?:-?\d+)?"
    r"|[WXU]?SON(?:-?\d+)?|PLCC\d*|BGA\d*"
...
_PASSIVE_TOLERANCE_CODE = re.compile(r"\b([BCDFGJKM])\s*$", re.I)
_PASSIVE_TEXT_WORDS = re.compile(
    r"\b(?:capacitor|resistor|inductor|ceramic|tantal(?:um)?|mlcc|cap|res|ind|"
    r"C0G|NP0|X5R|X7R|Y5V|Z5U|SMD|SMT|THT|DIP)\b",
    re.I,
)


def _passive_spec_only(text: str) -> bool:
    """True when a nominal PN cell contains only passive specifications.

    Header names are not authoritative in field BOMs.  A string such as
    ``10uF/6.3V 1608`` must become parametric evidence, while mixed strings
    such as ``1.5uH/VLS3012HBX-1R5M`` must retain the actual MPN segment.
    The residual check is deliberately strict so a real identifier is never
    discarded merely because it embeds an electrical value.
    """

    primary_patterns = (_RE_CAP, _RE_IND, _RE_RES_OHM, _RE_RES_CODE)
    if not any(pattern.search(text) for pattern in primary_patterns):
        return False
    residual = text
    for pattern in (
        *_GRAMMAR,
        ...
        compiled = pattern[1] if isinstance(pattern, tuple) else pattern
        residual = compiled.sub(" ", residual)
    residual = _PASSIVE_TOLERANCE_CODE.sub(" ", residual)
    residual = _PASSIVE_TEXT_WORDS.sub(" ", residual)
    return not re.search(r"[A-Za-z0-9가-힣]", residual)
...
_TYPE_RULES = [  # (enum, 키워드 정규식) — 구체적인 것 먼저
    ...
    ("ic", re.compile(  # "IC LED DRVR", "LED Driver"는 led가 아니라 ic
        r"\bic\b|i\.c\b|mcu|driver|drvr|regulator|amplifier|\bamp\b|codec"
        r"|\bmems\s+mic(?:rophone)?\b"
        r"|opamp|controller"
...
def extract_reference(cell: str) -> Optional[str]:
    ...
    if not c:
        return None
    canonical = reference_designators(c)
    if canonical:
...
_PN_REJECT_KW = re.compile(
    r"\bled\b|\bbead\b|f\.bead|\bheader\b|\bhdr\b|\bmcu\b|x-?tal\b|\bic\b|\bcap\b"
...
_RE_PKG_PIN_PREFIX = re.compile(
    r"^(?P<pins>\d{1,3})(?P<family>TSSOP|SSOP|MSOP|SOIC|SOP|QFN|DFN|"
    r"LQFP|TQFP|QFP|DIP)(?:$|[-_/])",
    re.I,
)
...
_RE_PASSIVE_PHYSICAL_FOOTPRINT = re.compile(
    r"^(?:CAP(?:ACITOR)?|RES(?:ISTOR)?|IND(?:UCTOR)?)"
    r"[ _-]?C?(0603|1005|1608|2012|3216|3225|4532|5025|6432)"
    r"X\d{2,4}(?:L\d*)?(?:[_-]|$)",
    re.I,
)
_PASSIVE_METRIC_TO_IMPERIAL = {
    "0603": "0201",
    "1005": "0402",
    "1608": "0603",
    "2012": "0805",
...
def _pkg_from_cell(cell: str) -> Optional[str]:
    ...
    if not c:
        return None
    physical_passive = _RE_PASSIVE_PHYSICAL_FOOTPRINT.match(c)
    if physical_passive:
        return _PASSIVE_METRIC_TO_IMPERIAL[physical_passive.group(1).upper()]
    pin_prefixed = _RE_PKG_PIN_PREFIX.match(c)
    if pin_prefixed:
        return (
            f"{pin_prefixed.group('family').upper()}-"
            f"{pin_prefixed.group('pins')}"
        )
...
def package_from_footprint(cell: str, part_type: str | None) -> Optional[str]:
    """Promote only physically meaningful PCB footprint notation.

    PCB DECAL columns also contain library names or repeated MPNs.  Those raw
    values remain available through ``footprint`` but must not become package
    requirements unless a known package/dimension grammar can verify them.
    """

    c = cell.strip()
    if not c:
        return None
    parsed = _pkg_from_cell(c)
    if not parsed:
        return None
    if (
        part_type == "capacitor"
        and re.search(r"(?:^|[^A-Z0-9])E\s*/?\s*C(?:[^A-Z0-9]|$)", c, re.I)
        and _find_dim(c, strict=False)
    ):
        # Preserve the SMD/radial marker together with the cylindrical size;
        # the supplier planner turns these into mount/diameter requirements.
        return c
    return parsed
...
def package_from_source_cell(cell: str, part_type: str | None) -> Optional[str]:
    ...
    Passive CAD footprints encode a physical package before library suffixes;
    other footprints retain the existing conservative promotion contract.
    """
    ...
    return (_pkg_from_cell(c) if passive_cad else None) or package_from_footprint(
        c, part_type
    )
...
def extract_row(labels: List[str], roles: Dict[str, List[int]],
    ...
    for i in pn_cols:
        ...
        if _passive_spec_only(c):
            # A misleading Part Number header must not turn a complete passive
            # specification into an identity query.  Keep the original cell in
            # the normal grammar path so capacitance/voltage/package/tolerance
            # are recovered with their cell evidence.
            pre_texts.append(c)
            continue
    ...
    for i in roles.get("value", []):
        ...
        # token.  Parse the structure directly instead of allowing PN heuristics
        # (and especially token length) to decide its meaning.
        passive = _RE_PASSIVE_COMPOSITE.fullmatch(c)
        if passive:
            prefix = passive.group("prefix").upper()
            raw_value = passive.group("value").strip()
            passive_field = None
            if prefix == "C" and _RE_CAP.fullmatch(raw_value):
                passive_field = "capacitance"
            elif prefix == "R" and _RE_RES_CODE.fullmatch(raw_value):
                passive_field = "resistance"
            elif prefix == "L" and _RE_IND.fullmatch(raw_value):
                passive_field = "inductance"
            if passive_field:
                if passive_field == "resistance":
                    put_resistance_code(raw_value)
                else:
                    put(passive_field, raw_value, "col")
                put("package", f"{prefix}{passive.group('size')}", "col")
                put(
                    "part_type",
                    {"C": "capacitor", "R": "resistor", "L": "inductor"}[prefix],
                    "infer",
                )
                continue

        passive_tolerance = _RE_PASSIVE_VALUE_TOLERANCE.fullmatch(c)
        if passive_tolerance:
            put("resistance", passive_tolerance.group("resistance"), "col")
            put("tolerance", passive_tolerance.group("tolerance").upper(), "col")
            put("part_type", "resistor", "infer")
            continue

        # E/C_100uF/25V and ECAP-47uF/16V are category prefixes followed by
        # specifications, not manufacturer part numbers.
        electrolytic = _RE_ELECTROLYTIC_PREFIX.fullmatch(c)
        if electrolytic:
            put("part_type", "capacitor", "infer")
            c = electrolytic.group("body").strip()

        # A recognized parenthetical value/package annotation belongs outside
        # the MPN.  Unknown parentheses remain untouched to preserve legitimate
        # identifiers such as M7(1N4007SMD).
        annotated = re.fullmatch(r"(.{3,}?)\s*\(([^()]+)\)", c)
        if annotated:
            base = annotated.group(1).strip()
            inner = annotated.group(2).strip()
            annotation_handled = False
            for field, pattern in _GRAMMAR:
    ...
    if "part_number" not in val:
        ...
        # recovers compact diode/connector MPNs without loosening the general
        # free-text heuristic or introducing a product-specific exception.
        for i in roles.get("value", []):
            ...
            corroborated = any(
                j != i
                and candidate_key
                == re.sub(r"[^A-Za-z0-9]", "", cell(j)).casefold()
    ...
    if "tolerance" not in val:
        # EIA class letters at the end of a complete passive specification are
        # unambiguous (``10Ω 1608 F``, ``100nF 0603 J``).  Do not scan generic
        # prose or bare MPNs for a trailing letter.
        specification_cells = [
            cell(i)
            for i in (*roles.get("part_number", []), *roles.get("value", []))
            if cell(i)
        ]
        for specification in specification_cells:
            if not any(
                pattern.search(specification)
                for pattern in (_RE_CAP, _RE_RES_OHM, _RE_RES_CODE)
            ):
                continue
            tolerance = _PASSIVE_TOLERANCE_CODE.search(specification)
            if tolerance:
                put("tolerance", tolerance.group(1).upper(), "col")
                break
    ...
    if "manufacturer" not in val:
        ...
        found = None
        pkg_idx = set(roles.get("package", [])) | set(roles.get("footprint", []))
        # CAD명 속 벤더 태그
    ...
    # package evidence, not a manufacturer part number.  Genuine PN columns
    # and the same token appearing in any non-footprint cell remain untouched.
    if (
        val.get("part_type") in {"resistor", "capacitor", "inductor"}
        and src.get("part_number") == "text"
        and val.get("part_number")
    ):
        part_number_key = re.sub(
            r"[^A-Za-z0-9]", "", str(val["part_number"])
        ).casefold()
        footprint_indexes = set(roles.get("package", [])) | set(
            roles.get("footprint", [])
        )
        footprint_owned = any(
            part_number_key
            and part_number_key
            in re.sub(r"[^A-Za-z0-9]", "", cell(i)).casefold()
            and package_from_footprint(
                cell(i), str(val.get("part_type") or "") or None
            )
            is not None
            and bool(
                re.match(
        ...
        corroborated_elsewhere = any(
            ...
            if i not in footprint_indexes
        )
        if footprint_owned and not corroborated_elsewhere:
            val.pop("part_number", None)
            src.pop("part_number", None)
    ...
    footprint_cell = next(
        (cell(i) for i in roles.get("footprint", []) if cell(i)),
        "",
    )
    ...
    if (
        verbatim
        and col_code
        and _RE_PKG_IMP.fullmatch(col_code)
        and passive_cad_footprint
    ):
        # 수동소자 CAD 풋프린트("CAP 0201/0603X03L_C")의 긴 원문보다
        # 그 안에서 확인된 물리 패키지를 사용한다. 원문 토큰은 PN이 아니다.
        verbatim = None
    ...
    if verbatim and "_" not in verbatim:
        ...
        footprint_package = package_from_source_cell(
            footprint_cell,
            str(val.get("part_type") or "") or None,
        )
        if footprint_package:
            pkg = (footprint_package, "col")
    ...
    if pkg and pkg[0]:
        ...
        pn_text = str(val.get("part_number", "")).strip()
        pn_package_split = False
        pn_parts = pn_text.rsplit(None, 1)
        pkg_suffix_key = re.sub(r"[^A-Za-z0-9]", "", str(pkg[0])).upper()
        if len(pn_parts) == 2:
            pn_head_text, pn_tail = pn_parts
            pn_tail_key = re.sub(r"[^A-Za-z0-9]", "", pn_tail).upper()
            if (
                pkg_suffix_key
                and pn_tail_key == pkg_suffix_key
                and _looks_like_pn(pn_head_text)
                and not _pn_reject(pn_head_text)
            ):
                # "ET1100-0003 BGA128"처럼 패키지 열과 동일한 공백
                # 접미사는 패키지를 버리기 전에 PN에서 분리한다.
                val["part_number"] = _strip_pn_alt(pn_head_text)
                src["part_number"] = "col"
                pn_package_split = True
        ...
        if (not pn_package_split and pn_norm
                and (cand_norm == pn_norm or cand_norm == pn_head
    ...
    # Supplier specifications commonly append the package after a whitespace
    # boundary ("MPN BGA128", "MPN SOT-23").  Split only when the suffix is
    # exactly the independently extracted package, so input order and unrelated
    # spaces inside a legitimate MPN cannot affect the result.
    package_value = str(val.get("package") or "").strip()
    package_key = re.sub(r"[^A-Za-z0-9]", "", package_value).upper()

    def split_mpn_package(candidate: str) -> Optional[str]:
        match = re.fullmatch(r"(.+?)\s+(\S+)", candidate.strip())
        if not match:
            return None
        head, tail = match.group(1).strip(), match.group(2).strip()
        tail_key = re.sub(r"[^A-Za-z0-9]", "", tail).upper()
        if (
            package_key
            and tail_key == package_key
            and _looks_like_pn(head)
            and not _pn_reject(head)
        ):
            return _strip_pn_alt(head)
        return None

    if package_key:
        current_part_number = str(val.get("part_number") or "").strip()
        split_current = split_mpn_package(current_part_number)
        if split_current:
            val["part_number"] = split_current
            src["part_number"] = "col"
        elif "part_number" not in val:
            for i in roles.get("value", []):
                split_candidate = split_mpn_package(cell(i))
                if split_candidate:
                    val["part_number"] = split_candidate
                    src["part_number"] = "col"
                    break
    ...
    if "part_number" not in val and pkg_cell and "." not in pkg_cell \
        ...
        if (len(h) >= 7 and _RE_HAS_DIGIT.search(h)
                ...
                and not _RE_PKG_IMP.search(pkg_cell)
                and not _RE_PKG_SLASH.search(pkg_cell)
                and not _RE_PKG_CADISH.search(pkg_cell)
...
def infer_column_roles(roles: Dict[str, List[int]], labels: List[str],
    ...
    if pair is not None:
        ...
        for indices in roles.values():
            if ref_col in indices:
                indices.remove(ref_col)
            if qty_col in indices:
                indices.remove(qty_col)
        # 관계로 검증된 열은 라벨만으로 추정한 후보보다 우선한다. 추출기는
        # 첫 비어 있지 않은 수량 열을 사용하므로 뒤에 append하면 소요/구매
        # 수량이 보드당 Quantity를 덮어쓸 수 있다.
        roles.setdefault("designator", []).insert(0, ref_col)
        roles.setdefault("quantity", []).insert(0, qty_col)

    # Some vendor templates label the component-class column "Reference" and
    # the actual designators "Part Name".  Once the reference/quantity pair is
    # established, re-evaluate any remaining label-derived designator columns
    # by content.  Requiring a strong type vocabulary majority and almost no
    # valid references keeps ordinary Reference columns untouched.
    for i in list(roles.get("designator", [])):
        vals = col_vals(i)
        if len(vals) < 3:
            continue
        reference_ratio = sum(
            reference_list_count(value) is not None for value in vals
        ) / len(vals)
        typed_ratio = sum(infer_part_type(value) is not None for value in vals) / len(vals)
        if typed_ratio >= 0.6 and reference_ratio < 0.2:
            roles["designator"].remove(i)
            roles.setdefault("part_type", []).append(i)
    ...
    for i in range(len(labels)):
        ...
        if typed >= 0.6:
            roles.setdefault("part_type", []).append(i)
        elif pkg >= 0.6 and "package" not in roles:
...
def compute_roles(case: dict) -> Dict[str, List[int]]:
    ...
    # 병합 헤더가 두 실제 열을 덮는 내보내기 형식:
    # ``Part\PCB Footprint`` 아래 왼쪽은 값/MPN, 오른쪽 무제목 열은
    # footprint다. 파일명이 아니라 헤더의 두 의미와 인접 데이터 열로
    # 판단하며, value 역할은 스펙과 MPN의 이중성을 이미 안전하게 처리한다.
    for i, label in enumerate(labels[:-1]):
        normalized = _norm_label(label)
        if not (re.search(r"\bpart\b", normalized)
                and re.search(r"footprint|pcb\s*decal", normalized)):
            continue
        if _norm_label(labels[i + 1]):
            continue
        if not any(
            i + 1 < len(row.get("cells") or [])
            and str(row["cells"][i + 1] or "").strip()
            for row in rows
        ):
            continue
        for indices in roles.values():
            ...
            if i + 1 in indices:
                indices.remove(i + 1)
        roles.setdefault("value", []).insert(0, i)
        roles.setdefault("footprint", []).insert(0, i + 1)
