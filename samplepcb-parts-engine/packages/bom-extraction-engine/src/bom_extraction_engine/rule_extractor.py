...
_VALUE_PAT = re.compile(r"^value$|^comment$|comp[\s_]*value|^규격$"
                        ...
                        r"|^designation$|^용량$")  # KiCad export의 값 열
...
_RE_PASSIVE_COMPOSITE = re.compile(
    ...
    r"5025|5750|6032|6332|7343)_(?P<value>.+)$",
    re.I,
)
...
def extract_row(labels: List[str], roles: Dict[str, List[int]],
    ...
    if verbatim and "_" not in verbatim:
        ...
        footprint_package = package_from_footprint(
            footprint_cell,
...
def infer_column_roles(roles: Dict[str, List[int]], labels: List[str],
    ...
    if pair is not None:
        ...
        roles.setdefault("designator", []).append(ref_col)
        roles.setdefault("quantity", []).append(qty_col)
