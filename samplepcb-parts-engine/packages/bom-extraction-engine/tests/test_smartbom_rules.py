...
CASES += [
    ...
    ("콤마 구분 CAD 저항 크기와 값을 분리",
     (["Part", "Description", "Reference", "Q'ty"],
      ["R1608,10k", "Chip Resistor", "R1", "1"]),
     lambda a: a.part_number is None and a.part_type == "resistor"
     and a.resistance == "10k" and a.package == "R1608"),
    ("슬래시 공차 저항값을 품번으로 오인하지 않음",
     (["Geo", "Value", "Reference", "Q'ty"],
      ["SMD-R(1005)", "44.2/F", "R1", "1"]),
     lambda a: a.part_number is None and a.resistance == "44.2"
     and a.tolerance == "F" and a.package == "1005"),
    ("선행 소수점 커패시턴스 보존",
     (["Value", "Reference", "Q'ty"], [".47uF", "C1", "1"]),
     lambda a: a.capacitance == ".47uF"),
    ("대괄호 단위 커패시턴스 보존",
     (["Value", "Reference", "Q'ty"], ["10[uF]", "C1", "1"]),
     lambda a: a.capacitance == "10[uF]"),
    ("유전체 코드 안의 숫자를 전압으로 오인하지 않음",
     (["Description", "Reference", "Q'ty"],
      ["Ceramic capacitor 0.1uF, Y5V, 50V", "C1", "1"]),
     lambda a: a.voltage == "50V"),
    ("소수 쉼표 전압을 전체 값으로 보존",
     (["Description", "Reference", "Q'ty"],
      ["Electrolytic capacitor 47uF 6,3V", "C1", "1"]),
     lambda a: a.voltage == "6,3V"),
    ("하이픈 전압 범위를 전체 값으로 보존",
     (["Description", "Reference", "Q'ty"],
      ["AC input module 85-265 VAC", "U1", "1"]),
     lambda a: a.voltage == "85-265 VAC"),
    ("한국어 형명 열의 품번을 구조화",
     (["품명", "형명", "규격", "ITEM", "수량"],
      ["IC", "PIC12F675", "SOIC-8", "U1", "1"]),
     lambda a: a.part_number == "PIC12F675" and a.package == "SOIC-8"),
    ("병합된 Part와 Footprint 하위 열 역할 분리",
     (["Quantity", "Reference", "Part\\PCB Footprint", ""],
      ["2", "C1 C2", "0.1uF", "C2012"],
      (["1", "D1", "SS14", "SS14_DO214AC"],
       ["3", "R1 R2 R3", "10k", "R2012"],
       ["1", "U1", "MAX485ESA", "SOIC-8"])),
     lambda a: a.part_number is None and a.capacitance == "0.1uF"
     and a.package == "C2012"),
    ("유통사 SKU 열은 값이나 품번 역할을 침범하지 않음",
     (["References", "Quantity", "Description", "MPN", "Newark", "RS"],
      ["C1", "1", "Capacitor", "", "68R4788", "123-456"],
      (["C2", "1", "Capacitor", "", "38K1669", "234-567"],
       ["C3", "1", "Capacitor", "", "35R0687", "345-678"])),
     lambda a: a.part_number is None and a.resistance is None),
