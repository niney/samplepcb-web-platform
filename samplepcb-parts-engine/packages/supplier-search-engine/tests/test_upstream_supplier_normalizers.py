...
def crystal_query() -> PlannedQuery:
    return PlannedQuery(
        component_id="x2",
        mode=SearchMode.PARAMETRIC,
        part_type="crystal",
...
def test_digikey_normalizes_parameters_and_offers():
    ...
    assert product.manufacturer_evidence == ManufacturerEvidence.STRUCTURED
...
def test_digikey_crystal_combines_generic_supplier_package_with_physical_size():
    ...
    raw = RawSupplierResponse(
        ...
        payload={
            "Product": {
                "ManufacturerProductNumber": "XTAL-32MHZ",
                "Description": {"ProductDescription": "32MHz Crystal"},
                "Parameters": [
                    {"ParameterText": "Package / Case", "ValueText": "4-SMD"},
                    {
                        "ParameterText": "Size / Dimension",
                        "ValueText": "1.60mm x 1.20mm",
                    },
                    {"ParameterText": "Supplier Device Package", "ValueText": "4-SMD"},
                ],
            }
        },
    )

    product = client.normalize(raw, crystal_query())[0]

    assert product.package == "4-SMD (1.60mm x 1.20mm)"
    assert product.normalized_specs["package"] == "1612"


def test_digikey_crystal_prefers_specific_supplier_package_over_other_dimensions():
    client = DigiKeyClient(client_id=None, client_secret=None, account_id=None)
    raw = RawSupplierResponse(
        supplier=Supplier.DIGIKEY,
        ok=True,
                ...
                "Parameters": [
                    {
                        ...
                        "ValueText": "4-SMD(1.6x1.2)",
                    },
                    {"ParameterText": "Size / Dimension", "ValueText": "2.0 x 1.6 mm"},
                    {"ParameterText": "Package / Case", "ValueText": "4-SMD"},
                ],
            }
        },
    ...
    assert product.package == "4-SMD(1.6x1.2)"
    assert product.normalized_specs["package"] == "1612"


def test_digikey_generic_crystal_package_without_size_remains_unverified():
    client = DigiKeyClient(client_id=None, client_secret=None, account_id=None)
    raw = RawSupplierResponse(
        supplier=Supplier.DIGIKEY,
        ok=True,
    ...
    assert product.package == "4-SMD"
    assert "package" not in product.normalized_specs
...
def test_mouser_crystal_description_supplies_physical_package():
    client = MouserClient(api_key=None)
    raw = RawSupplierResponse(
        supplier=Supplier.MOUSER,
        ok=True,
        payload={
            "SearchResults": {
                "Parts": [
                    {
                        "ManufacturerPartNumber": "XTAL-32MHZ",
                        "Description": "32MHz crystal 4-SMD 1.6mm x 1.2mm",
                    }
                ]
            }
        },
...
def test_digikey_discards_only_invalid_price_breaks():
    client = DigiKeyClient(client_id=None, client_secret=None, account_id=None)
    raw = RawSupplierResponse(
        supplier=Supplier.DIGIKEY,
        ok=True,
                ...
                "ProductVariations": [
                    {
                        "DigiKeyProductNumber": "A-1-ND",
                        "StandardPricing": [
                            {"BreakQuantity": 1, "UnitPrice": 0},
                            {"BreakQuantity": 10, "UnitPrice": -1},
                            {"BreakQuantity": 20, "UnitPrice": ""},
                            {"BreakQuantity": 30, "UnitPrice": "bad"},
                            {"BreakQuantity": 40, "UnitPrice": 0.1},
                        ],
                    }
                ],
            }
    ...
    offer = client.normalize(raw, query())[0].offers[0]
...
def test_mouser_discards_only_invalid_price_breaks():
    client = MouserClient(api_key=None)
    raw = RawSupplierResponse(
        supplier=Supplier.MOUSER,
        ok=True,
                ...
                "Parts": [
                    {
                        ...
                        "MouserPartNumber": "603-RC0603-10K",
                        "PriceBreaks": [
                            {"Quantity": 1, "Price": "$0", "Currency": "USD"},
                            {"Quantity": 10, "Price": "-$1", "Currency": "USD"},
                            {"Quantity": 20, "Price": "", "Currency": "USD"},
                            {"Quantity": 30, "Price": "bad", "Currency": "USD"},
                            {"Quantity": 40, "Price": "$0.12", "Currency": "USD"},
                        ],
                    }
                ]
            }
...
def test_unikeyic_discards_only_invalid_price_breaks():
    client = UniKeyICClient(api_key=None, base_url="")
    raw = RawSupplierResponse(
        supplier=Supplier.UNIKEYIC,
        ok=True,
                ...
                "products": [
                    {
                        ...
                        "pro_sno": "RC0603-10K",
                        "nums": [1, 10, 20, 30, 40],
                        "calc_sale_usd_price": [0, -1, "", "bad", {"USD": 0.2}],
                    }
                ]
            },
        },
