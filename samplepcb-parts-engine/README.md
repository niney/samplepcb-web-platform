...
세부 평가를 그대로 보존하면서 자동 선정한다. 단, BOM 원본끼리 충돌한 입력 플래그가 있으면 정확
MPN도 자동 확정하지 않고 검토 대상으로 유지한다. 일반 불일치는 화면 검증 정보로 계속 노출하되
검토 확인을 요구하지 않는다. 포장 접미사 등 품번 변형과 파라메트릭 후보의 조건 불일치는 기존처럼 차단한다.
