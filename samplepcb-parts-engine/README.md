...
동기화 기준: `sp-smartbom-eye` `be36f35`(`Protel BOM 복합 표기와 풋프린트 추출 개선`, 직전
`ff91256` 공급사 스펙 검색 정규화와 `1d597a5` 관계 기반 추출 개선 포함).
...
(현재 365 passed, 1 skipped), 리팩토링은 그린을 유지하며 단계적으로. 엔진 로직은 "다시 쓰는 코드"가 아니라
"연구 계보에서 re-sync 하는 vendored 코드"로 취급 — 리팩토링 에너지는 신규 `app/` 계층에 집중한다.

## 계약 식별자 보존

결과의 `"engine": "smartbom"`, `parser_version="smartbom-rules/1.2"` 등 식별자는 프론트/저장 호환을 위해
