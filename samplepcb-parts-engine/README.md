...
근거가 충돌하면 자동 은폐하지 않고 검토 플래그로 보존한다.
...
- 검색 계약 1.1은 추출 엔진의 정규화 수치값을 검색 계획에 직접 전달한다. 공급사별 실제 검색어는
...
동기화 기준: `sp-smartbom-eye` `32d94b0`(`BOM 축약 참조번호와 혼합 스펙 추출 개선`, 직전
`be36f35` Protel 복합 표기·풋프린트 개선과 관계 기반 추출·공급사 정규화 개선 포함).
대상 프로젝트 고유 기능(이미지·UniKeyIC 패키징 정규화·선택 시트·영속 공급사 잡)은 유지했다.

### 기술 순위와 구매 적용 후보

공급사 검색 스키마 1.6과 `supplier-selection-application-v2`는 기술 판단과 실제 구매 적용을
...
(현재 372 passed, 1 skipped), 리팩토링은 그린을 유지하며 단계적으로. 엔진 로직은 "다시 쓰는 코드"가 아니라
"연구 계보에서 re-sync 하는 vendored 코드"로 취급 — 리팩토링 에너지는 신규 `app/` 계층에 집중한다.

## 계약 식별자 보존

결과의 `"engine": "smartbom"`, `parser_version="smartbom-rules/1.3"` 등 식별자는 프론트/저장 호환을 위해
