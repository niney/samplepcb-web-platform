...
필요수량 대비 절대 초과량과 초과비율이 정책 한도를 모두 넘는 오퍼는 구매 가능 상태와 수동 선택은
유지하되 자동·검토 추천에서는 제외한다. 기술 사전 선정 그룹의 오퍼가 이 조건에 걸리면 다음 구매
가능 기술 그룹을 평가하며, 대안이 없으면 과다수량 오퍼를 조용히 견적에 넣지 않고 미선정으로 남긴다.
...
항상 보존하고, 합집합에 든 동일 부품의 타 공급사 오퍼도 함께 남겨 가격 비교 근거가 사라지지 않는다.
