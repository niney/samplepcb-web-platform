...
- `user_requirements`가 있으면 원본 추출값·계보는 그대로 보존하고 사용자 조건을 마지막에 적용해
  ...
  최소조건이다. 저항 정격전력은 지정한 경우만 비교하며 TCR은 계약에서 제외한다. 비워 둔 공차·전압·
  유전체는 임의 기본값으로 채우지 않고, 부품 종류별 자동선정 핵심조건이 부족하면 후보를 수동검토로
  남긴다. 전해 커패시터의 `8x10.2mm` 같은 기구 패키지는 직경 조건으로 변환한다.
