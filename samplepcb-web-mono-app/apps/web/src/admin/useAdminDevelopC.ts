...
// 무효화 루트 키는 ['admin','develop-c'] 하나 — 상세 액션이 목록 배지·counts 까지 같이 되살린다.
...
const DEVELOP_KEY = ['admin', 'develop-c'] as const;
...
export function useAdminDevelopList(filters: Ref<AdminDevelopFilters>) {
  return useQuery({
    queryKey: ['admin', 'develop-c', 'requests', 'list', filters],
...
export function useDevelopModuleSignals(enabled: Ref<boolean>) {
  return useQuery({
    queryKey: ['admin', 'develop-c', 'module-signals'],
...
export function useAdminDevelopDetail(requestId: Ref<number | null>) {
  return useQuery({
    queryKey: ['admin', 'develop-c', 'requests', 'detail', requestId],
    queryFn: () =>
...
const versionsKey = (requestId: Ref<number | null>) => ['admin', 'develop-c', 'requests', 'detail', requestId, 'review-versions'] as const;
...
// 여기 훅은 전부 쓰기 전용이고, 성공하면 ['admin','develop-c'] 를 무효화해 상세·현황을 같이 되살린다.
...
export function useAdminDevelopSettings() {
  return useQuery({
    queryKey: ['admin', 'develop-c', 'settings'],
