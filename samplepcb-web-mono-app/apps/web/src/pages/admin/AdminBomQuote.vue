...
function openSheetManager(): void {
  ...
  sheetManagerTriggerElement = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  ...
  <div class="h-full">
    ...
    <section v-else-if="isParsing" class="m-6 rounded-2xl border border-gray-200 bg-surface p-10 text-center shadow-sm">
      ...
      <template v-else>
        ...
        >
          <p id="admin-bom-parsing-error-title" class="text-lg font-semibold text-red-600">
          ...
          <div class="mt-4 flex flex-col justify-center gap-2 sm:flex-row">
            ...
            <button
              ...
              :class="parsingFailureKind === 'temporary' ? 'border border-gray-300 bg-surface text-gray-700 hover:bg-gray-50' : 'bg-blue-600 text-white hover:bg-blue-700'"
              @click="goToUpload()"
            >
              새 BOM 업로드
            </button>
    ...
    >
      <h1 id="admin-bom-build-failed-title" class="text-lg font-bold text-red-700">계산할 수 있는 BOM 시트를 찾지 못했습니다</h1>
    ...
    <div v-else-if="detail && detail.buildStatus === 'ready'" class="flex h-full min-h-0 flex-col gap-4 overflow-y-auto p-5 bomwide:flex-row bomwide:overflow-visible">
      ...
      <aside class="w-full shrink-0 [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-line-strong [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:w-[6px] bomwide:min-h-0 bomwide:w-[286px] bomwide:overflow-y-auto bomwide:pb-1">
          ...
          <section>
            ...
            <div class="mt-[10px] grid grid-cols-2 gap-[8px]">
              ...
              <button
                ...
                :class="resultMatchFilter === 'nostock' ? 'ring-2 ring-state-nostock ring-offset-1 shadow-sm' : stats.nostock > 0 ? 'cursor-pointer hover:-translate-y-px hover:shadow-sm' : 'opacity-45'"
