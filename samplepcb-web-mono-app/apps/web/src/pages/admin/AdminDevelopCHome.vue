...
import type { AdminDevelopRequestListItemType, DevelopRequestStatusType } from '@sp/api-contract/develop-c';
import { emptyDevelopFilters, useAdminDevelopList } from '../../admin/useAdminDevelopC';
import DevelopHomeCard from '../../components/admin/develop-c/DevelopHomeCard.vue';
...
const chips = computed(() => {
  ...
  return [
    { key: 'intake', n: d.counts.intake, to: 'admin-develop-c-intake', tone: 'gray' },
    { key: 'contract', n: d.counts.contract, to: 'admin-develop-c-contracts', tone: 'gray' },
    { key: 'inProgress', n: d.counts.in_progress, to: 'admin-develop-c-projects', tone: 'gray' },
    { key: 'delivered', n: d.counts.delivered, to: 'admin-develop-c-deliveries', tone: 'gray' },
    { key: 'docsAwaiting', n: d.signals.docsAwaiting, to: 'admin-develop-c-projects', tone: 'amber' },
    { key: 'replyOverdue', n: d.signals.replyOverdue, to: 'admin-develop-c-projects', tone: 'red' },
    { key: 'inquiriesOpen', n: d.signals.inquiriesOpen, to: 'admin-develop-c-inquiries', tone: 'amber' },
  ] as const;
});
...
<template>
  <div class="space-y-4">
    <a href="/develop/c/request" target="_blank" rel="noopener" class="inline-flex rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white">{{ t('admin.prototypeRequest', { variant: 'C' }) }}</a>
    <div>
      <h1 class="text-2xl font-bold">{{ t('admin.developC.home.title') }}</h1>
      <p class="mt-0.5 text-sm text-gray-500">{{ t('admin.developC.home.desc') }}</p>
    </div>
    ...
    <div class="flex flex-wrap gap-1.5">
      ...
      >
        {{ t(`admin.developC.home.chip.${chip.key}`) }}
    ...
    <p v-else class="rounded-xl border border-gray-200 bg-white px-4 py-10 text-center text-sm text-gray-400">
      {{ isFetching ? t('admin.developC.loading') : t('admin.developC.home.empty') }}
    </p>
  </div>
</template>
