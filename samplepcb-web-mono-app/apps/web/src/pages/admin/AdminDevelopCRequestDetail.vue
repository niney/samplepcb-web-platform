...
import DevelopQuoteSection from '../../components/admin/develop-c/DevelopQuoteSection.vue';
import DevelopRequestContent from '../../components/admin/develop-c/DevelopRequestContent.vue';
import DevelopReviewPanel from '../../components/admin/develop-c/DevelopReviewPanel.vue';
import DevelopStatusBar from '../../components/admin/develop-c/DevelopStatusBar.vue';
import DevelopTimeline from '../../components/admin/develop-c/DevelopTimeline.vue';
import { developStatusBadgeClass } from '../../components/admin/develop-c/develop-badge';
import { downloadAdminDevelopFile } from '../../components/admin/develop-c/develop-files';
...
const SIDE_KEY = 'develop-c.admin.sideContent';
...
const badges = computed<Record<Tab, Badge[]>>(() => {
  ...
  const review: Badge[] = [];
  if (d.review.draftRunning) review.push({ text: t('admin.developC.nav.badge.running'), tone: 'blue' });
  else if (d.review.draftError !== null) review.push({ text: t('admin.developC.nav.badge.error'), tone: 'red' });
  else if (d.review.publicReview !== null && d.review.publishedStale) review.push({ text: t('admin.developC.nav.badge.publishedStale'), tone: 'amber' });
  else if (d.review.publicReview !== null) review.push({ text: t('admin.developC.nav.badge.published'), tone: 'emerald' });
  else if (d.review.working !== null || d.review.draft !== null) review.push({ text: t('admin.developC.nav.badge.ready'), tone: 'gray' });
  if (reviewDirty.value) review.push({ text: t('admin.developC.nav.badge.editing'), tone: 'amber' });

  const diagram: Badge[] = [];
  if (d.diagram.published && d.diagram.publishedStale) diagram.push({ text: t('admin.developC.nav.badge.publishedStale'), tone: 'amber' });
  else if (d.diagram.published) diagram.push({ text: t('admin.developC.nav.badge.published'), tone: 'emerald' });
  else if (d.diagram.html !== null) diagram.push({ text: t('admin.developC.nav.badge.ready'), tone: 'gray' });

  const quotes: Badge[] = [];
  if (d.quotes.length > 0) quotes.push({ text: String(d.quotes.length), tone: 'gray' });
  const sent = d.quotes.filter((q) => q.status === 'sent').length;
  if (sent > 0) quotes.push({ text: t('admin.developC.nav.badge.sent', { count: sent }), tone: 'blue' });
  if (quoteEditing.value) quotes.push({ text: t('admin.developC.nav.badge.editing'), tone: 'amber' });
  ...
  if (d.progress.pendingApprovals > 0) {
    documents.push({ text: t('admin.developC.nav.badge.awaiting', { count: d.progress.pendingApprovals }), tone: 'amber' });
  }
  const drafts = d.documents.filter((doc) => doc.status === 'draft').length;
  if (drafts > 0) documents.push({ text: t('admin.developC.nav.badge.drafting', { count: drafts }), tone: 'gray' });
  if (docsDirty.value) documents.push({ text: t('admin.developC.nav.badge.editing'), tone: 'amber' });
...
async function downloadFromPreview(fileId: number, name: string): Promise<void> {
  ...
  try {
    ...
    previewDownloadError.value = apiErrorMessage(error, t('admin.developC.content.downloadFail'));
  }
}
...
<template>
  <div class="w-full space-y-4" :class="sideOpen ? 'max-w-[1560px]' : 'max-w-[1120px]'">
    ...
    <p v-if="isLoading" class="py-16 text-center text-base text-gray-400">{{ t('admin.developC.loading') }}</p>
    <p v-else-if="isError || detail === undefined" class="py-16 text-center text-base text-red-600">
      {{ t('admin.developC.detailFail') }}
    </p>
    <template v-else>
      <!-- 헤더 — 어느 탭에서도 보인다 -->
      <header class="rounded-xl border border-gray-200 bg-white p-4">
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ t('admin.developC.owner') }}: {{ detail.owner.name === '' ? detail.owner.mbId : detail.owner.name }}
      ...
      <div class="flex flex-wrap items-end gap-2 border-b border-gray-200">
        ...
        <label v-if="tab !== 'content'" class="mb-1.5 ml-auto inline-flex cursor-pointer items-center gap-1.5 text-xs font-semibold text-gray-600">
          ...
          {{ sideWanted ? t('admin.developC.nav.sideClose') : t('admin.developC.nav.sideOpen') }}
        </label>
      </div>
