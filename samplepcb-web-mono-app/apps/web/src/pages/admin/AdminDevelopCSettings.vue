...
import type { DevelopMilestoneTriggerType, DevelopVatModeType } from '@sp/api-contract/develop-c';
import { apiErrorMessage } from '@sp/ui';
import { useAdminDevelopSettings, useSaveAdminDevelopSettings } from '../../admin/useAdminDevelopC';
...
const issues = computed<string[]>(() => {
  const list: string[] = [];
  if (milestones.value.length === 0) list.push(t('admin.developC.settings.errNoMilestone'));
  if (milestones.value.some((m) => m.title.trim() === '')) list.push(t('admin.developC.settings.errMilestoneTitle'));
  if (milestones.value.some((m) => !Number.isFinite(ratioBpOf(m)) || ratioBpOf(m) < 1 || ratioBpOf(m) > 10_000)) {
    list.push(t('admin.developC.settings.errMilestoneRatio'));
  }
  if (milestones.value.length > 0 && ratioSum.value !== 10_000) list.push(t('admin.developC.settings.errRatioSum'));
  if (badEmails.value.length > 0) list.push(t('admin.developC.settings.errEmail', { emails: badEmails.value.join(', ') }));
  return list;
});
...
async function onSubmit(): Promise<void> {
  ...
  if (issues.value.length > 0) {
    ...
    notice.value = t('admin.developC.settings.blocked');
    return;
  }
  try {
    ...
    notice.value = t('admin.developC.settings.saved');
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.settings.saveFail'));
  }
}
</script>
...
<template>
  <div class="space-y-4">
    <h1 class="text-2xl font-bold">{{ t('admin.developC.settings.title') }}</h1>
    <p v-if="isLoading" class="text-base text-gray-500">{{ t('admin.developC.loading') }}</p>
    ...
    <form v-else class="max-w-4xl space-y-5" @submit.prevent="onSubmit">
      ...
      <section class="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
        <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.settings.quoteDefaults') }}</h2>

        <label class="block text-base">
          <span class="font-medium text-gray-800">{{ t('admin.developC.settings.terms') }}</span>
          ...
          <span class="mt-0.5 block text-sm text-gray-500">{{ t('admin.developC.settings.termsHint') }}</span>
        </label>

        <label class="block text-base">
          <span class="font-medium text-gray-800">{{ t('admin.developC.settings.exclusions') }}</span>
          ...
          <span class="mt-0.5 block text-sm text-gray-500">{{ t('admin.developC.settings.exclusionsHint') }}</span>
        </label>

        <div class="grid gap-3 sm:grid-cols-4">
          <label class="block text-base">
            <span class="font-medium text-gray-800">{{ t('admin.developC.settings.warrantyDays') }}</span>
      ...
      <section class="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
        <div class="flex flex-wrap items-center gap-2">
          <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.settings.milestones') }}</h2>
          <span class="text-sm" :class="ratioSum === 10000 ? 'text-gray-500' : 'font-bold text-red-600'">
            {{ t('admin.developC.settings.ratioSum', { percent: (ratioSum / 100).toFixed(2) }) }}
          </span>
          <button
            type="button"
            ...
            @click="addMilestone"
          >
            {{ t('admin.developC.settings.addMilestone') }}
          </button>
        </div>
        <div v-for="(m, i) in milestones" :key="`ms-${i}`" class="flex flex-wrap items-center gap-2">
          <input
            ...
            type="text"
            :maxlength="100"
            :placeholder="t('admin.developC.settings.milestoneTitle')"
          ...
          >
            {{ t('admin.developC.settings.removeMilestone') }}
          </button>
        </div>
        <p class="text-sm text-gray-500">{{ t('admin.developC.settings.milestonesHint') }}</p>
      </section>

      <!-- 알림·AI -->
      <section class="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
        <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.settings.notifyAi') }}</h2>
        <label class="block text-base">
          <span class="font-medium text-gray-800">{{ t('admin.developC.settings.notifyEmails') }}</span>
          <textarea v-model="notifyEmails" rows="3" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 font-mono text-xs" />
          <span class="mt-0.5 block text-sm text-gray-500">{{ t('admin.developC.settings.notifyEmailsHint') }}</span>
        </label>
        <label class="flex items-center gap-1.5 text-base text-gray-700">
          <input v-model="aiAutoDraft" type="checkbox">
          {{ t('admin.developC.settings.aiAutoDraft') }}
        </label>
        <label class="flex items-center gap-1.5 text-base text-gray-700">
          <input v-model="aiDiagramAutoDraft" type="checkbox">
          {{ t('admin.developC.settings.aiDiagramAutoDraft') }}
        </label>
      </section>
      ...
      <div class="flex items-center gap-3">
        ...
        >
          {{ save.isPending.value ? t('admin.developC.saving') : t('admin.developC.settings.save') }}
        </button>
        ...
        <span v-if="data !== undefined && data.data.updatedAt !== null" class="ml-auto text-sm text-gray-400">
          {{ t('admin.developC.settings.updatedAt') }} {{ formatDateTime(data.data.updatedAt) }}
        </span>
      </div>
    </form>
  </div>
