...
<template>
  <div class="space-y-4">
    <h1 class="text-xl font-bold">{{ t('admin.develop.title') }}</h1>
    ...
    <div class="flex flex-wrap items-center gap-2">
      <div class="flex flex-wrap rounded-lg border border-gray-200 bg-white p-1 text-xs font-semibold">
        ...
        >
          ...
          <span v-if="data !== undefined" class="ml-1 text-[11px] opacity-70">{{ data.data.counts[tabKey] }}</span>
        </button>
      </div>
      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3">
              <span class="whitespace-nowrap rounded-full px-2 py-0.5 text-[11px] font-bold" :class="developStatusBadgeClass(r.status)">
                {{ DEVELOP_REQUEST_STATUS_LABELS[r.status] }}
              </span>
            </td>
            <td class="px-4 py-3 text-xs text-gray-600">
              <p>{{ r.owner.name === '' ? r.owner.mbId : r.owner.name }}</p>
              <p class="text-[11px] text-gray-400">{{ r.owner.email ?? r.owner.mbId }}</p>
            </td>
            <td class="px-4 py-3 text-xs text-gray-600">
              <p>{{ r.contact.name }}<span v-if="r.contact.company !== null"> · {{ r.contact.company }}</span></p>
              <p class="text-[11px] text-gray-400">{{ r.contact.phone }}</p>
            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-xs text-gray-600">
              <template v-if="r.latestQuote !== null">
                ...
                <p class="text-[11px] font-semibold text-gray-700">{{ formatKrw(r.latestQuote.totalAmount) }}</p>
              </template>
              <span v-else class="text-gray-300">—</span>
            </td>
            <td class="px-4 py-3 text-xs text-gray-600">{{ r.assigneeMbId ?? '—' }}</td>
    ...
    <div v-if="data !== undefined" class="flex items-center justify-between">
      <p class="text-sm text-gray-500">{{ t('admin.develop.total', { total: data.data.total }) }}</p>
      <UiPagination
        :page="filters.page"
