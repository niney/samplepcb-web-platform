...
const tabs = computed<PartnerWorkqueueTab<TabKey>[]>(() => [
  { key: 'todo', label: pt('확인할 발주'), count: work.toConfirm.value.length, emphasize: true },
  { key: 'active', label: pt('진행 중'), count: activePos.value.length },
  { key: 'done', label: pt('완료'), count: donePos.value.length },
  { key: 'all', label: pt('전체'), count: work.poItems.value.length },
]);
const emptyText = computed(() => {
  if (q.value.trim() !== '') return pt('검색 결과가 없습니다.');
  if (tab.value === 'todo') return pt('확인할 발주가 없습니다 🎉');
  if (tab.value === 'active') return pt('진행 중인 발주가 없습니다.');
  if (tab.value === 'done') return pt('완료된 발주가 아직 없습니다.');
...
<template>
  <div class="space-y-4">
    <PartnerPageHeader
      :title="pt('발주 관리')"
      :subtitle="pt('받은 발주서입니다 — 발주 확인 → 📦 보내기에서 담아 발송 → 입고 확인까지의 진행을 봅니다.')"
    />
