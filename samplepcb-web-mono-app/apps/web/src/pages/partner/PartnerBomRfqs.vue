...
const tabs = computed<PartnerWorkqueueTab<TabKey>[]>(() => [
  { key: 'todo', label: pt('회신할 견적'), count: work.pendingRfqs.value.length, emphasize: true },
  { key: 'done', label: pt('회신함'), count: work.doneRfqs.value.length },
  { key: 'all', label: pt('전체'), count: work.rfqItems.value.length },
]);
const emptyText = computed(() => {
  if (q.value.trim() !== '') return pt('검색 결과가 없습니다.');
  if (tab.value === 'todo') return pt('회신할 견적이 없습니다 🎉');
  if (tab.value === 'done') return pt('회신한 견적이 아직 없습니다.');
  return pt('받은 견적요청이 없습니다.');
});
</script>
...
<template>
  <div class="space-y-4">
    <PartnerPageHeader
      :title="pt('견적요청')"
      :subtitle="pt('받은 견적요청에 회신합니다. 회신한 내용은 마감 전까지 다시 고칠 수 있습니다.')"
    />
    ...
    <template v-else>
      <PartnerWorkqueueTabs v-model="tab" :tabs="tabs" accent="indigo">
        <input
          v-model="q"
          type="search"
          :placeholder="pt('견적 제목 검색')"
          :aria-label="pt('견적 제목 검색')"
      ...
      <p v-if="work.rfqsQuery.isLoading.value" class="text-sm text-gray-400">{{ pt('불러오는 중…') }}</p>
      ...
      <template v-else>
        ...
        <div v-if="filtered.length > PAGE_SIZE" class="flex items-center justify-between">
          <p class="text-sm text-gray-500">{{ pt('총 {value1}건', { value1: filtered.length }) }}</p>
          <UiPagination
            :page="page"
            :page-size="PAGE_SIZE"
