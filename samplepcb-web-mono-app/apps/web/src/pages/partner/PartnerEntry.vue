...
<template>
  <p v-if="isLoading" class="text-sm text-gray-400">{{ pt('불러오는 중…') }}</p>
  <div
    v-else-if="access !== null && !access.isPartner"
    class="rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800"
  >
    {{ pt('승인된 파트너 계정이 아닙니다. 파트너 등록·계정 연결은 샘플피씨비 담당자에게 문의해 주세요.') }}
  </div>
  <div
    v-else-if="noTrack"
    class="rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800"
  >
    {{ pt('참여 중인 트랙(BOM 부품·PCB 제작)이 없습니다 — 샘플피씨비 담당자에게 문의해 주세요.') }}
  </div>
</template>
