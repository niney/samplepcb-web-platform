...
<template>
  <div class="space-y-6">
    ...
    <div v-else-if="noTrack" class="rounded-xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800">
      {{ pt('이 조직은 PCB 제작 트랙에 참여하지 않습니다.') }}<RouterLink v-if="hasBomTrack" :to="{ name: 'partner-bom' }" class="font-semibold underline">{{ pt('BOM 부품으로 이동 →') }}</RouterLink>
    </div>
    ...
    <template v-else>
      ...
      >
        {{ pt('🔧 A/S 재생산 검토 요청 {value1}건 — 재생산 가능/불가를 회신해 주세요 →', { value1: pn(pendingAsCount) }) }}
      </RouterLink>
      ...
      >
        <span class="text-sm font-bold text-amber-800">{{ pt('💰 미수금 {value1}건', { value1: pn(unpaidCount) }) }}</span>
        <span
        ...
        <span class="ml-auto text-xs font-semibold text-amber-700">{{ pt('수금 현황 보기 →') }}</span>
      </RouterLink>
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        >
          ...
          <p class="mt-1 text-2xl font-bold" :class="pendingRfqs.length > 0 ? 'text-cyan-700' : 'text-gray-300'">
            {{ pn(pendingRfqs.length) }} <span class="text-sm font-semibold">{{ pt('건') }}</span>
          </p>
        </a>
        <a
          href="#po"
        ...
        >
          <p class="text-sm text-gray-500">{{ pt('진행할 발주') }}</p>
          <p class="mt-1 text-2xl font-bold" :class="myTurnPos.length > 0 ? 'text-teal-700' : 'text-gray-300'">
            {{ pn(myTurnPos.length) }} <span class="text-sm font-semibold">{{ pt('건') }}</span>
          </p>
        </a>
        <RouterLink
        ...
        >
          ...
          <p class="mt-1 text-2xl font-bold" :class="shelf.length > 0 ? 'text-teal-700' : 'text-gray-300'">
            {{ pn(shelf.length) }} <span class="text-sm font-semibold">{{ pt('건') }}</span>
          </p>
          <p v-if="boxes.length > 0" class="mt-0.5 text-xs font-bold text-teal-600">{{ pt('준비 중인 박스 {value1}개 — 계속하기 →', { value1: pn(boxes.length) }) }}</p>
          <p v-else-if="shelf.length > 0" class="mt-0.5 text-xs font-semibold text-teal-600">{{ pt('받는 곳이 같으면 묶어서 보내기 →') }}</p>
          <!-- producing 은 이름과 달리 "생산완료 전 수주 전부"다(발주접수·EQ 포함) —
               '생산 진행 중'이라 말하면 발주 직후에도 생산 중이라 읽힌다(MD 실주행 확정). -->
          <p v-else-if="producing.length > 0" class="mt-0.5 text-xs font-semibold text-gray-500">{{ pt('진행 중 {value1}건 — 생산완료되면 담을 수 있습니다', { value1: pn(producing.length) }) }}</p>
        </RouterLink>
        <RouterLink
          :to="{ name: 'partner-pcb-ship' }"
        ...
        >
          ...
          <p class="mt-1 text-2xl font-bold" :class="activeShipments.length > 0 ? 'text-gray-800' : 'text-gray-300'">
            {{ pn(activeShipments.length) }} <span class="text-sm font-semibold">{{ pt('건') }}</span>
          </p>
          <p v-if="activeShipments.length > 0" class="mt-0.5 text-xs font-semibold text-blue-600">{{ pt('보내기 화면에서 진행 →') }}</p>
        </RouterLink>
      </div>
      ...
      <section v-if="pendingRfqs.length > 0" id="reply">
        <div class="flex items-baseline justify-between gap-3">
          <h2 class="text-sm font-bold text-gray-700">{{ pt('회신할 견적 ({value1})', { value1: pn(pendingRfqs.length) }) }}</h2>
          <RouterLink
      ...
      <section v-if="myTurnPos.length > 0" id="po">
        <div class="flex items-baseline justify-between gap-3">
          <h2 class="text-sm font-bold text-gray-700">{{ pt('진행할 발주 ({value1})', { value1: pn(myTurnPos.length) }) }}</h2>
          <RouterLink
      ...
      <details v-if="watchingPos.length > 0" open class="rounded-xl border border-gray-200 bg-surface">
        <summary class="cursor-pointer px-4 py-3 text-sm font-bold text-gray-700">{{ pt('진행 중 발주 ({value1})', { value1: pn(watchingPos.length) }) }}</summary>
