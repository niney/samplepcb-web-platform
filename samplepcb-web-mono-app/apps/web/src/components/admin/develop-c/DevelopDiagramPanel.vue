...
import type { AdminDevelopDiagramStateType } from '@sp/api-contract/develop-c';
  ...
  try {
    ...
    notice.value =
      res.data.skipped === null
        ? t('admin.developC.diagram.runStarted')
        : t('admin.developC.diagram.runSkipped', { reason: res.data.skipped });
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.diagram.runFail'), {
      AI_CONSENT_REQUIRED: t('admin.developC.noAiConsentHint'),
      AI_RUNNING: t('admin.developC.diagram.errorRunning'),
    });
  }
}
...
async function onAction(action: 'publish' | 'unpublish'): Promise<void> {
  ...
  try {
    ...
    notice.value = t(`admin.developC.diagram.done.${action}`);
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.diagram.actionFail'), {
      DIAGRAM_EMPTY: t('admin.developC.diagram.errorEmpty'),
    });
  }
}
...
async function onPickFile(event: Event): Promise<void> {
  ...
  try {
    ...
    noticeError.value = false;
    notice.value = t('admin.developC.diagram.uploaded');
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.diagram.uploadFail'), {
      FILE_TOO_LARGE: t('admin.developC.diagram.errorTooLarge'),
      FILE_UNSUPPORTED: t('admin.developC.diagram.errorUnsupported'),
      FILE_REQUIRED: t('admin.developC.diagram.errorNoFile'),
    });
  } finally {
    input.value = '';
  }
...
<template>
  <section class="rounded-xl border border-indigo-100 bg-indigo-50/20 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.diagram.title') }}</h2>
      <span
        v-if="diagram.published"
        class="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-700"
      >
        {{ t('admin.developC.diagram.published') }}
        <template v-if="diagram.publishedAt !== null"> · {{ formatDateTime(diagram.publishedAt) }}</template>
      </span>
      <span v-if="diagram.publishedStale" class="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">
        {{ t('admin.developC.diagram.publishedStale') }}
      </span>
      ...
      <div class="ml-auto flex flex-wrap items-center gap-1.5">
        ...
        >
          {{ upload.isPending.value ? t('admin.developC.diagram.uploading') : t('admin.developC.diagram.upload') }}
        </button>
        <button
          type="button"
        ...
        >
          {{ diagram.published ? t('admin.developC.diagram.republish') : t('admin.developC.diagram.publish') }}
        </button>
        <button
          v-if="diagram.published"
          type="button"
        ...
        >
          {{ t('admin.developC.diagram.unpublish') }}
        </button>
      </div>
    </div>
    ...
    <input
      ...
      @change="onPickFile"
    >
    <p class="mt-1 text-xs text-gray-500">{{ t('admin.developC.diagram.uploadHint') }}</p>
