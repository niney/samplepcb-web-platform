...
<template>
  ...
  >
    ...
    <p class="-mt-1 text-xs text-gray-500">
      ...
      <span class="text-gray-400"> · {{ t('admin.developC.home.assignee') }} {{ item.assigneeMbId ?? '—' }}</span>
    </p>
    ...
    <div class="grid gap-1 text-xs">
      <p v-if="item.ops.pendingApprovals > 0" :class="item.ops.replyOverdue ? 'font-bold text-red-600' : 'text-amber-700'">
        {{ t('admin.developC.home.pending', { n: item.ops.pendingApprovals }) }}
        <span v-if="item.ops.nextReplyDueOn !== null"> · {{ t('admin.developC.queue.replyDue', { date: item.ops.nextReplyDueOn }) }}</span>
        <span v-if="item.ops.replyOverdue"> · {{ t('admin.developC.queue.overdue') }}</span>
      </p>
      <template v-if="item.ops.openInquiries > 0">
        <p class="font-bold text-amber-700">{{ t('admin.developC.queue.inquiryOpen', { n: item.ops.openInquiries }) }}</p>
    ...
    <div class="flex flex-wrap gap-1.5">
      ...
      >
        {{ t(`admin.developC.home.open.${link.key}`) }}
      </RouterLink>
    </div>
  </article>
</template>
