...
import { DEVELOP_DOC_TYPE_LABELS, developDocStatusLabel } from '@sp/api-contract/develop-c';
  ...
  <article :id="`develop-doc-${String(doc.documentId)}`" class="rounded-lg border border-gray-200 bg-white p-3">
    <header class="flex flex-wrap items-center gap-2">
      ...
      <span v-if="doc.approval" class="rounded-full bg-indigo-50 px-2 py-0.5 text-[11px] font-bold text-indigo-700">
        {{ t('admin.developC.docs.card.approval') }}
      </span>
      ...
      <span v-if="doc.replyDueOn !== null" class="text-xs font-semibold text-amber-700">
        {{ t('admin.developC.docs.status.replyDue', { date: doc.replyDueOn }) }}
      </span>
      <span v-if="doc.files.length > 0" class="text-xs text-gray-400">{{ t('admin.developC.docs.card.files', { count: doc.files.length }) }}</span>
    </header>
    ...
    <div v-if="older.length > 0" class="mt-2 border-t border-gray-100 pt-2 print:hidden">
      <button type="button" class="text-[11px] font-bold text-gray-500 hover:text-gray-800" @click="olderOpen = !olderOpen">
        {{ t('admin.developC.docs.card.older', { count: older.length }) }}
        <span class="ml-1 font-normal text-gray-400">{{ olderOpen ? t('admin.developC.side.memoClose') : t('admin.developC.side.memoOpen') }}</span>
      </button>
