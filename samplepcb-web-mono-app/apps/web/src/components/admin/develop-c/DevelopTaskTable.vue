...
import { useAdminDevelopTasksPut, useInvalidateAdminDevelop } from '../../../admin/useAdminDevelopC';
...
const issueOf = (index: number): string =>
  issues.value
    ...
    .map((i) =>
      i.code === 'NAME'
        ? t('admin.developC.docs.task.errName')
        : i.code === 'ORDER'
          ? t('admin.developC.docs.task.errOrder')
          : t('admin.developC.docs.task.errCoherence'),
    )
    .join(' · ');
...
function removeRow(index: number): void {
  ...
  if (row.taskId !== null && Number(row.progressPct.replace(/[^\d.-]/g, '')) > 0) {
    ...
    notice.value = t('admin.developC.docs.task.errRemoveProgress');
    return;
  }
...
function seed(kind: 'defaults' | 'schedule'): void {
  ...
  if (rows.value.some((r) => r.taskId !== null && Number(r.progressPct.replace(/[^\d.-]/g, '')) > 0)) {
    ...
    noticeError.value = true;
    notice.value = t('admin.developC.docs.task.errSeedProgress');
    return;
  }
...
async function onSave(): Promise<void> {
  ...
  try {
    ...
    notice.value = apiErrorMessage(error, t('admin.developC.docs.task.saveFail'), {
      REVISION_CONFLICT: t('admin.developC.docs.task.errConflict'),
      TASK_HAS_PROGRESS: t('admin.developC.docs.task.errRemoveProgress'),
      TASK_NOT_FOUND: t('admin.developC.docs.task.errNotFound'),
    });
  ...
  <section class="rounded-xl border border-gray-200 bg-white p-4">
    <div class="flex flex-wrap items-center gap-2">
      ...
      <span v-if="overdue > 0" class="rounded-full bg-red-100 px-2 py-0.5 text-[11px] font-bold text-red-700">
        {{ t('admin.developC.docs.task.previewOverdue', { n: overdue }) }}
      </span>
    ...
    <div v-if="notice !== ''" class="mt-1.5 flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        @click="reload"
      >
        {{ t('admin.developC.docs.task.reload') }}
      </button>
    </div>
