...
import type { AdminDevelopDocumentViewType } from '@sp/api-contract/develop-c';
import { apiErrorMessage, canPreview } from '@sp/ui';
import type { PreviewTarget } from '@sp/ui';
import { useAdminDevelopDocumentRevise } from '../../../admin/useAdminDevelopC';
...
async function onRevise(): Promise<void> {
  ...
  try {
    ...
    notice.value = t('admin.developC.docs.view.revised');
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.docs.view.reviseFail'), {
      DOC_DRAFT_EXISTS: t('admin.developC.docs.view.errDraftExists'),
      DOC_IS_DRAFT: t('admin.developC.docs.view.errIsDraft'),
    });
  }
}
...
async function onDownload(fileId: number, name: string): Promise<void> {
  ...
  try {
    ...
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.content.downloadFail'));
  }
}
...
<template>
  <div ref="cardEl" class="grid gap-2.5">
    ...
    <p v-else class="text-xs text-gray-400">{{ t('admin.developC.docs.view.empty') }}</p>
    ...
    <p v-if="doc.internalNote !== null && doc.internalNote !== ''" class="rounded border border-gray-200 bg-gray-50 p-2 text-[11px] whitespace-pre-line text-gray-600 print:hidden">
      <b>{{ t('admin.developC.docs.editor.internalNote') }}</b> · {{ doc.internalNote }}
    </p>
    ...
    <div v-if="doc.mailSubject !== null" class="rounded border border-gray-200 print:hidden">
      <button type="button" class="flex w-full items-center gap-2 px-2 py-1.5 text-left text-[11px] font-bold text-gray-600" @click="mailOpen = !mailOpen">
        {{ t('admin.developC.docs.view.mail') }}
        <span class="ml-auto font-normal text-gray-400">{{ mailOpen ? t('admin.developC.side.memoClose') : t('admin.developC.side.memoOpen') }}</span>
      </button>
    ...
    <div class="flex flex-wrap items-center gap-2 border-t border-gray-100 pt-2 print:hidden">
      ...
      <button type="button" class="ml-auto rounded-md border border-gray-300 px-2.5 py-1 text-xs font-bold text-gray-700 hover:bg-gray-50" @click="onPrint">
        {{ t('admin.developC.docs.view.print') }}
      </button>
      <button
        v-if="doc.isCurrent"
        type="button"
        ...
        @click="onRevise"
      >
        {{ t('admin.developC.docs.view.revise') }}
      </button>
    </div>
