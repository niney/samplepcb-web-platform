...
import type { AdminDevelopReviewVersionListResponseType, DevelopReviewVersionMetaType } from '@sp/api-contract/develop-c';
import { DevReviewView, apiErrorMessage } from '@sp/ui';
import { useAdminDevelopReviewRestore, useAdminDevelopReviewVersion } from '../../../admin/useAdminDevelopC';
...
const label = (seq: number | null): string => {
  ...
  return v === undefined ? '' : `v${String(v.seq)} · ${t(`admin.developC.review.versions.kind.${v.kind}`)} · ${formatDateTime(v.createdAt)}`;
};
...
async function onRestore(seq: number): Promise<void> {
  try {
    ...
    emit('notice', t('admin.developC.review.versions.restored', { seq }), false);
  } catch (error) {
    emit('notice', apiErrorMessage(error, t('admin.developC.review.versions.restoreFail')), true);
  }
}
</script>
...
<template>
  <div class="grid gap-3">
    <p class="text-xs text-gray-500">{{ t('admin.developC.review.versions.hint') }}</p>

    <p v-if="isLoading" class="py-6 text-center text-sm text-gray-400">{{ t('admin.developC.review.versions.loading') }}</p>
    <p v-else-if="isError" class="py-6 text-center text-sm text-red-600">{{ t('admin.developC.review.versions.loadFail') }}</p>
    <p v-else-if="items.length === 0" class="rounded-lg border border-dashed border-gray-300 px-4 py-6 text-center text-sm text-gray-400">
      {{ t('admin.developC.review.versions.empty') }}
    </p>
        ...
        >
          <div class="flex flex-wrap items-center gap-1.5">
            ...
            <span class="rounded-full px-1.5 py-0.5 text-[11px] font-bold" :class="KIND_CLASS[v.kind]">
              {{ t(`admin.developC.review.versions.kind.${v.kind}`) }}
            </span>
            <span v-if="v.seq === current.draftSeq" class="rounded-full border border-indigo-200 px-1.5 py-0.5 text-[11px] font-semibold text-indigo-700">{{ t('admin.developC.review.versions.current.draft') }}</span>
            <span v-if="v.seq === current.workingSeq" class="rounded-full border border-gray-300 px-1.5 py-0.5 text-[11px] font-semibold text-gray-700">{{ t('admin.developC.review.versions.current.working') }}</span>
            <span v-if="v.seq === current.publicSeq" class="rounded-full border border-emerald-300 px-1.5 py-0.5 text-[11px] font-semibold text-emerald-700">{{ t('admin.developC.review.versions.current.public') }}</span>
          ...
          <div class="flex flex-wrap items-center gap-1 pt-0.5">
            <label class="inline-flex items-center gap-1 text-xs font-semibold text-gray-600">
              <input v-model="seqA" type="radio" name="dev-review-version-a" :value="v.seq" class="h-3.5 w-3.5">{{ t('admin.developC.review.versions.pickA') }}
            </label>
            <label class="inline-flex items-center gap-1 text-xs font-semibold text-gray-600">
              <input v-model="seqB" type="radio" name="dev-review-version-b" :value="v.seq" class="h-3.5 w-3.5">{{ t('admin.developC.review.versions.pickB') }}
            </label>
            <button
              type="button"
            ...
            >
              {{ viewSeq === v.seq ? t('admin.developC.review.versions.hideView') : t('admin.developC.review.versions.view') }}
            </button>
            <template v-if="v.seq !== current.workingSeq">
              <template v-if="confirmSeq === v.seq">
                <span class="text-[11px] font-semibold text-amber-700">{{ t('admin.developC.review.versions.restoreConfirm', { seq: v.seq }) }}</span>
                <span v-if="dirty" class="text-[11px] font-semibold text-red-600">{{ t('admin.developC.review.versions.restoreDirtyWarn') }}</span>
                <button
                  type="button"
                ...
                >
                  {{ t('admin.developC.review.versions.restoreYes') }}
                </button>
              ...
              >
                {{ t('admin.developC.review.versions.restore') }}
              </button>
            </template>
          </div>
        </li>
        ...
        <div v-if="viewSeq !== null" class="rounded-lg border border-gray-200 bg-white p-4">
          <div class="mb-3 flex items-center gap-2">
            ...
            <button type="button" class="ml-auto rounded-md border border-gray-300 px-2 py-0.5 text-xs font-bold text-gray-600" @click="viewSeq = null">
              {{ t('admin.developC.review.versions.hideView') }}
            </button>
          </div>
          <DevReviewView
          ...
          />
          <p v-else class="py-6 text-center text-sm text-gray-400">{{ t('admin.developC.review.versions.loading') }}</p>
        </div>
        <template v-else>
          <p v-if="seqA === null || seqB === null" class="rounded-lg border border-dashed border-gray-300 px-4 py-6 text-center text-sm text-gray-400">
            {{ t('admin.developC.review.versions.needTwo') }}
          </p>
          <DevelopReviewDiff
