...
import { DEVELOP_REQUEST_STATUS_LABELS, isDevelopClosed } from '@sp/api-contract/develop-c';
import type { AdminDevelopStatusBodyType, DevelopRequestStatusType } from '@sp/api-contract/develop-c';
import { useAuthStore } from '@sp/shared';
import { apiErrorMessage } from '@sp/ui';
import { useAdminDevelopStatus, usePatchAdminDevelop } from '../../../admin/useAdminDevelopC';
...
async function go(to: AdminDevelopStatusBodyType['to'], withReason: string | null): Promise<void> {
  ...
  try {
    ...
    notice.value = t('admin.developC.status.done', { status: DEVELOP_REQUEST_STATUS_LABELS[to] });
    reasonTarget.value = null;
    reason.value = '';
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.status.fail'), {
      REASON_REQUIRED: t('admin.developC.status.reasonRequired'),
      INVALID_TRANSITION: t('admin.developC.status.invalid'),
    });
  }
}
...
async function onAssignee(event: Event): Promise<void> {
  ...
  try {
    ...
    noticeError.value = false;
    notice.value = t('admin.developC.status.assigneeSaved');
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.status.fail'));
  }
  ...
  <div class="grid gap-2">
    <div class="flex flex-wrap items-center gap-2">
      <label class="flex items-center gap-1.5 text-sm text-gray-600">
        {{ t('admin.developC.status.assignee') }}
        <select
          :value="assigneeMbId ?? ''"
          ...
          @change="onAssignee"
        >
          <option value="">{{ t('admin.developC.status.unassigned') }}</option>
      ...
      <div class="ml-auto flex flex-wrap items-center gap-1.5">
        ...
        >
          {{ t('admin.developC.status.toReviewing') }}
        </button>
        <button
          v-if="canStart"
          type="button"
        ...
        >
          {{ t('admin.developC.status.toInProgress') }}
        </button>
        <button
          v-if="canComplete"
          type="button"
        ...
        >
          {{ t('admin.developC.status.toCompleted') }}
        </button>
        <button
          v-if="canDecline"
          type="button"
        ...
        >
          {{ t('admin.developC.status.toDeclined') }}
        </button>
        <button
          v-if="canCancel"
          type="button"
        ...
        >
          {{ t('admin.developC.status.toCancelled') }}
        </button>
      </div>
    </div>
    ...
    <div v-if="reasonTarget !== null" class="grid gap-2 rounded-lg border border-red-200 bg-red-50/40 p-3">
      <p class="text-sm font-bold text-red-700">
        {{ reasonTarget === 'declined' ? t('admin.developC.status.declineTitle') : t('admin.developC.status.cancelTitle') }}
      </p>
      <textarea
        v-model="reason"
        rows="2"
        :maxlength="1000"
        :placeholder="t('admin.developC.status.reasonPlaceholder')"
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          @click="confirmReason"
        >
          {{ t('admin.developC.status.reasonConfirm') }}
        </button>
        <button
          type="button"
