...
import { usePatchAdminDevelop } from '../../../admin/useAdminDevelopC';
...
async function save(body: { internalMemo?: string | null; reviewDays?: number }): Promise<void> {
  ...
  try {
    ...
    notice.value = t('admin.developC.side.saved');
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.side.saveFail'));
  }
}
...
<template>
  <div class="grid gap-2.5">
    ...
    <div class="flex flex-wrap items-center gap-1.5 text-xs">
      ...
      >
        <b class="text-gray-800">{{ t(`admin.developC.side.ts.${row.key}`) }}</b>
        {{ formatDateTime(row.at) }}
      </span>
      <span v-if="detail.cancelReason !== null" class="rounded-full bg-red-50 px-2 py-0.5 text-red-700">
        {{ t('admin.developC.side.cancelReason') }}: {{ detail.cancelReason }}
      </span>
      <span v-if="detail.declinedReason !== null" class="rounded-full bg-red-50 px-2 py-0.5 text-red-700">
        {{ t('admin.developC.side.declinedReason') }}: {{ detail.declinedReason }}
      </span>

      <label class="ml-auto inline-flex items-center gap-1.5 text-gray-600" :title="t('admin.developC.side.reviewDaysHint')">
        {{ t('admin.developC.side.reviewDays') }}
        <input v-model="reviewDays" type="number" min="1" max="90" class="h-8 w-16 rounded-md border border-gray-300 px-2 text-xs">
        {{ t('admin.developC.side.days') }}
        <button
          type="button"
    ...
    <div class="rounded-lg border border-dashed border-gray-300 bg-gray-50/60">
      ...
      >
        <span class="font-bold text-gray-700">{{ t('admin.developC.side.memo') }}</span>
        <span v-if="!memoOpen && memoPreview !== ''" class="min-w-0 flex-1 truncate text-gray-500">{{ memoPreview }}</span>
        <span v-else-if="!memoOpen" class="text-gray-400">{{ t('admin.developC.side.memoPlaceholder') }}</span>
        <span class="ml-auto shrink-0 text-xs text-gray-500">{{ memoOpen ? t('admin.developC.side.memoClose') : t('admin.developC.side.memoOpen') }}</span>
      </button>
      <div v-if="memoOpen" class="grid gap-1.5 border-t border-gray-200 px-3 py-2.5">
        <textarea
          ...
          rows="5"
          :maxlength="20000"
          :placeholder="t('admin.developC.side.memoPlaceholder')"
          class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-xs leading-relaxed"
        />
        <div class="flex items-center gap-2">
          <span class="text-xs text-gray-500">{{ t('admin.developC.side.memoHint') }}</span>
          <button
            type="button"
