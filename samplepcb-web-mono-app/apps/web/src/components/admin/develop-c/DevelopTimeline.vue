...
import type { AdminDevelopEventPayloadType, DevelopEventViewType, DevelopRequestStatusType } from '@sp/api-contract/develop-c';
import { apiErrorMessage, canPreview } from '@sp/ui';
import type { PreviewTarget } from '@sp/ui';
import { useAdminDevelopEventCreate } from '../../../admin/useAdminDevelopC';
...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    notice.value = t('admin.developC.timeline.created');
    resetForm();
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.timeline.createFail'), {
      EMPTY_EVENT: t('admin.developC.timeline.errorEmpty'),
      INVALID_TRANSITION: t('admin.developC.timeline.errorNotInProgress'),
      PAYLOAD_SCHEMA_MISMATCH: t('admin.developC.timeline.errorPayload'),
      FILE_UPLOAD_FAILED: t('admin.developC.timeline.errorUpload'),
    });
  }
}
...
<template>
  <section class="rounded-xl border border-gray-200 bg-white p-4">
    <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.timeline.title') }}</h2>
    ...
    <form class="mt-3 grid gap-2 rounded-lg border border-gray-200 bg-gray-50/60 p-3" @submit.prevent="onSubmit">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          type="text"
          :maxlength="200"
          :placeholder="t('admin.developC.timeline.titlePlaceholder')"
      ...
      <textarea
        ...
        rows="3"
        :maxlength="10000"
        :placeholder="t('admin.developC.timeline.bodyPlaceholder')"
      ...
      <label v-if="type === 'note'" class="inline-flex items-center gap-1.5 text-sm text-gray-700">
        ...
        {{ t('admin.developC.timeline.visible') }}
      </label>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          :disabled="!canSubmit"
        >
          {{ create.isPending.value ? t('admin.developC.saving') : t('admin.developC.timeline.submit') }}
        </button>
      </div>
    ...
    <ul class="mt-4 grid gap-2">
      ...
      >
        <div class="flex flex-wrap items-center gap-2">
          ...
          <span v-if="!e.visibleToCustomer" class="rounded-full bg-gray-300 px-2 py-0.5 text-[11px] font-bold text-gray-700">
            {{ t('admin.developC.timeline.internal') }}
          </span>
          <button
        ...
        <ul v-if="e.files.length > 0" class="mt-1.5 grid gap-1">
          <li v-for="f in e.files" :key="f.fileId" class="flex min-w-0 items-center gap-2 rounded border border-gray-100 bg-white px-2 py-1">
            ...
            <span v-if="f.locked" class="shrink-0 rounded-full bg-amber-100 px-1.5 py-0.5 text-[11px] font-bold text-amber-700">
              {{ t('admin.developC.timeline.lockedFile') }}
            </span>
      ...
      <li v-if="events.length === 0" class="rounded-lg border border-dashed border-gray-200 px-4 py-6 text-center text-sm text-gray-400">
        {{ t('admin.developC.timeline.empty') }}
      </li>
    </ul>
  </section>
</template>
