...
import type { AdminDevelopDocumentViewType, DevelopProgressViewType } from '@sp/api-contract/develop-c';
...
<template>
  <section class="rounded-xl border border-gray-200 bg-white p-4">
    <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.docs.status.title') }}</h2>

    <div class="mt-3 grid gap-3 sm:grid-cols-3">
      <div class="rounded-lg border border-gray-100 bg-gray-50/60 p-3">
        <p class="text-xs font-semibold text-gray-500">{{ t('admin.developC.docs.status.progress') }}</p>
      ...
      <div class="rounded-lg border border-gray-100 bg-gray-50/60 p-3">
        ...
        <p class="mt-0.5 text-xs text-gray-500">
          {{ t('admin.developC.docs.status.baseStartOn') }}: {{ progress.baseStartOn ?? t('admin.developC.docs.status.undecided') }}
        </p>
      </div>
      <div class="rounded-lg border border-gray-100 bg-gray-50/60 p-3">
        <p class="text-xs font-semibold text-gray-500">{{ t('admin.developC.docs.status.expectedEndOn') }}</p>
        <p class="mt-0.5 text-lg font-bold text-gray-900">
          {{ progress.expectedEndOn ?? t('admin.developC.docs.status.undecided') }}
        </p>
        <p class="mt-0.5 text-xs text-gray-500">
          {{ t('admin.developC.docs.status.plannedEndOn') }}: {{ progress.plannedEndOn ?? t('admin.developC.docs.status.undecided') }}
        </p>
      </div>
    </div>
    ...
    <div class="mt-4 grid gap-3 lg:grid-cols-2">
      ...
      <div class="rounded-lg border p-3" :class="pending.length > 0 ? 'border-amber-200 bg-amber-50/50' : 'border-gray-100'">
        <p class="text-xs font-bold text-gray-700">
          {{ t('admin.developC.docs.status.pending') }}
          ...
          <li v-for="d in pending" :key="d.documentId">
            ...
            >
              ...
              <span v-if="d.replyDueOn !== null" class="ml-auto font-semibold text-amber-700">
                {{ t('admin.developC.docs.status.replyDue', { date: d.replyDueOn }) }}
              </span>
            </button>
          </li>
        </ul>
        <p v-else class="mt-1.5 text-xs text-gray-400">{{ t('admin.developC.docs.status.pendingNone') }}</p>
      </div>

      <!-- 최근 결정 -->
      <div class="rounded-lg border border-gray-100 p-3">
        <p class="text-xs font-bold text-gray-700">{{ t('admin.developC.docs.status.decisions') }}</p>
        ...
        <p v-else class="mt-1.5 text-xs text-gray-400">{{ t('admin.developC.docs.status.decisionsNone') }}</p>
      </div>
    </div>
  </section>
</template>
