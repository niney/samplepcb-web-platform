...
import { DEVELOP_REGISTRY } from '@sp/api-contract/develop-c';
import type { AdminDevelopReviewStateType, MarketDevReviewType } from '@sp/api-contract/develop-c';
...
const workingVersionLabel = computed(() => (workingSeq.value === null ? undefined : t('admin.developC.review.versions.workingLine', { seq: workingSeq.value })));
...
async function onSave(): Promise<void> {
  ...
  issues.value = found;
  if (found.length > 0) {
    setNotice(t('admin.developC.review.saveBlocked'), true);
    return;
  }
  try {
    await reviewPut.mutateAsync({ requestId: props.requestId, review });
    setNotice(t('admin.developC.review.saved'), false);
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.review.saveFail')), true);
  }
}
...
async function runAction(action: 'publish' | 'unpublish' | 'reset'): Promise<void> {
  try {
    ...
    setNotice(t(`admin.developC.review.done.${action}`), false);
  } catch (error) {
    setNotice(
      apiErrorMessage(error, t('admin.developC.review.actionFail'), {
        REVIEW_EMPTY: t('admin.developC.review.errorReviewEmpty'),
        DRAFT_EMPTY: t('admin.developC.review.errorDraftEmpty'),
      }),
      true,
    );
  }
  ...
  try {
    ...
    setNotice(
      res.data.skipped === null
        ? t('admin.developC.review.runStarted')
        : t('admin.developC.review.runSkipped', { reason: res.data.skipped }),
      res.data.skipped !== null,
    );
  } catch (error) {
    setNotice(
      apiErrorMessage(error, t('admin.developC.review.runFail'), {
        AI_CONSENT_REQUIRED: t('admin.developC.noAiConsentHint'),
        AI_RUNNING: t('admin.developC.review.errorRunning'),
      }),
      true,
    );
  }
...
async function onSaveSupplement(): Promise<void> {
  try {
    ...
    setNotice(t('admin.developC.review.supplementSaved'), false);
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.review.saveFail')), true);
  }
}
...
<template>
  <section class="rounded-xl border border-blue-100 bg-blue-50/20 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <h2 class="text-base font-bold text-gray-800">{{ t('admin.developC.review.title') }}</h2>
      <span
      ...
      <button
        ...
        :title="aiConsent ? '' : t('admin.developC.noAiConsentHint')"
        @click="onRegenerate"
      >
        {{ review.draftRunning ? t('admin.developC.review.running') : t('admin.developC.review.regenerate') }}
      </button>
    </div>

    <p v-if="!aiConsent" class="mt-2 text-sm font-semibold text-amber-700">{{ t('admin.developC.noAiConsentHint') }}</p>
    <p v-if="review.draftError !== null" class="mt-2 text-sm font-semibold text-red-600">
      {{ t('admin.developC.review.draftError', { error: review.draftError }) }}
    </p>
    ...
    <dl class="mt-2 grid grid-cols-[80px_1fr] gap-y-1 text-xs text-gray-500">
      <dt>{{ t('admin.developC.review.draftAt') }}</dt>
      <dd>{{ review.draftAt === null ? '—' : formatDateTime(review.draftAt) }}</dd>
      <dt>{{ t('admin.developC.review.editedAt') }}</dt>
      <dd>
        {{ review.editedAt === null ? '—' : formatDateTime(review.editedAt) }}
        <span v-if="review.editedBy !== null"> · {{ review.editedBy }}</span>
      </dd>
      <dt>{{ t('admin.developC.review.publishedAt') }}</dt>
      <dd>{{ review.publishedAt === null ? t('admin.developC.review.notPublished') : formatDateTime(review.publishedAt) }}</dd>
    </dl>

    <!-- AI 보충 메모 — 코퍼스에 담당자 자료로 합류한다. 고객 비노출. -->
    <div class="mt-4 grid gap-1">
      <span class="text-sm font-semibold text-gray-700">{{ t('admin.developC.review.supplement') }}</span>
      <textarea
        v-model="supplement"
        rows="3"
        :maxlength="20000"
        :placeholder="t('admin.developC.review.supplementPlaceholder')"
        class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-xs leading-relaxed"
      />
      <div class="flex items-center gap-2">
        <span class="text-xs text-gray-500">{{ t('admin.developC.review.supplementHint') }}</span>
        <button
          type="button"
        ...
        >
          {{ t('admin.developC.review.supplementSave') }}
        </button>
      </div>
    </div>
    ...
    <div v-if="source === null" class="mt-4 rounded-lg border border-dashed border-gray-300 bg-white px-4 py-8 text-center">
      <p class="text-base font-semibold text-gray-700">{{ t('admin.developC.review.emptyTitle') }}</p>
      <p class="mt-1 text-sm text-gray-500">{{ t('admin.developC.review.emptyHint') }}</p>
    </div>
    <template v-else>
      <div class="mt-4 flex flex-wrap items-center gap-2">
        <div class="flex rounded-lg border border-gray-200 bg-white p-0.5 text-sm font-semibold">
          <button
            ...
            @click="tab = 'edit'"
          >
            {{ t('admin.developC.review.tabEdit') }}
          </button>
          <button
            type="button"
            class="rounded-md px-3 py-1"
            ...
            @click="tab = 'preview'"
          >
            {{ t('admin.developC.review.tabPreview') }}
          </button>
          <button
            type="button"
            class="rounded-md px-3 py-1"
            ...
            @click="tab = 'versions'"
          >
            {{ t('admin.developC.review.tabVersions') }}
        ...
        <span v-if="fromDraftOnly" class="text-xs font-semibold text-amber-700">{{ t('admin.developC.review.fromDraft') }}</span>
        <span v-else-if="dirty" class="text-xs font-semibold text-amber-700">{{ t('admin.developC.review.dirty') }}</span>
        ...
        <div class="ml-auto flex flex-wrap items-center gap-1.5">
          <button
            ...
            @click="onSave"
          >
            {{ reviewPut.isPending.value ? t('admin.developC.saving') : t('admin.developC.review.save') }}
          </button>
          <template v-if="confirmReset">
            ...
            >
              {{ t('admin.developC.review.resetConfirm') }}
            </button>
          ...
          >
            {{ review.publicReview === null ? t('admin.developC.review.publish') : t('admin.developC.review.republish') }}
          </button>
          <button
          ...
          >
            {{ t('admin.developC.review.unpublish') }}
          </button>
        </div>
      </div>
      <p v-if="dirty" class="mt-1 text-xs text-gray-500">{{ t('admin.developC.review.publishUsesSaved') }}</p>
