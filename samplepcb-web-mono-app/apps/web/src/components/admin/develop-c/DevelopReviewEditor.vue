...
import type { MarketDevReviewType } from '@sp/api-contract/develop-c';
import {
...
<template>
  <fieldset class="grid gap-5" :disabled="props.disabled">
    ...
    <label class="grid gap-1 text-base">
      <span class="font-semibold text-gray-800">{{ t('admin.developC.editor.summary') }}</span>
      <textarea
    ...
    <div class="grid gap-2">
      <div class="flex items-center gap-2">
        <span class="text-base font-semibold text-gray-800">{{ t('admin.developC.editor.requirements') }}</span>
        ...
        >
          {{ t('admin.developC.editor.addRow') }}
        </button>
      </div>
      <div v-for="(row, i) in local.requirements" :key="`req-${i}`" class="grid gap-1 rounded-md border border-gray-100 p-2">
        <div class="flex items-start gap-2">
          <input
            ...
            :placeholder="t('admin.developC.editor.requirementPlaceholder')"
          ...
          >
            {{ t('admin.developC.editor.removeRow') }}
          </button>
        </div>
      ...
      <p v-if="local.requirements.length === 0" class="text-sm text-gray-400">{{ t('admin.developC.editor.noRows') }}</p>
    </div>
    ...
    <div v-for="(area, ai) in local.areas" :key="`area-${area.area}-${ai}`" class="grid gap-3 rounded-lg border border-gray-200 p-3">
      ...
      <label class="grid gap-1 text-base">
        <span class="text-sm font-semibold text-gray-600">{{ t('admin.developC.editor.areaSummary') }}</span>
      ...
      <div class="grid gap-2">
        <div class="flex items-center gap-2">
          <span class="text-sm font-semibold text-gray-600">{{ t('admin.developC.editor.spec') }}</span>
        ...
        <div v-for="(row, si) in area.spec" :key="`spec-${si}`" class="grid gap-1 rounded-md border border-gray-100 p-2">
          <div class="flex items-start gap-2">
            <input
              ...
              :placeholder="t('admin.developC.editor.specItemPlaceholder')"
            ...
            <input
              ...
              type="text"
              :maxlength="L.factTextLen"
              :placeholder="t('admin.developC.editor.specTextPlaceholder')"
              class="min-w-0 flex-1 rounded-md border border-gray-300 px-2.5 py-1.5 text-sm"
            >
            <button
              type="button"
      ...
      <div class="grid gap-2">
        <div class="flex items-center gap-2">
          <span class="text-sm font-semibold text-gray-600">{{ t('admin.developC.editor.observations') }}</span>
    ...
    <div class="grid gap-2 rounded-lg border border-gray-200 p-3">
      <div class="flex flex-wrap items-center gap-2">
        <span class="text-base font-semibold text-gray-800">{{ t('admin.developC.review.schedule.title') }}</span>
        ...
        >
          {{ t('admin.developC.review.schedule.addPhase') }}
        </button>
        <button
        ...
        >
          {{ t('admin.developC.review.schedule.clear') }}
        </button>
      </div>

      <p class="text-xs text-gray-500">{{ t('admin.developC.review.schedule.hint') }}</p>
      ...
      <div v-if="confirmScheduleClear" class="flex flex-wrap items-center gap-2 rounded-md border border-red-200 bg-red-50 p-2">
        <span class="text-xs font-bold text-red-800">{{ t('admin.developC.review.schedule.clearConfirm') }}</span>
        <button type="button" class="rounded-md bg-red-600 px-2.5 py-1 text-xs font-bold text-white hover:bg-red-700" @click="clearSchedule">
          {{ t('admin.developC.review.schedule.clearYes') }}
        </button>
      ...
      <div v-for="(phase, pi) in schedulePhases" :key="`phase-${pi}`" class="grid gap-1.5 rounded-md border border-gray-100 p-2">
        <div class="flex flex-wrap items-center gap-1.5">
          ...
          <input
            ...
            :placeholder="t('admin.developC.review.schedule.namePlaceholder')"
          ...
          <label class="flex items-center gap-1 text-xs text-gray-600">
            ...
            >
            {{ t('admin.developC.review.schedule.weeks') }}
          </label>
          <button
            type="button"
          ...
          >
            {{ t('admin.developC.review.schedule.moveUp') }}
          </button>
          <button
            type="button"
            class="rounded-md border border-gray-300 px-2 py-1.5 text-xs text-gray-500 hover:bg-gray-50 disabled:opacity-30"
            ...
            @click="moveSchedulePhase(pi, 1)"
          >
            {{ t('admin.developC.review.schedule.moveDown') }}
          </button>
          <button
            type="button"
        ...
        <div class="grid gap-1.5 sm:grid-cols-3">
          <input
            ...
            :placeholder="t('admin.developC.review.schedule.outputPlaceholder')"
          ...
          <input
            ...
            type="text"
            :maxlength="L.schedulePhaseTextLen"
            :placeholder="t('admin.developC.review.schedule.prerequisitePlaceholder')"
            class="h-9 w-full min-w-0 rounded-md border border-gray-200 px-2.5 text-xs"
          >
          <input
            v-model="phase.note"
            type="text"
            :maxlength="L.schedulePhaseTextLen"
            :placeholder="t('admin.developC.review.schedule.notePlaceholder')"
            class="h-9 w-full min-w-0 rounded-md border border-gray-200 px-2.5 text-xs"
          >
        </div>
      </div>

      <p v-if="schedulePhases.length === 0" class="text-sm text-gray-400">{{ t('admin.developC.review.schedule.empty') }}</p>

      <template v-if="local.schedule !== null && local.schedule !== undefined && schedulePhases.length > 0">
        <label class="grid gap-1">
          <span class="text-sm font-semibold text-gray-600">{{ t('admin.developC.review.schedule.assumptions') }}</span>
          <input
            v-model="local.schedule.assumptions"
            type="text"
            :maxlength="L.scheduleAssumptionsLen"
            :placeholder="t('admin.developC.review.schedule.assumptionsPlaceholder')"
            class="w-full rounded-md border border-gray-300 px-2.5 py-1.5 text-xs"
          >
        </label>
        <p class="text-sm font-semibold text-gray-700">
          {{ t('admin.developC.review.schedule.total', { min: scheduleTotals.minWeeks, max: scheduleTotals.maxWeeks }) }}
        </p>
    ...
    <div class="grid gap-2">
      <div class="flex items-center gap-2">
        <span class="text-base font-semibold text-gray-800">{{ t('admin.developC.editor.openQuestions') }}</span>
      ...
      <div v-for="(row, qi) in local.openQuestions" :key="`q-${qi}`" class="grid gap-1.5 rounded-md border border-amber-100 bg-amber-50/30 p-2">
        <div class="flex items-start gap-2">
          ...
          <input
            ...
            :placeholder="t('admin.developC.editor.questionPlaceholder')"
            class="min-w-0 flex-1 rounded-md border border-gray-300 px-2.5 py-1.5 text-sm"
          >
          <button
            type="button"
        ...
        <input
          ...
          type="text"
          :maxlength="L.whyLen"
          :placeholder="t('admin.developC.editor.whyPlaceholder')"
        ...
        <textarea
          ...
          :placeholder="t('admin.developC.editor.resolutionPlaceholder')"
          class="w-full rounded-md border border-emerald-200 px-2.5 py-1.5 text-xs leading-relaxed"
        />
      </div>
      <p v-if="local.openQuestions.length === 0" class="text-sm text-gray-400">{{ t('admin.developC.editor.noRows') }}</p>
    </div>
    ...
    <!-- 담당자 의견 -->
    <label class="grid gap-1 text-base">
      <span class="font-semibold text-gray-800">{{ t('admin.developC.editor.adminComment') }}</span>
      <textarea
        v-model="local.adminComment"
        rows="4"
        :maxlength="L.adminCommentLen"
        :placeholder="t('admin.developC.editor.adminCommentPlaceholder')"
        class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm leading-relaxed"
      />
      <span class="text-xs text-gray-500">{{ t('admin.developC.editor.adminCommentHint') }}</span>
    </label>
  </fieldset>
</template>
