...
import type { DevelopQuoteViewType } from '@sp/api-contract/develop-c';
...
async function onWithdraw(): Promise<void> {
  ...
  try {
    ...
    setNotice(t('admin.developC.quote.withdrawn'), false);
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.quote.withdrawFail'), errorCodes.value), true);
  }
}
...
async function onMarkPaid(): Promise<void> {
  ...
  try {
    ...
    payingId.value = null;
    setNotice(t('admin.developC.quote.markPaidDone'), false);
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.quote.markPaidFail'), errorCodes.value), true);
  }
}
</script>
...
<template>
  <article class="rounded-lg border border-gray-200 bg-white p-3">
    ...
    <p class="mt-0.5 text-xs text-gray-500">
      {{ t('admin.developC.quote.validUntil') }} {{ quote.validUntil }}
      <template v-if="quote.durationDays !== null"> · {{ t('admin.developC.quote.durationDaysShort', { days: quote.durationDays }) }}</template>
      <template v-if="quote.sentAt !== null"> · {{ t('admin.developC.quote.sentAt') }} {{ formatDateTime(quote.sentAt) }}</template>
      <template v-if="quote.acceptedAt !== null"> · {{ t('admin.developC.quote.acceptedAt') }} {{ formatDateTime(quote.acceptedAt) }}<template v-if="quote.acceptedName !== null"> ({{ quote.acceptedName }})</template></template>
      <template v-if="quote.declinedAt !== null"> · {{ t('admin.developC.quote.declinedAt') }} {{ formatDateTime(quote.declinedAt) }}</template>
    </p>
    <p v-if="quote.declineReason !== null" class="mt-0.5 text-xs text-red-600">
      {{ t('admin.developC.quote.declineReason') }}: {{ quote.declineReason }}
    </p>
    <p v-if="quote.poFile !== null" class="mt-0.5 text-xs text-gray-500">
      {{ t('admin.developC.quote.poFile') }}: {{ quote.poFile.name }}
    </p>
    ...
    <ul class="mt-2 grid gap-0.5 border-t border-gray-100 pt-2 text-sm">
      <li v-for="it in quote.items" :key="it.itemId" class="flex flex-wrap items-baseline gap-2">
        ...
        <span v-if="it.durationDays !== null" class="text-xs text-gray-400">{{ t('admin.developC.quote.durationDaysShort', { days: it.durationDays }) }}</span>
    ...
    <div class="mt-2 border-t border-gray-100 pt-2">
      <h4 class="text-xs font-bold text-gray-600">{{ t('admin.developC.quote.milestones') }}</h4>
      <ul class="mt-1 grid gap-1">
        <li v-for="m in quote.milestones" :key="m.milestoneId" class="rounded-md bg-gray-50 px-2 py-1.5 text-xs">
          ...
          <p v-if="m.payment !== null" class="mt-0.5 text-gray-500">
            ...
            <template v-if="m.payment.misu > 0"> · {{ t('admin.developC.quote.misu', { amount: formatKrw(m.payment.misu) }) }}</template>
          </p>
          <p v-if="m.paidAt !== null" class="mt-0.5 text-emerald-700">
            {{ t('admin.developC.quote.paidAt') }} {{ formatDateTime(m.paidAt) }}
          </p>
          <div v-if="m.status === 'pending' && payingId !== m.milestoneId" class="mt-1">
            ...
            >
              {{ t('admin.developC.quote.markPaid') }}
            </button>
          </div>
          <div v-if="payingId === m.milestoneId" class="mt-1 grid gap-1 rounded-md border border-amber-300 bg-amber-50 p-2">
            <p class="font-bold text-amber-900">{{ t('admin.developC.quote.markPaidConfirm') }}</p>
            <input
              v-model="payNote"
              type="text"
              :maxlength="500"
              :placeholder="t('admin.developC.quote.markPaidNote')"
    ...
    <p v-if="quote.internalNote !== null" class="mt-2 rounded-md bg-gray-50 px-2 py-1 text-xs text-gray-600">
      {{ t('admin.developC.quote.internalNote') }}: {{ quote.internalNote }}
    </p>

    <!-- 철회 확인 -->
    <div v-if="confirmWithdraw" class="mt-2 rounded-md border border-red-300 bg-red-50 p-2">
      <p class="text-xs font-bold text-red-800">{{ t('admin.developC.quote.withdrawConfirm') }}</p>
    ...
    <div class="mt-2 flex flex-wrap items-center gap-2">
      ...
      >
        {{ t('admin.developC.quote.withdraw') }}
      </button>
