...
import { DEVELOP_TASK_STATUS_LABELS } from '@sp/api-contract/develop-c';
...
const nameOf = (name: string, index: number): string =>
  name.trim() === '' ? t('admin.developC.docs.task.unnamed', { seq: index + 1 }) : name;
</script>

<template>
  <div class="rounded-lg border border-gray-200 p-3">
    <div class="flex flex-wrap items-center gap-2">
      <h3 class="text-sm font-bold text-gray-800">{{ t('admin.developC.docs.gantt.title') }}</h3>
      <span v-if="model !== null" class="text-xs text-gray-500">
        {{ model.startOn }} ~ {{ model.endOn }} · {{ t('admin.developC.docs.gantt.days', { days: model.totalDays }) }}
      </span>
    </div>

    <p v-if="model === null" class="mt-2 text-xs text-gray-400">{{ t('admin.developC.docs.gantt.noRange') }}</p>
    ...
    <div v-else class="mt-2 overflow-x-auto">
      <div class="min-w-[640px]">
        ...
        <p v-if="model.undated.length > 0" class="mt-2 text-[11px] text-gray-400">
          {{ t('admin.developC.docs.gantt.undated') }}:
