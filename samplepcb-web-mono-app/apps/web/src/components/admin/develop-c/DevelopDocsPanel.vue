...
import type { AdminDevelopRequestDetailType, AdminDevelopDocumentViewType, DevelopDocTypeType } from '@sp/api-contract/develop-c';
import { apiErrorMessage } from '@sp/ui';
import type { PreviewTarget } from '@sp/ui';
import { useAdminDevelopDocumentCreate } from '../../../admin/useAdminDevelopC';
...
async function onCreate(): Promise<void> {
  ...
  try {
    ...
    notice.value = t('admin.developC.docs.created', { docNo: res.data.docNo });
    focusDoc(res.data.documentId);
  } catch (error) {
    noticeError.value = true;
    notice.value = apiErrorMessage(error, t('admin.developC.docs.createFail'), {
      INVALID_TRANSITION: t('admin.developC.docs.errNotStarted'),
      DOC_TYPE_NOT_ALLOWED: t('admin.developC.docs.errNotDelivered'),
  ...
  <div class="grid gap-4">
    ...
    <section class="rounded-xl border border-gray-200 bg-white p-4">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="ml-auto flex flex-wrap items-center gap-1.5">
          ...
          <button
            ...
            @click="onCreate"
          >
            {{ create.isPending.value ? t('admin.developC.saving') : t('admin.developC.docs.new') }}
          </button>
        </div>
      </div>
      <p v-if="!canCreate" class="mt-1 text-xs text-amber-700">{{ t('admin.developC.docs.errNotStarted') }}</p>
      ...
      <div class="mt-3 grid gap-3">
        ...
        />
        <p v-if="groups.length === 0" class="py-6 text-center text-sm text-gray-400">{{ t('admin.developC.docs.empty') }}</p>
      </div>
    </section>
  </div>
</template>
