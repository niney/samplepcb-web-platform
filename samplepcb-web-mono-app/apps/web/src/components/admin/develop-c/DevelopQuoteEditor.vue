...
const errorCodes = computed(() => ({
  KIND_MISMATCH: t('admin.developC.quote.errKindMismatch'),
  QUOTE_NOT_DRAFT: t('admin.developC.quote.errNotDraft'),
  QUOTE_NOT_OPEN: t('admin.developC.quote.errNotOpen'),
  QUOTE_EMPTY: t('admin.developC.quote.errEmpty'),
  INVALID_TRANSITION: t('admin.developC.quote.errClosed'),
}));
...
const applyPaste = (): void => {
  ...
  if (parsed.items.length === 0) {
    pasteNote.value = t('admin.developC.quote.pasteEmpty');
    return;
  }
  ...
  pasteNote.value =
    parsed.rejected.length === 0
      ? t('admin.developC.quote.pasteAdded', { count: parsed.items.length })
      : `${t('admin.developC.quote.pasteAdded', { count: parsed.items.length })} ${t('admin.developC.quote.pasteRejected', { lines: parsed.rejected.join(' / ') })}`;
};
...
const applyScheduleDuration = (): void => {
  ...
  scheduleApplied.value = t('admin.developC.quote.fromScheduleNote');
};
...
const applyScheduleMilestones = (): void => {
  ...
  const first = t('admin.developC.quote.fromScheduleFirstTitle');
  const last = t('admin.developC.quote.fromScheduleLastTitle');
...
async function saveDraft(): Promise<number | null> {
  ...
  issues.value = found;
  if (found.length > 0) {
    setNotice(t('admin.developC.quote.saveBlocked'), true);
    return null;
  }
...
async function onSave(): Promise<void> {
  try {
    ...
    if (id !== null) setNotice(t('admin.developC.quote.saved'), false);
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.quote.saveFail'), errorCodes.value), true);
  }
}
...
async function onSendConfirm(): Promise<void> {
  ...
  try {
    ...
    setNotice(t('admin.developC.quote.sent'), false);
    emit('close');
  } catch (error) {
    setNotice(apiErrorMessage(error, t('admin.developC.quote.sendFail'), errorCodes.value), true);
  }
}
...
async function onDeleteConfirm(): Promise<void> {
  ...
  try {
    ...
    setNotice(apiErrorMessage(error, t('admin.developC.quote.deleteFail'), errorCodes.value), true);
  }
}
</script>
  ...
  <section class="rounded-xl border border-blue-200 bg-blue-50/40 p-4">
    <div class="flex flex-wrap items-center gap-2">
      <h3 class="text-base font-bold text-gray-900">
        {{ version === null ? t('admin.developC.quote.newTitle') : t('admin.developC.quote.editTitle', { version: version }) }}
      </h3>
      <button
        type="button"
    ...
    <div class="mt-3 grid gap-3 rounded-lg border border-gray-200 bg-white p-3 sm:grid-cols-2 xl:grid-cols-3">
      <label class="block text-sm sm:col-span-2 xl:col-span-3">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.fieldTitle') }}</span>
        <input
          v-model="form.title"
          type="text"
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.kind') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.vatMode') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.validUntil') }}</span>
        <input v-model="form.validUntil" type="date" class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
      </label>
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.durationDays') }}</span>
        <div class="mt-1 flex items-center gap-1.5">
          ...
          <button
            ...
            :title="t('admin.developC.quote.fromScheduleTitle')"
            @click="onScheduleDurationClick"
          >
            {{ t('admin.developC.quote.fromSchedule') }}
          </button>
        </div>
        <span v-if="hasSchedule" class="mt-0.5 block text-xs text-gray-500">
          {{ t('admin.developC.quote.fromScheduleTitle') }} — {{ scheduleTotals.minWeeks }}~{{ scheduleTotals.maxWeeks }}주
        </span>
        <div v-if="scheduleConfirm === 'duration'" class="mt-1 flex flex-wrap items-center gap-1.5 rounded-md border border-amber-300 bg-amber-50 p-2">
          <span class="text-xs font-bold text-amber-900">{{ t('admin.developC.quote.fromScheduleConfirm') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.reviewDays') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.warrantyDays') }}</span>
    ...
    <div class="mt-3 rounded-lg border border-gray-200 bg-white p-3">
      <div class="flex flex-wrap items-center gap-2">
        <h4 class="text-sm font-bold text-gray-800">{{ t('admin.developC.quote.items') }}</h4>
        <button
          type="button"
          ...
          @click="addItem"
        >
          {{ t('admin.developC.quote.addItem') }}
        </button>
      </div>
      ...
      <div v-for="(it, i) in form.items" :key="`item-${i}`" class="mt-2 grid gap-1.5 rounded-md border border-gray-100 p-2">
        <div class="flex flex-wrap items-center gap-1.5">
          ...
          <input
            ...
            :placeholder="t('admin.developC.quote.itemTitle')"
          ...
          <input
            ...
            type="text"
            inputmode="numeric"
            :placeholder="t('admin.developC.quote.itemAmount')"
          ...
          <input
            ...
            :placeholder="t('admin.developC.quote.itemDays')"
          ...
          <button
            ...
            @click="moveItem(i, 1)"
          >
            {{ t('admin.developC.quote.moveDown') }}
          </button>
          <button
            type="button"
          ...
          >
            {{ t('admin.developC.quote.removeRow') }}
          </button>
        </div>
        <input
          ...
          :placeholder="t('admin.developC.quote.itemDescription')"
          class="h-9 w-full rounded-md border border-gray-200 px-3 text-xs"
        >
      </div>
      <p v-if="form.items.length === 0" class="mt-2 text-sm text-gray-400">{{ t('admin.developC.quote.noItems') }}</p>

      <!-- 붙여넣기로 채우기 -->
      <div class="mt-3 rounded-md border border-dashed border-gray-300 p-2">
        <p class="text-xs font-bold text-gray-600">{{ t('admin.developC.quote.paste') }}</p>
        <textarea
          v-model="pasteText"
          rows="3"
          :placeholder="t('admin.developC.quote.pastePlaceholder')"
          class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-xs leading-relaxed"
        />
        <div class="mt-1 flex flex-wrap items-center gap-2">
          <span class="text-xs text-gray-500">{{ t('admin.developC.quote.pasteHint') }}</span>
          <button
            type="button"
            ...
            @click="applyPaste"
          >
            {{ t('admin.developC.quote.pasteApply') }}
          </button>
        </div>
      ...
      <dl class="mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 border-t border-gray-100 pt-2 text-sm">
        <dt class="text-gray-500">{{ t('admin.developC.quote.supply') }}</dt>
        <dd class="text-right text-gray-800">{{ formatKrw(amounts.supplyAmount) }}</dd>
        <dt class="text-gray-500">{{ t('admin.developC.quote.vat') }}</dt>
        <dd class="text-right text-gray-800">{{ formatKrw(amounts.vatAmount) }}</dd>
        <dt class="font-bold text-gray-800">{{ t('admin.developC.quote.total') }}</dt>
    ...
    <div class="mt-3 rounded-lg border border-gray-200 bg-white p-3">
      <div class="flex flex-wrap items-center gap-2">
        <h4 class="text-sm font-bold text-gray-800">{{ t('admin.developC.quote.milestones') }}</h4>
        <span class="text-xs" :class="ratioSum === 10000 ? 'text-gray-500' : 'font-bold text-red-600'">
          {{ t('admin.developC.quote.ratioSum', { percent: (ratioSum / 100).toFixed(2) }) }}
        </span>
        <button
          v-if="hasSchedule"
          type="button"
        ...
        >
          {{ t('admin.developC.quote.fromScheduleMilestones') }}
        </button>
        <button
          type="button"
          ...
          @click="addMilestone"
        >
          {{ t('admin.developC.quote.addMilestone') }}
        </button>
      </div>
      <div v-if="scheduleConfirm === 'milestones'" class="mt-2 flex flex-wrap items-center gap-1.5 rounded-md border border-amber-300 bg-amber-50 p-2">
        <span class="text-xs font-bold text-amber-900">{{ t('admin.developC.quote.fromScheduleMilestonesConfirm') }}</span>
      ...
      <div v-for="(m, i) in form.milestones" :key="`ms-${i}`" class="mt-2 flex flex-wrap items-center gap-1.5">
        <input
          ...
          :placeholder="t('admin.developC.quote.milestoneTitle')"
        ...
        <label class="flex items-center gap-1 text-xs text-gray-600">
          ...
          {{ t('admin.developC.quote.unlocks') }}
        </label>
      ...
      <p class="mt-2 text-xs text-gray-500">{{ t('admin.developC.quote.milestonesHint') }}</p>
    ...
    <div class="mt-3 grid gap-3 rounded-lg border border-gray-200 bg-white p-3">
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.deliverables') }}</span>
        ...
        <span class="mt-0.5 block text-xs text-gray-500">{{ t('admin.developC.quote.deliverablesHint') }}</span>
      </label>
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.scheduleNote') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.exclusions') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.terms') }}</span>
        ...
        <span class="mt-0.5 block text-xs text-gray-500">{{ t('admin.developC.quote.termsHint') }}</span>
      </label>
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.note') }}</span>
      ...
      <label class="block text-sm">
        <span class="font-medium text-gray-800">{{ t('admin.developC.quote.internalNote') }}</span>
        ...
        <span class="mt-0.5 block text-xs text-gray-500">{{ t('admin.developC.quote.internalNoteHint') }}</span>
      </label>
    </div>
    ...
    <div v-if="confirmMode === 'send'" class="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3">
      <p class="text-sm font-bold text-amber-900">{{ t('admin.developC.quote.sendConfirmTitle') }}</p>
      <p class="mt-1 text-sm text-amber-900">
        {{ t('admin.developC.quote.total') }} <b>{{ formatKrw(amounts.totalAmount) }}</b>
        ({{ DEVELOP_VAT_MODE_LABELS[form.vatMode] }}) · {{ t('admin.developC.quote.itemsCount', { count: form.items.length }) }}
      </p>
      ...
      <div class="mt-2 flex items-center gap-2">
        <button
          ...
          @click="onSendConfirm"
        >
          {{ t('admin.developC.quote.sendConfirm') }}
        </button>
        <button
          type="button"
    ...
    <div v-if="confirmMode === 'delete'" class="mt-3 rounded-lg border border-red-300 bg-red-50 p-3">
      <p class="text-sm font-bold text-red-800">{{ t('admin.developC.quote.deleteConfirm') }}</p>
      <div class="mt-2 flex items-center gap-2">
        <button
          type="button"
    ...
    <div class="mt-3 flex flex-wrap items-center gap-2">
      <button
        ...
        @click="onSave"
      >
        {{ busy ? t('admin.developC.saving') : t('admin.developC.quote.save') }}
      </button>
      <button
        type="button"
