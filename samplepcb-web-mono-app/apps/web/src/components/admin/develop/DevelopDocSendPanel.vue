...
<template>
  <section class="rounded-lg border border-blue-200 bg-blue-50/40 p-3">
    <div class="flex flex-wrap items-center gap-2">
      <h3 class="text-sm font-bold text-gray-800">{{ t('admin.develop.docs.send.title', { docNo: doc.docNo }) }}</h3>
    ...
    <div class="mt-2 grid gap-3 lg:grid-cols-2">
      ...
      <div class="rounded border border-gray-200 bg-white p-2.5">
        <p class="text-[11px] font-bold text-gray-500">{{ t('admin.develop.docs.send.preview') }}</p>
        ...
        <p v-else class="mt-1.5 text-xs text-gray-400">{{ t('admin.develop.docs.send.previewEmpty') }}</p>
      </div>
      ...
      <div class="grid gap-1.5">
        ...
        <div v-if="aiDraft !== null" class="grid gap-1.5 rounded border border-indigo-200 bg-indigo-50 p-2 text-[11px] text-indigo-900">
          <span class="font-bold">{{ t('admin.develop.docs.send.aiConfirm') }}</span>
          ...
          <div class="flex gap-1.5">
            ...
            <button type="button" class="rounded bg-indigo-600 px-2 py-1 font-bold text-white" @click="applyAi">
              {{ t('admin.develop.docs.send.aiApply') }}
            </button>
          </div>
        </div>

        <label class="grid gap-0.5 text-[11px] font-semibold text-gray-600">
          {{ t('admin.develop.docs.send.subject') }}
        ...
        <label class="grid gap-0.5 text-[11px] font-semibold text-gray-600">
          {{ t('admin.develop.docs.send.body') }}
        ...
        <label class="inline-flex flex-wrap items-center gap-1.5 text-xs text-gray-700">
          ...
          {{ t('admin.develop.docs.send.sendMail') }}
          <span class="text-gray-400">{{ customerEmail ?? t('admin.develop.docs.send.noEmail') }}</span>
        </label>
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          >
            {{ t('admin.develop.docs.send.submit') }}
          </button>
        </div>

        <div v-if="confirming" class="grid gap-1.5 rounded border border-blue-300 bg-white p-2 text-[11px] text-gray-700">
          <span class="font-bold">{{ t('admin.develop.docs.send.confirmTitle') }}</span>
          <span>{{ doc.docNo }} · {{ sendMail ? (customerEmail ?? t('admin.develop.docs.send.noEmail')) : t('admin.develop.docs.send.noMail') }}
            <template v-if="replyDueOn !== null"> · {{ t('admin.develop.docs.status.replyDue', { date: replyDueOn }) }}</template>
          </span>
          <div class="flex gap-1.5">
            ...
            <button type="button" class="rounded bg-blue-600 px-2 py-1 font-bold text-white disabled:opacity-40" :disabled="send.isPending.value" @click="onSend">
              {{ send.isPending.value ? t('admin.develop.saving') : t('admin.develop.docs.send.confirmOk') }}
            </button>
          </div>
        </div>
