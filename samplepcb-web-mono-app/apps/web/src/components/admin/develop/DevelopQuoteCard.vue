...
<template>
  <article class="rounded-lg border border-gray-200 bg-white p-3">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      <b class="text-sm text-gray-900">v{{ quote.version }}</b>
      <span class="text-[11px] text-gray-500">{{ DEVELOP_QUOTE_KIND_LABELS[quote.kind] }}</span>
      <span class="rounded-full px-2 py-0.5 text-[10px] font-bold" :class="statusClass">
        {{ DEVELOP_QUOTE_STATUS_LABELS[quote.status] }}
      </span>
      <span class="ml-auto text-sm font-bold text-gray-900">{{ formatKrw(quote.totalAmount) }}</span>
      <span class="text-[11px] text-gray-500">{{ DEVELOP_VAT_MODE_LABELS[quote.vatMode] }}</span>
    </div>
    <p class="mt-0.5 text-xs text-gray-700">{{ quote.title }}</p>
    ...
    <p v-if="quote.declineReason !== null" class="mt-0.5 text-[11px] text-red-600">
      {{ t('admin.develop.quote.declineReason') }}: {{ quote.declineReason }}
    </p>
    <p v-if="quote.poFile !== null" class="mt-0.5 text-[11px] text-gray-500">
      {{ t('admin.develop.quote.poFile') }}: {{ quote.poFile.name }}
    </p>

    <!-- 항목 -->
    <ul class="mt-2 grid gap-0.5 border-t border-gray-100 pt-2 text-xs">
      <li v-for="it in quote.items" :key="it.itemId" class="flex flex-wrap items-baseline gap-2">
        ...
        <span v-if="it.durationDays !== null" class="text-[11px] text-gray-400">{{ t('admin.develop.quote.durationDaysShort', { days: it.durationDays }) }}</span>
        <span class="font-semibold text-gray-700">{{ formatKrw(it.amount) }}</span>
      </li>
    </ul>
    <dl class="mt-1.5 grid grid-cols-[auto_1fr] gap-x-3 border-t border-gray-100 pt-1.5 text-[11px]">
    ...
    <div class="mt-2 border-t border-gray-100 pt-2">
      <h4 class="text-[11px] font-bold text-gray-600">{{ t('admin.develop.quote.milestones') }}</h4>
      <ul class="mt-1 grid gap-1">
        <li v-for="m in quote.milestones" :key="m.milestoneId" class="rounded-md bg-gray-50 px-2 py-1.5 text-[11px]">
          <div class="flex flex-wrap items-center gap-1.5">
          ...
          <div v-if="m.status === 'pending' && payingId !== m.milestoneId" class="mt-1">
            <button
              type="button"
              class="rounded-md border border-gray-300 bg-white px-2 py-1 text-[11px] font-bold text-gray-700 hover:bg-gray-50"
          ...
          <div v-if="payingId === m.milestoneId" class="mt-1 grid gap-1 rounded-md border border-amber-300 bg-amber-50 p-2">
            ...
            <input
              ...
              class="h-8 w-full rounded-md border border-gray-300 px-2 text-[11px]"
            >
            <div class="flex items-center gap-1.5">
              <button
                type="button"
                class="rounded-md bg-blue-600 px-2.5 py-1 text-[11px] font-bold text-white hover:bg-blue-700 disabled:opacity-40"
              ...
              <button
                type="button"
                class="rounded-md border border-gray-300 bg-white px-2.5 py-1 text-[11px] font-bold text-gray-600 hover:bg-gray-50"
                @click="payingId = null"
              >
    ...
    <p v-if="quote.internalNote !== null" class="mt-2 rounded-md bg-gray-50 px-2 py-1 text-[11px] text-gray-600">
    ...
    <div v-if="confirmWithdraw" class="mt-2 rounded-md border border-red-300 bg-red-50 p-2">
      <p class="text-[11px] font-bold text-red-800">{{ t('admin.develop.quote.withdrawConfirm') }}</p>
      <div class="mt-1.5 flex items-center gap-1.5">
        <button
          type="button"
          class="rounded-md bg-red-600 px-2.5 py-1 text-[11px] font-bold text-white hover:bg-red-700 disabled:opacity-40"
    ...
    <div class="mt-2 flex flex-wrap items-center gap-2">
      <button
        ...
        type="button"
        class="rounded-md border border-blue-300 px-2.5 py-1 text-[11px] font-bold text-blue-700 hover:bg-blue-50"
        @click="emit('edit')"
      >
      ...
      <button
        ...
        type="button"
        class="rounded-md border border-red-200 px-2.5 py-1 text-[11px] font-bold text-red-600 hover:bg-red-50"
        @click="confirmWithdraw = true"
      >
        {{ t('admin.develop.quote.withdraw') }}
      </button>
      <span v-if="notice !== ''" class="text-[11px] font-semibold" :class="noticeError ? 'text-red-600' : 'text-emerald-700'">{{ notice }}</span>
    </div>
  </article>
</template>
