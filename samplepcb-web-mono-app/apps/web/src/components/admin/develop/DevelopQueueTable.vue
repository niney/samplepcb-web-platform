...
import { developDetailTo, developQueueQuery, developQueueState } from '../../../admin/develop-navigation';
...
const syncQuery = (): void => {
  const next = developQueueQuery(filters.value, { tab: props.defaultTab, signal: props.signal ?? null });
  const current = developQueueQuery({ ...fromQuery() }, { tab: props.defaultTab, signal: props.signal ?? null });
    ...
    <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white">
      <table class="w-full text-left text-base">
        <thead class="border-b border-gray-200 text-sm text-gray-500">
          <tr>
            <th v-for="column in columns" :key="column" class="px-4 py-3">{{ t(`admin.develop.col.${column}`) }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            ...
            <td v-if="has('progress')" class="px-4 py-3 text-sm text-gray-600">
              ...
              <p class="mt-0.5 text-xs text-gray-500">
                ...
                <span v-if="r.ops.taskCount > 0" class="text-gray-400"> · {{ t('admin.develop.queue.taskCount', { n: r.ops.taskCount }) }}</span>
            ...
            <td v-if="has('docs')" class="px-4 py-3 text-sm">
              <template v-if="r.ops.pendingApprovals > 0">
                ...
                <p v-if="r.ops.nextReplyDueOn !== null" class="mt-0.5 text-xs" :class="r.ops.replyOverdue ? 'font-bold text-red-600' : 'text-gray-500'">
                  ...
                  <span v-if="r.ops.replyOverdue"> · {{ t('admin.develop.queue.overdue') }}</span>
                </p>
              </template>
              <span v-else class="text-gray-300">—</span>
            </td>
            ...
            <td v-if="has('money')" class="px-4 py-3 text-sm">
              <template v-if="r.ops.paidAmount > 0 || r.ops.pendingAmount > 0 || r.ops.openableMilestones > 0">
                <p class="text-xs text-gray-700">{{ t('admin.develop.queue.paid', { amount: formatKrw(r.ops.paidAmount) }) }}</p>
            ...
            <td v-if="has('inquiry')" class="max-w-72 px-4 py-3 text-sm">
              <template v-if="r.ops.openInquiries > 0">
                <span class="whitespace-nowrap rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">
                  {{ t('admin.develop.queue.inquiryOpen', { n: r.ops.openInquiries }) }}
                </span>
          ...
          <tr v-if="(data?.data.items ?? []).length === 0">
            <td :colspan="colCount" class="px-4 py-10 text-center text-sm text-gray-400">
              {{ isFetching ? t('admin.develop.loading') : (emptyText ?? t('admin.develop.empty')) }}
            </td>
          </tr>
        </tbody>
      </table>
