...
const earlyReceive = computed(
  () => !(arrivedOrLater.value || existing.value?.receivedAt != null),
);
...
const fileLabel = (kind: BomShipmentFileTypeType): string =>
  mode.value === 'domestic' ? BOM_SHIPMENT_FILE_DOMESTIC_LABELS[kind] : BOM_SHIPMENT_FILE_LABELS[kind];
...
<template>
  <div v-if="open && po !== null" class="fixed inset-0 z-40 grid place-items-center bg-black/30 p-4" @click.self="emit('close')">
    <div class="w-full max-w-md rounded-2xl bg-surface p-6 shadow-2xl">
      ...
      >
        ...
        <p v-else class="text-gray-500">
          ⏳ 협력사의 '{{ savedLabel(savedNext) }}' 처리를 기다리는 중입니다 — 필요하면 아래에서 관리자가 대신 진행할 수 있습니다.
        </p>
      </div>

      <div class="mt-4 grid grid-cols-2 gap-2 text-xs">
        <label class="text-gray-500">모드{{ modeLocked ? ' (박제됨)' : '' }}
          <select v-model="mode" :disabled="modeLocked" class="mt-1 h-8 w-full rounded-md border border-gray-300 px-2">
            <option v-for="m in BOM_SHIPMENT_MODES" :key="m" :value="m">{{ BOM_SHIPMENT_MODE_LABELS[m] }}</option>
          </select>
        </label>
        ...
        <label class="text-gray-500">운송사
          <input v-model="carrier" type="text" maxlength="50" class="mt-1 h-8 w-full rounded-md border border-gray-300 px-2">
        </label>
        <label class="text-gray-500">송장번호
          <input v-model="trackingNumber" type="text" maxlength="100" class="mt-1 h-8 w-full rounded-md border border-gray-300 px-2 font-mono">
        </label>
        <label class="text-gray-500">출고예정일
          <input v-model="shipDate" type="date" class="mt-1 h-8 w-full rounded-md border border-gray-300 px-2">
        </label>
        <label class="text-gray-500">추적 URL
          <input v-model="trackingUrl" type="url" maxlength="500" class="mt-1 h-8 w-full rounded-md border border-gray-300 px-2">
        </label>
      </div>
      <p v-if="awbWarning" class="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-700">
      ...
      <div class="mt-3 space-y-1.5 rounded-xl border border-gray-200 p-3">
        ...
        <div v-for="kind in BOM_SHIPMENT_FILE_TYPES" :key="kind" class="flex flex-wrap items-center gap-2 text-xs">
          ...
          <template v-if="fileOf(kind) !== null">
            ...
            <span class="text-[10px] text-gray-400">{{ fileOf(kind)?.uploadedBy === 'PARTNER' ? '협력사 첨부' : '관리자 첨부' }}</span>
      ...
      <div class="mt-4 rounded-xl border p-3" :class="earlyReceive ? 'border-gray-100 opacity-80' : 'border-gray-200'">
        ...
        <p v-if="earlyReceive" class="mt-1.5 rounded bg-amber-50 px-2 py-1.5 text-[11px] text-amber-700">
          아직 도착 전 단계입니다 — 선적 추적 없이 물품을 이미 수령한 경우에만 사용하세요.
        </p>
        <label class="mt-2 block text-xs text-gray-500">편차 메모(수량 부족·불량 등 — 선택)
          <textarea v-model="receiveNote" rows="2" maxlength="2000" class="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs" />
        </label>
        <button
          type="button"
          class="mt-2 rounded-lg px-4 py-2 text-xs font-bold text-white disabled:opacity-40"
          :class="earlyReceive ? 'bg-gray-400 hover:bg-gray-500' : 'bg-emerald-600 hover:bg-emerald-700'"
