...
<template>
  <div ref="root" class="relative shrink-0">
    ...
    >
      ...
      <span class="hidden max-w-[120px] truncate sm:inline">{{ auth.isLoggedIn ? displayNick : pt($t('auth.login')) }}</span>
    ...
    <div
      ...
      role="menu"
      :aria-label="pt('프로필 메뉴')"
    >
      <div class="border-b border-line-soft bg-surface-sunken px-4 py-3">
        <p class="truncate text-sm font-bold text-ink-strong">{{ pt('{name}님', { name: displayNick }) }}</p>
        <p class="mt-0.5 truncate text-[11px] text-ink-muted">{{ auth.me?.mbId }}</p>
        <span v-if="auth.me?.isAdmin" class="mt-2 inline-flex rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700">{{ pt($t('auth.admin')) }}</span>
      </div>
