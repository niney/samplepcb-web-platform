...
import { DEVELOP_REQUEST_STATUS_LABELS, developAreaBadge } from '@sp/api-contract/develop-c';
import type { DevelopRequestStatusType } from '@sp/api-contract/develop-c';
...
const customerPath = (requestId: number): string => `/develop/c/requests/${String(requestId)}`;
const adminPath = (requestId: number): string => `/app/admin/develop-c/requests/${String(requestId)}`;
