...
import { buildDevelopRequestDetail, customerEmailOf, toQuoteView } from './develop-c-requests';
...
export const adminDevelopRequestRoutes: FastifyPluginCallbackZod = (fastify, _opts, done) => {
  ...
  fastify.get('/develop-c/requests', { schema: { querystring: AdminDevelopRequestListQuery } }, async (request) => {
    ...
    const search: Prisma.SpDevelopRequestWhereInput = { AND: [searchBase, developPrototypeWhere('c')] };
    ...
    const [rowsAll, totalAll, grouped, activeRows] = await Promise.all([
      ...
      prisma.spDevelopRequest.findMany({ where: { AND: [developPrototypeWhere('c'), { status: { in: [...ACTIVE_STATUSES] } }] }, select: { id: true, status: true } }),
    ]);
  ...
  fastify.get('/develop-c/requests/:id', { schema: { params: RequestIdParams, response: { 200: AdminDevelopRequestDetailResponse, 404: ApiError } } }, async (request, reply) => {
  ...
  fastify.get(
    '/develop-c/requests/:id/review/versions',
  ...
  fastify.get(
    '/develop-c/requests/:id/review/versions/:seq',
  ...
  fastify.post(
    '/develop-c/requests/:id/review/versions/:seq/restore',
  ...
  fastify.post(
    '/develop-c/requests/:id/review/:action',
  ...
  fastify.post(
    '/develop-c/requests/:id/diagram/:action',
  ...
  fastify.post('/develop-c/requests/:id/diagram/upload', async (request, reply) => {
  ...
  fastify.post('/develop-c/requests/:id/events', async (request, reply) => {
    if (!request.isMultipart()) return reply.status(400).send({ error: 'MULTIPART_REQUIRED', message: 'multipart 요청이어야 합니다' });
  ...
  fastify.get('/develop-c/files/:fileId', { schema: { params: FileIdParams } }, async (request, reply) => {
  ...
  fastify.get('/develop-c/files/:fileId/preview', { schema: { params: FileIdParams } }, async (request, reply) => {
    const file = await findDevelopFile(request.params.fileId);
