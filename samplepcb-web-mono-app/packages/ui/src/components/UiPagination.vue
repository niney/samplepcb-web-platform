...
// 현재 페이지 주변 최대 5개 번호(sp-vue UiPagination 미러, 톤만 마켓 팔레트).
...
<template>
  <nav class="flex items-center justify-center gap-1 text-sm" aria-label="pagination">
    <button
      type="button"
      class="rounded-md px-2 py-1 text-tx-3 hover:bg-line disabled:opacity-40"
    ...
    <button
      ...
      :class="p === props.page ? 'bg-ink-900 font-semibold text-white' : 'text-tx-2 hover:bg-line'"
      @click="go(p)"
    >
      {{ p }}
    </button>
