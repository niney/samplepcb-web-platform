...
import { MARKET_UNKNOWN_CHOICE, isTextQuestion } from '@sp/api-contract';
...
// kind='text' 문항(개발의뢰 시스템개발·서술형)은 칩 없이 여러 줄 서술만 받는다 — 답은 note 에 담긴다.
...
<template>
  <div class="grid gap-2">
    <p class="text-label font-semibold text-tx-2">
      ...
      <span v-if="question.multi && !textKind" class="font-normal text-tx-3">(여러 개 가능)</span>
    </p>
    <textarea
      v-if="textKind"
      :value="state.note"
      ...
      maxlength="2000"
      class="rounded-lg border bg-white p-3 text-body font-normal leading-relaxed text-tx-1 outline-none focus:border-brand-500"
