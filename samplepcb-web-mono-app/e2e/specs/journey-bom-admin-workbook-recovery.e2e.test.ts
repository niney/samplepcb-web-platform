...
const OWNER_ID = `e2e-bom-admin-workbook-${RUN_KEY}`;
...
function quoteIdFromPage(page: Page): string {
  const match = /^\/app\/admin\/bom\/(\d+)$/.exec(new URL(page.url()).pathname);
...
async function cardFitsViewport(page: Page): Promise<boolean> {
  ...
  const card = fileInput.locator('xpath=preceding-sibling::div[1]');
  const box = await card.boundingBox();
  const viewport = page.viewportSize();
  return box !== null && viewport !== null && box.x >= 0 && box.x + box.width <= viewport.width;
}

describe.skipIf(!RUN || !JOURNEY)('BOM 여정 21호 — 관리자 다중 시트 분석 영속·복구', () => {
  const rp = createJourneyReport(
    'findings-bom-admin-workbook-recovery',
    'BOM 여정 21호 관리자 다중 시트 분석 영속·복구 탐색 주행 리포트',
  );
  const { F, ledger } = rp;
  ...
  afterAll(async () => {
    ...
    for (const row of rows) {
      ...
      const removed = await api(adminToken, 'DELETE', `/api/bom/quotes/${String(row.id)}`);
    ...
    expect(await prisma.spBomQuote.count({ where: { mbId: OWNER_ID } }), '21호 견적 잔재').toBe(0);
    await closeBrowser();
    await disconnectPrisma();
  }, 180_000);

  test('R01. 390px 업로드 카드 → 단일 버튼·업로드 503 인지·재시도 가능', async () => {
    const page = admin.page;
    await page.setViewportSize({ width: 390, height: 844 });
    await page.goto(`${BASE_URL}/app/admin/bom`, { waitUntil: 'domcontentloaded' });
    const fileInput = page.locator('input[type="file"]').first();
    const card = fileInput.locator('xpath=preceding-sibling::div[1]');
    const fits = await cardFitsViewport(page);
    expect.soft(fits, '390px에서 업로드 카드 전체가 보여야 합니다').toBe(true);
    if (!fits) F('R01', 'bug', '640px 고정 업로드 카드가 390px 화면의 좌우를 벗어남');
    expect
      .soft(
      ...
      .toBe(true);
    const uploadButtons = card.getByRole('button');
    const uploadButtonCount = await uploadButtons.count();
    const namedUploadButton = card.getByRole('button', {
      name: 'BOM 파일 업로드',
      exact: true,
    });
    expect
      .soft(uploadButtonCount, '업로드 카드에는 중첩되지 않은 버튼 하나만 있어야 합니다')
      .toBe(1);
    expect
      .soft(await namedUploadButton.count(), '카드 전체 업로드 버튼의 이름이 명확해야 합니다')
      .toBe(1);
    if (uploadButtonCount !== 1 || (await namedUploadButton.count()) !== 1) {
      F(
        'R01',
        'ux',
        '업로드 카드가 role=button 안에 button을 중첩하고 업로드 목적을 한국어로 알리지 않음',
      );
    }

    let uploadFailed = false;
    const uploadPattern = '**/api/bom/quotes';
    const uploadHandler = async (route: Route): Promise<void> => {
      if (!uploadFailed && route.request().method() === 'POST') {
        uploadFailed = true;
        await route.fulfill({
          status: 503,
    ...
    await fileInput.setInputFiles(MULTI_SHEET_XLSX);
    await waitForBodyText(page, '분석 엔진에 연결할 수 없습니다. 잠시 후 다시 시도해 주세요.');
    const uploadAlert = page.getByRole('alert');
    const hasUploadAlert = (await uploadAlert.count()) === 1;
    expect.soft(hasUploadAlert, '업로드 실패를 alert로 알려야 합니다').toBe(true);
    if (hasUploadAlert) {
      expect
        .soft(await uploadAlert.evaluate((element) => element === document.activeElement))
        .toBe(true);
    } else {
      F('R01', 'ux', '업로드 503 문구가 alert·초기 포커스 없이 표시됨');
    }
    await rp.shot(admin, 'R01-mobile-bom-upload');
  ...
  test('R02. prepare 502 → 같은 엔진 잡 재시도 → BOM 2개·비BOM 1개 선택', async () => {
    const page = admin.page;
    ...
    await page.waitForURL((url) => /^\/app\/admin\/bom\/\d+$/.test(url.pathname), {
      timeout: 60_000,
    });
    ...
    await page.waitForFunction(
      () =>
        document.body.innerText.includes('분석 실패') ||
        document.body.innerText.includes('BOM 분석 결과를 불러오지 못했습니다'),
      undefined,
      { timeout: 240_000 },
    );
    ...
    await rp.shot(admin, 'R02-prepare-retry');
    ...
    if (!hasRetry)
      F('R02', 'bug', 'prepare 502가 재업로드만 안내해 완료된 엔진 잡을 재사용할 수 없음');
    if (!hasNamedAlert)
      F('R02', 'ux', '분석 실패 영역에 alert 이름·초기 포커스가 없어 오류 인지가 늦음');
    ...
    if (hasRetry) {
      ...
      const recovered = await api(adminToken, 'POST', `/api/bom/quotes/${multiQuoteId}/prepare`);
    ...
    await rp.shot(admin, 'R02-sheet-selection');
    ...
    await rp.shot(admin, 'R02-ready-from-persisted-analysis');
    F('R02', 'obs', '엔진 잡 삭제·화면 재로드 뒤에도 DB 분석 스냅샷만으로 5개 라인을 계산함');
  }, 720_000);

  test('R03. 시트 관리 키보드 경계·PUT 503 → 선택 유지 재시도 → 제외', async (ctx) => {
    ...
    await rp.shot(admin, 'R03-sheet-manager-keyboard');
    ...
    if (!focusInside || !scrollLocked)
      F('R03', 'ux', '시트 관리 모달이 초기 포커스·배경 스크롤을 제어하지 않음');
    ...
    if (!tabStayedInside || !escaped)
      F('R03', 'ux', '시트 관리 모달에 Tab 순환 또는 Escape 닫기가 없음');
    ...
    await dialog
      ...
      .waitFor();
    await rp.shot(admin, 'R03-sheet-manager-503');
    ...
    if (hasInlineAlert) {
      ...
      F('R03', 'ux', '시트 변경 503 문구가 alert·포커스 없이 작게 표시됨');
    }
    ...
    F('R03', 'obs', 'PUT 503 뒤 선택을 유지해 재시도했고 Option BOM만 합계에서 제외함');
  }, 180_000);

  test('R04. 제외 시트 복원 → 원래 라인 ID·분석 연결 그대로 재사용', async (ctx) => {
    if (multiQuoteId === null) return ctx.skip();
    const quoteId = multiQuoteId;
    const page = admin.page;
    // 앞 단계(R03)의 시트 제외가 재계산·재검색을 유발한다 — 그게 끝나기 전에 다시
    ...
    F('R04', 'obs', '복원은 삭제·재생성 없이 동일 line ID와 analysisComponent 연결을 재사용함');
  }, 120_000);

  test('R05. 유효 BOM 없는 XLSX → 시트별 이유·영속 실패 화면·모바일 재업로드', async () => {
    const page = admin.page;
    await page.goto(`${BASE_URL}/app/admin/bom`, { waitUntil: 'domcontentloaded' });
    ...
    await rp.shot(admin, 'R05-no-valid-bom');
    ...
    if (hasAlert) {
      ...
      F('R05', 'ux', '유효 BOM 없음 결과가 alert·초기 포커스 없이 일반 카드로 표시됨');
    }
    ...
    await page.setViewportSize({ width: 390, height: 844 });
    let sheetNamesFit = true;
    for (const sheetName of ['Read Me', 'Stock Ledger']) {
      const fits = await page
        .getByText(sheetName, { exact: true })
        .evaluate((element) => element.scrollWidth <= element.clientWidth);
      sheetNamesFit &&= fits;
      expect.soft(fits, `${sheetName} 시트명이 모바일에서 생략되지 않아야 합니다`).toBe(true);
    }
    if (!sheetNamesFit)
      F('R05', 'bug', '390px 실패 화면에서 시트명이 말줄임되어 원본 구분이 어려움');
    expect
      .soft(
        await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth),
      ...
      .toBe(true);
    await rp.shot(admin, 'R05-no-valid-bom-mobile');
    await page.getByRole('button', { name: '새 BOM 업로드', exact: true }).click();
    await page.waitForURL('**/app/admin/bom');
    expect
      .soft(await cardFitsViewport(page), '재업로드 카드도 390px 안에 보여야 합니다')
      .toBe(true);
    await page.setViewportSize({ width: 1440, height: 900 });
    F('R05', 'obs', '엔진 잡 삭제·재로드 뒤에도 비BOM 2개 사유가 DB 스냅샷에서 유지됨');
  }, 480_000);
});
