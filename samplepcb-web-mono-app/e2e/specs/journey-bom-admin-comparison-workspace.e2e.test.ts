// Smart BOM 완주 여정 20호 — 관리자 전체 BOM 비교·필터·오류 복구·모바일 열 탐색.
//
...
const OWNER_ID = `e2e-bom-admin-comparison-${RUN_KEY}`;
const OWNER = { mbId: OWNER_ID, mbNick: 'BOM 전체 비교 E2E 관리자', isAdmin: true };
const QUANTITY = 10;
...
const LINES: readonly LinePlan[] = [
  ...
  {
    key: 'attention-beta',
    mpn: `E2E20-ATTENTION-BETA-${RUN_KEY}`,
  ...
  {
    key: 'missing-gamma',
    mpn: `E2E20-MISSING-GAMMA-${RUN_KEY}`,
  ...
  },
  {
    ...
    mpn: `E2E20-ATTENTION-EPSILON-${RUN_KEY}`,
  ...
  {
    key: 'missing-zeta',
    mpn: `E2E20-MISSING-ZETA-${RUN_KEY}`,
  ...
  {
    key: 'manual-theta',
    mpn: `E2E20-MANUAL-THETA-${RUN_KEY}`,
...
function candidatePayload(line: LinePlan, rowIdx: number): Record<string, unknown> {
  ...
  const candidateKey = `ik1:e2e20-${line.key}-${RUN_KEY}`;
  const offerKey = `ok2:e2e20-${line.key}-${RUN_KEY}`;
  ...
  return {
    ...
    offers: [
      {
        ...
        supplierSku: `E2E20-${line.key.toUpperCase()}`,
        packaging: 'Cut Tape',
        stock: 5_000,
        moq: 1,
        orderMultiple: 1,
    ...
    procurementDecision: {
      ...
      required_quantity: QUANTITY,
      target_currency: 'KRW',
      currency_rate_snapshot_id: `e2e-journey-20-${RUN_KEY}`,
...
function selectedOffer(line: LinePlan, rowIdx: number): Record<string, unknown> | null {
  ...
  const unitPrice = 100 + rowIdx * 10;
  return {
    offerKey: `ok2:e2e20-${line.key}-${RUN_KEY}`,
    supplier: line.supplier,
    supplierSku: `E2E20-${line.key.toUpperCase()}`,
    packaging: 'Cut Tape',
    breakQty: 1,
...
async function seedQuote(): Promise<SeededQuote> {
  const title = `[BOM 여정 20호] 전체 비교 ${RUN_KEY}`;
  const fileName = `bom-journey-20-${RUN_KEY}.xlsx`;
  return getPrisma().$transaction(async (tx: any) => {
    const quote = await tx.spBomQuote.create({
      data: {
        ...
        uncostedCount: 2,
        adminMemo: '20호 전체 BOM 비교 E2E fixture',
        sheets: {
          create: [
            {
              sheetIndex: 0,
      ...
      if (line.sheetIndex !== null && line.sheetName !== null) {
        ...
        const component = await tx.spBomAnalysisComponent.create({
          data: {
            analysisRunId: analysisRun.id,
            analysisSheetId,
            engineComponentId: `e2e20-${String(rowIdx)}`,
            sourceRows: [rowIdx + 2],
            ...
            payload: {
              component_id: `e2e20-${String(rowIdx)}`,
...
async function createAdmin(label: string): Promise<E2eSession> {
...
async function openQuote(page: Page): Promise<void> {
  await page.goto(`${BASE_URL}/app/admin/bom/${seeded.id}`, { waitUntil: 'domcontentloaded' });
  await page
    .getByRole('heading', { name: seeded.fileName, exact: true })
    .waitFor({ state: 'visible' });
  await page.getByRole('button', { name: /BOM 비교/ }).waitFor({ state: 'visible' });
}

async function clickComparisonTrigger(trigger: Locator): Promise<void> {
  const hitTarget = await trigger.evaluate((element) => {
    const rect = element.getBoundingClientRect();
    const hit = document.elementFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2);
    return hit === element || (hit !== null && element.contains(hit));
  });
  expect.soft(hitTarget, 'BOM 비교 버튼 중앙이 다른 패널에 가려지지 않음').toBe(true);
  if (hitTarget) await trigger.click();
  else await trigger.evaluate((element) => (element as HTMLButtonElement).click());
}
...
async function openComparison(page: Page): Promise<{ trigger: Locator; dialog: Locator }> {
  const trigger = page.getByRole('button', { name: /BOM 비교/ });
  const responseWait = page.waitForResponse(isComparisonResponse);
  await clickComparisonTrigger(trigger);
...
const rp = createJourneyReport(
  'findings-bom-admin-comparison-workspace',
  'BOM 여정 20호 관리자 전체 BOM 비교·필터·복구 탐색 주행 리포트',
);
const { F, ledger } = rp;
const sessions: E2eSession[] = [];
let seeded: SeededQuote;

describe.skipIf(!RUN || !JOURNEY)('BOM 여정 20호 — 관리자 전체 BOM 비교·필터·오류 복구', () => {
  beforeAll(async () => {
  ...
  afterAll(async () => {
    rp.write(
      Object.fromEntries(sessions.map((session, index) => [`관리자${String(index + 1)}`, session])),
    );
  ...
  test('Q01. 전체화면 dialog: 초기 초점·배경 잠금·Tab 순환·Esc·호출 버튼 복귀', async () => {
    const admin = await createAdmin('비교 레이어 키보드 관리자');
    const page = admin.page;
    await openQuote(page);
    await rp.shot(admin, 'Q01-admin-comparison-trigger-hit-area');
    const previousOverflow = await page.evaluate(() => document.body.style.overflow);
    const { trigger, dialog } = await openComparison(page);
    expect
      .soft(
        await trigger.locator('span').first().getAttribute('aria-hidden'),
        '비교 아이콘은 버튼 이름에서 제외',
      )
      .toBe('true');
    ...
    F('Q01', 'obs', '비교 전체화면의 초기 초점·스크롤·Tab·Esc·호출 버튼 복귀를 확인');
    expect(admin.pageErrors, 'Q01 pageerror').toEqual([]);
  });

  test('Q02. 영속 비교 원장: 엔진 잡 없이 요약·페이지·직접 추가·선택 제외 시트 범위 유지', async () => {
    const admin = await createAdmin('비교 영속 원장 관리자');
    const page = admin.page;
    await openQuote(page);
    ...
    const visibleHeadings = await dialog.locator('.comparison-item .item-title').allTextContents();
    expect
    ...
    await rp.shot(admin, 'Q02-bom-comparison-persistent-page');
    F('Q02', 'obs', '박제 원문·후보만으로 4/2/2 요약과 2페이지·직접 추가·제외 시트 범위를 복원');
    expect(admin.pageErrors, 'Q02 pageerror').toEqual([]);
  });

  test('Q03. 검색·판정·시트 필터: 서버 결과와 제어값이 일치하고 다른 시트 선택지를 보존', async () => {
    const admin = await createAdmin('비교 필터 관리자');
    const page = admin.page;
    await openQuote(page);
    const { dialog } = await openComparison(page);
    const search = dialog.locator('input[type="search"]');
    const status = dialog.locator('.toolbar select').nth(0);
    const sheet = dialog.locator('.toolbar select').nth(1);
    const supplier = dialog.locator('.toolbar select').nth(2);
    expect
      .soft(await search.getAttribute('aria-label'), '검색 입력 접근 가능한 이름')
      .toBe('BOM 비교 검색');
    expect
      .soft(await status.getAttribute('aria-label'), '판정 필터 접근 가능한 이름')
      .toBe('판정 필터');
    expect
      .soft(await sheet.getAttribute('aria-label'), '시트 필터 접근 가능한 이름')
      .toBe('시트 필터');
    expect
      .soft(await supplier.getAttribute('aria-label'), '공급사 열 필터 접근 가능한 이름')
      .toBe('공급사 열 필터');
    ...
    await rp.shot(admin, 'Q03-bom-comparison-sheet-filter');
    F('Q03', 'obs', '설명 검색·판정·시트 필터 결과와 활성 시트 선택지의 정합을 확인');
    expect(admin.pageErrors, 'Q03 pageerror').toEqual([]);
  });

  test('Q04. 비교 GET 503: 열린 화면·오류 초점 유지 후 같은 자리에서 재시도', async () => {
    const admin = await createAdmin('비교 실패 복구 관리자');
    const page = admin.page;
    await openQuote(page);
    let failed = false;
    ...
    expect(
      await dialog.locator('.summary-grid').count(),
      '실패 중 실제 통계처럼 보이는 숫자 없음',
    ).toBe(0);
    expect
      .soft(
      ...
      .toBe(true);
    await rp.shot(admin, 'Q04-bom-comparison-load-error');
    ...
    F('Q04', 'obs', '비교 503을 열린 화면에서 안내하고 같은 자리 재시도로 영속 원장을 복원');
    expect(admin.pageErrors, 'Q04 pageerror').toEqual([]);
  });

  test('Q05. 닫기·재열기: 화면 제어값과 서버 쿼리를 모두 전체 1페이지로 초기화', async () => {
    const admin = await createAdmin('비교 재진입 관리자');
    const page = admin.page;
    await openQuote(page);
    let opened = await openComparison(page);
    const firstSheet = opened.dialog.locator('.toolbar select').nth(1);
    ...
    await clickComparisonTrigger(page.getByRole('button', { name: /BOM 비교/ }));
    opened = {
      trigger: page.getByRole('button', { name: /BOM 비교/ }),
    ...
    expect(
      await opened.dialog.locator('.toolbar select').nth(1).inputValue(),
      '재열기 제어값 전체 시트',
    ).toBe('all');
    expect
    ...
    expect
      ...
      .toBe(true);
    F('Q05', 'obs', '필터를 사용한 뒤 닫고 다시 열어도 제어값과 서버 결과가 같은 전체 범위로 시작');
    expect(admin.pageErrors, 'Q05 pageerror').toEqual([]);
  });

  test('Q06. 390px 비교표: 원본 맥락을 남기면서 공급사 후보 열을 실제로 노출', async () => {
    const admin = await createAdmin('모바일 비교 열 관리자');
    const page = admin.page;
    ...
    const exposure = await comparisonScroll.evaluate((element) => {
      ...
      const candidateLabel = [...element.querySelectorAll<HTMLElement>('.column-head strong')].find(
        (cell) => cell.textContent?.trim() === 'UniKeyIC',
      );
      const candidate = candidateLabel?.closest<HTMLElement>('.column-head');
      const candidateRect = candidate?.getBoundingClientRect() ?? null;
      const candidateTextRange = document.createRange();
      if (candidateLabel !== undefined) candidateTextRange.selectNodeContents(candidateLabel);
      const candidateTextRect =
        candidateLabel === undefined ? null : candidateTextRange.getBoundingClientRect();
    ...
    expect
      ...
      .toBe(true);
    await rp.shot(admin, 'Q06-mobile-bom-comparison-columns');
    F('Q06', 'obs', '390px에서 원본 맥락과 공급사 후보 열을 가로 탐색으로 모두 확인');
    expect(admin.pageErrors, 'Q06 pageerror').toEqual([]);
  });
});
