...
첫 주행이 **박제한 결함 하나와 마찰 하나**(둘 다 고치지 않았다):

- **[bug] 회차 발주의 직송지가 원주문의 종결 동선을 뒤집는다**(X9). 고객 배송 큐의 직송 판정 축은
  "최상위·**최신 회차** 발주의 직송지"라(`routes/admin-pcb-orders.ts:56-66` `directShipPoBySpec`),
  원발주(round 0)가 **KR 로 실제 입고된** 주문이라도 그 뒤에 선 A/S 회차에 직송지를 주면 행이
  `직송 CN` 으로 바뀌고 종결 버튼이 [배송 처리] → **[직송 완료]**(운송장 없이 완료 종결)가 된다.
  실물은 자사 창고에 있는데 "현지에서 수령했다"로 닫히는 경로가 열린다. X9 의 마지막 어서션이
  그 현재 동작이므로, 교정하면 이 줄이 먼저 깨진다.
- **[ux] 포털 보드에서 박스가 왜 갈렸는지 읽을 수 없다**(X7). 박스 헤더는 받는곳·모드·건수만 내고
  직송지는 실리는데(`· 직송 CN`) **회차는 안 실린다** — r0 박스와 r1 박스의 헤더가
  '→ SamplePCB · 국외 발송' 으로 글자까지 같다. 회차 배지는 선반 항목에만 있다
  (`PartnerPcbShip.vue:118` 선반 vs `185~200` 박스).
