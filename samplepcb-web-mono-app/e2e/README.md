...
**전량 취소**에 각각 쳐서 `ORDER_CANCELED` 가드가 어느 쪽에서 서는지 나란히 세운다. 실측 결론은
**전량 취소는 3종 모두 409 로 막고, 부분 취소는 3종 모두 200 으로 통과한다**는 것이다 —
`isPcbOrderCanceled`(`lib/pcb-shipment.ts:183`)가 `od_status` 하나만 보므로 줄 단위 취소는 협력
트랙에 전혀 전달되지 않는다. 취소된 보드가 계속 생산되고 박스에 담겨 실제로 나갈 수 있다는 뜻이다.
10호는 그것을 고치지 않고 **현재 동작으로 박제**한다(`expect(200)`) — 가드가 줄 축을 갖게 되는 날
이 스펙이 먼저 빨개지는 것이 이 파일의 존재 이유다.

부분 취소는 협력 트랙 밖에서도 넷을 더 드러냈다: ① 과입금이 **음수 미수**로만 남고 환불 경로가
없다, ② 취소된 줄이 고객 상세의 **제작 진행 카드**에 그대로 살아 진행된다(od 만 보고 `ct_status`
를 안 본다), ③ 한 주문서 두 줄이 **같은 상품명**으로 찍혀 고객이 무엇이 취소됐는지 화면에서
구별할 수 없다, ④ PCB 고객 배송의 유일한 경로인 `force-status '배송'` 이 취소 줄을 **되살린다**
(취소 이력·금액이 조용히 사라진다). 정리는 `cleanup-probe.mts` 로 보강 없이 된다 — 같은
`force-status` 의 취소류 포함 규칙이 여기서는 정확히 필요한 동작이라서다.
