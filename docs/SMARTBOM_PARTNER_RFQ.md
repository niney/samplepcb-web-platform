...
상세 패널의 진입 버튼은 **회신 완료 협력사가 1곳 이상이거나 공급사 조회 가능한 선정 부품이
1개 이상이면** 노출한다. 협력사 회신 없이 3사만 비교하는 경우도 있으므로 명칭은
`공급사 비교·선정`으로 통일하고, 실제 공급사 대상 수는 sp-node 계산값을 사용한다.

단일검색에서 `협력사 보유`를 담은 행은 파일 분석 component가 없지만 비교에서 제외하지 않는다.
관리자가 `검토 시작`으로 `requested → reviewing` 전이하면 sp-node가 현재 단일검색 행을 최소
영속 분석 스냅샷으로 박제하고, 해당 협력사 행의 정확 MPN(+제조사)을 sp-engine의 기존
`admin_rfq_compare` 강제 라이브 검색에 태운다. 화면은 검토 전이를 먼저 확정하고 검색을
비동기로 폴링한다. 기존 협력사 선택과 금액 없음 상태는 유지하며, 3사 결과는 비교 후보로만
쌓이고 관리자가 `선정 적용`한 경우에만 구매 조건이 바뀐다. 이미 reviewing인 구형 Case는
`공급사 비교·선정` 진입이 같은 준비·검색을 수행해 게으르게 회복한다.
...
- **확정가 게이트(D16-1)의 인지 장치** — 확정가 없이 answered 가 되면 [주문하기]가 원천
  ...
  상태는 경고 박스 상시 표시 ② 관리자 [고객 회신 확정] 클릭 시 확정 총액 없으면 confirm 경고
...
- 주문 = 담기(기존 ctId 있으면 상태 검사: ordered→거부·cart→재사용·canceled/삭제→새 행 재담기) →
...
⑤ 파생 표시 — `orderState`(none|cart|ordered|canceled)를 `BomQuoteSummary` 로 승격(고객·관리자
...
- **액션 최소주의(1차)**: [입금확인]은 기존 `PATCH /api/admin/orders/status`(target='입금',
  ...
  조작 경로는 하나라 정합 안전. 결제 승인 취소·환불과 복잡한 주문 편집은 통합 관리에 남기고,
  미입금 BOM 부분취소·재주문은 D30에서 주문 축에 추가했다. 배송은 D21/D30의 활성 Case 규칙을 쓴다.
...
**§6.7 정정(2026-08-06) — 고객 회신 확정을 별도 업무 명령으로 분리**: 일반 PATCH는
`answered`를 거부하고 `reviewing` 상태에서만 `POST /api/admin/bom-quotes/:id/complete`가
품목 확인 상태와 낙관적 버전을 다시 검사한다. 따라서 요청 접수에서 고객 회신 확정으로 건너뛸 수
없으며, 확정 직전 저장 값과 이메일 발송 선택이 하나의 관리자 의사결정으로 묶인다. 관리자 화면은
`answered`를 **고객 확인 대기**, `closed`를 **마감**으로 표시한다. `answered → closed`는 기본
완료 순서가 아니라 더 진행하지 않는 견적의 신규 주문만 닫는 보조 작업이므로 **추가 작업 → 견적
마감**으로 분리하고, 주문·결제·발주·배송 완료가 아님을 확인창에서 고지한다.
...
- 문서: 견적번호=CASE-B 파생 채번·발행일=answeredAt, 수신(고객명 수기 초기화)/공급자
  ...
  미선정 '—')과 MPN 셀의 참고 이미지, 합계=부품 합계+운송료+관리비=
  **공급가액→부가세(10%)→합계**(D16 주문
  결제액 확정가×1.1 과 일치 — "견적서 합계=실결제액"), 회신 메모·결제계좌.
- 담당자: 관리자 `BOM 견적` 설정의 전용 담당자명·이메일을 고객·관리자 BOM 견적서에만 적용한다.
  두 값이 모두 비어 있으면 영카트 사업자정보의 정보관리책임자를 폴백으로 사용한다. 전용 값은
  `sp_config.bom_estimate_contact` JSON에 저장해 DB 마이그레이션 없이 역할을 분리하며, 회사·대표·
  주소·전화·계좌와 PCB 견적·주문 문서·협력사 메일의 담당자는 변경하지 않는다.
- 서버: 공용 `toBomQuotePrintDto`(합계는 저장 스냅샷 그대로 — 화면·문서 일치),
  이미지 원천은 선정 카탈로그→선정 후보→검증된 정확 일치 후보 순서이며 그 밖의 추측 이미지는
  반환하지 않는다. 이미지 URL은 표시 전용 nullable 계약이라 DB 마이그레이션은 없다.
  관리자 `GET /api/admin/bom-quotes/:id/print`(상태 무관 — 확정 전이면 예상값+**"가안"
  표기**), 고객 `GET /api/bom/quotes/:id/print`(**고객 회신 확정+확정가 시만** — 주문 게이트와
  동일 조건, 미충족·타인은 404 은닉).
- 화면: 공용 BomEstimateSheet/BomEstimateModal(components/smartbom, 로더 콜백 주입) —
  관리자 Case 상세 헤더와 고객 회신 확정 확인창의 [견적서 미리보기]에서 확정 전 **가안**을 확인하고,
  고객 /app/bom/:id 회신 박스 [🧾 견적서 보기·인쇄]는 고객 회신 확정 후에만 표시한다
  (canViewEstimate 조건부). 검토 중 저장한 `confirmed*`·`answerNote`도 고객 상세·목록 DTO에서
  숨겨 완료 전 초안이 새지 않게 한다. 미리보기는 11mm 이미지를 기본 표시하고, 인쇄 도구의
  [부품 이미지 포함]으로 이미지 없는 압축 문서를 선택할 수 있다. 인쇄는 외부 이미지의 로드·실패를
  최대 2.5초 기다린 뒤 진행하며 행 중간 페이지 분할을 피한다.
...
- **협력사 관점 상태 번역**(2026-08-02, "마감인데 담기 가능해 헷갈린다" 피드백):
  ...
  협력사에게 그대로 보여준 것. closed=견적 마감≠물류 완료(D22)라 담기 후보 유지가
...
| 견적관리 | 견적 | 검토 대기→RFQ 진행→고객 확인 대기→마감 | requested 수 |
...
### 6.21 관리자 견적 품목 확인 대기열 — D29 (2026-08-04)

관리자 Case 상세의 품목 표를 단순 결과 목록이 아니라 회신 전 확인 대기열로 쓴다. 기술적
호환성은 계속 sp-engine 판정이 정본이며, 응용 계층은 저장된 판정 근거와 수량·금액·구매 조건을
관리자 업무 우선순위로만 투영한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D29-1 | 공용 판정은 품목을 `즉시 처리 / 구매 확인 / 기술 검토 / 문의 진행 / 정상 / 제외` 중 하나로 분류하고 세부 사유를 함께 돌려준다. 수량 누락·미매칭을 최우선, 재고 문제·대체품·수명주기·요구조건 충돌·가격/재고 문의를 각각 해당 업무로 구분한다. | 관리자에게 다음 행동을 보여주되 sp-node·sp-vue에서 호환성 판단을 다시 구현하지 않음 |
| D29-2 | 협력사 RFQ 포함 체크와 관리자 확인 완료는 별도 열·별도 상태로 유지한다. 상태별 카운트 필터, MPN·제조사·Excel 위치 검색, 확인 대상 우선 정렬과 표시 목록 일괄 완료를 제공한다. | “협력사에 보낼 품목”과 “관리자가 검토한 품목”의 의미 혼동 방지 |
| D29-3 | 확인/재검토는 `sp_bom_quote_item_review`에 관리자·시각·현재 품목 지문과 함께 append-only로 남긴다. 수량, 매칭 근거, 선정 부품·구매 조건, 금액 또는 원본 위치가 바뀌면 지문이 달라져 이전 확인은 자동으로 `재확인 필요`가 된다. | 단순 불리언의 유실·오래된 확인 재사용 방지와 감사 가능성 확보 |
| D29-4 | 확인 이력 변경은 `requested|reviewing`에서만 허용하고 `answered` 전이는 서버가 미확인 품목 0건을 재검증한다. 기능 도입 전에 이미 회신된 Case는 소급 완료 처리하지 않고 읽기 전용 `확인 기록 없음`으로 표시한다. | 회신 안전장치를 우회하지 않으면서 과거 기록을 거짓으로 보정하지 않음 |
| D29-5 | 미매칭 행은 일반 확인과 구분해 `미매칭 상태 확인`으로 표시하고, 개별·일괄 완료 전에 부품·가격이 생성되지 않으며 고객 회신 게이트만 해제될 수 있음을 명시적으로 확인한다. 완료 후에도 미매칭·금액 미산출 표시는 유지한다. | 확인 완료를 매칭 해결로 오인하거나 미산출 행을 무심코 일괄 승인하는 사고 방지 |
| D29-6 | 엔진이 `automatic_selected`로 확정하고 확인 불필요·선정 구매 조건·금액까지 갖춘 행과 고객·관리자가 명시 선택해 `matchStatus=manual`이 된 행은 보조 누락·충돌·차순위 근거를 참고 정보로만 남기고 관리자 필수 기술확인에서 제외한다. `provisional_selected|confirmationRequired`는 고객 화면부터 Review로 표시하고, 미확정 review·미산출은 계속 확인 대상으로 유지한다. | 고객의 명시 결정과 엔진 자동 확정을 관리자가 반복 심사하지 않으면서 실제 미확정 행은 Matched로 오인하지 않게 함 |

- **API·데이터**: `PUT /admin/bom-quotes/:id/item-reviews`, migration
  `20260804112000_add_bom_quote_item_review`. 품목 변경과의 낙관적 동시성은
  `expectedQuoteUpdatedAt`으로 먼저 검사한다. 확인 이벤트도 같은 quote 버전을 원자적으로 올려
  다른 관리자의 확인·품목 변경·고객 회신과 직렬화하며, 지문 불일치가 최종 안전망이 된다.
- **회신 게이트**: sp-vue가 즉시 확인 필요 필터로 안내하며, 직접 API 호출에도 sp-node가
  `BOM_ITEM_REVIEW_REQUIRED` 409를 반환한다.

### 6.22 묶음 주문 부분취소 · 재주문 — D30 (2026-08-10)

배치 주문은 주문 헤더 하나에 BOM Case 여러 건이 연결된다. 따라서 주문 헤더가 `입금`이어도
개별 Case 행은 이미 `취소`일 수 있고, `quote.ctId`는 재주문 뒤 새 행을 가리킨다. 하류 업무는
헤더 상태만 보지 않고 영카트 카트행과 안정 키 `io_id=bom-{quoteId}`를 함께 사용한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D30-1 | sp-vue 즉시 취소는 대상 행·주문 헤더가 모두 `주문`, 정확한 `무통장`, 수납 0, PG 거래번호 없음, BOM PO 0건일 때만 허용한다. 그 밖의 결제 승인 취소·환불은 영카트 주문관리로 안내한다. | 외부 결제 취소보다 로컬 상태를 먼저 바꾸거나 이미 시작한 조달을 끊는 사고 방지 |
| D30-2 | 묶음에서 선택한 BOM 카트행만 `취소`하고 영카트 원칙으로 `od_cancel_price`·미수금·과세액을 재계산한다. 마지막 활성 행이면 주문 헤더도 `취소`한다. 취소 사유는 주문 변경이력에 남기고 메일은 자동 발송하지 않는다. | 다른 Case의 결제 가능 상태와 금전 정합을 보존하고 CS 안내 여부를 관리자 판단으로 유지 |
| D30-3 | 취소된 Case는 `answered` 확정 견적을 유지하고 고객 화면에 `주문 취소 · 재주문 가능`을 표시한다. 재주문은 옛 행을 수정하지 않고 새 카트행을 만들며 `quote.ctId`만 최신 시도로 옮긴다. | 과거 주문 감사 원장과 재주문 편의 동시 확보 |
| D30-4 | 관리자 주문 축은 현재 `ctId`만 역참조하지 않고 안정 `io_id`로 모든 주문 시도를 읽는다. 현재 활성/현재 취소/이전 주문 칩을 구분하고 배송·입고·발주 큐에는 현재 활성 시도만 참여시킨다. | 재주문 후 옛 주문에서 Case가 사라지거나 취소 이력이 새 주문 물류에 섞이는 문제 방지 |
| D30-5 | 결제·PO 게이트는 주문 헤더가 아니라 대상 행 상태를 쓴다. `취소|반품|품절|삭제|완료` 행은 새 PO를 막고, 배송·구매확정은 `preserveCanceled`로 활성 행만 전이한다. | 부분취소 뒤 동반 Case 결제 시 취소 Case가 발주 가능해지거나 배송으로 되살아나는 문제 방지 |

- **API·화면**: `GET/POST /admin/bom-orders/:quoteId/cancel[-preview]`, 주문·결제 화면의
  Case별 [취소]와 서버 판정 모달, 고객 BOM 상세·통합 견적관리의 [다시 주문하기]. 복잡한 취소는
  기존 영카트 주문관리 링크로 연결한다.
- **데이터**: 신규 테이블·컬럼 없이 `sp_bom_quote.ctId`, `g5_shop_cart.io_id/ct_status`,
  `g5_shop_order` 금액·변경이력을 재사용한다. 옛 카트행은 물리 삭제하거나 재활성화하지 않는다.
- **검증**: `journey-bom-reorder.e2e.test.ts`가 서로 다른 부품 Case 2건의 묶음 주문부터
  부분취소, 잔여 결제, 취소행 PO 409, 새 ct 재주문, 과거 연결 보존, 두 PO 묶음 입고,
  두 주문 배송·완료와 취소행 불변까지 순서대로 검증한다.

### 6.23 결제 후 공급 차질 · 잔량 대체발주 — D31 (2026-08-10)

고객 결제와 PO 발행 뒤 협력사가 일부 수량을 공급하지 못하는 경우를 취소·품목변경으로
되돌리지 않는다. 원 발주 문서와 고객 확정가는 보존하고, 미공급 수량만 다른 협력사의 기존
회신으로 추가 발주해 두 물류를 합쳐 이행한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D31-1 | 확인된 PO가 아직 발송에 담기기 전 협력사가 품목별 부족 수량·사유·메모를 1회 신고한다. 원 `sp_bom_po_item.qty/lineTotal`은 수정하지 않고 `sp_bom_po_shortage`에 별도 기록한다. | 발행 문서 불변과 실제 공급 차질 감사 이력을 함께 보존 |
| D31-2 | 원 PO의 Packing List·QR·Commercial Invoice·거래명세서는 `발주수량-부족수량`만 실제 출고량으로 사용한다. 품목 전량 부족도 허용하되 PO 전체 공급량이 0이 되는 신고는 1차에서 막는다. | 실물 수량과 통관·거래 문서 불일치 방지, 완전 공급불가는 별도 취소 협의로 분리 |
| D31-3 | 관리자는 같은 견적 품목에 회신한 다른 승인 협력사 중 단가>0, 재고·회신수량≥부족수량, 국가 등록, 기존 PO 없음 조건을 만족한 후보만 선택한다. 서버가 부족 수량을 그대로 새 PO 품목으로 박제하고 원 부족 원장과 연결한다. | 임의 수량·가격 조작과 중복·초과 발주 방지 |
| D31-4 | 대체 PO 발행은 `sp_bom_quote.selectedOffer/confirmedTotal`, 영카트 카트행·결제금액, 원 PO를 변경하지 않는다. 대체 단가 차이는 이 시나리오에서 내부 조달 차이로 처리한다. | 결제 후 고객 금액을 소급 변경하지 않는 정책 고정 |
| D31-5 | 미연결 부족분이 하나라도 있거나 현재 활성 Case의 PO 입고가 `n/n`이 아니면 고객 배송 큐를 열지 않는다. 일반 주문 `force-status 배송` 직접 호출도 서버가 현재 BOM Case를 재조회해 409로 막는다. | UI 경고 우회와 원 공급분만 받은 조기 고객 발송 방지 |
| D31-6 | 대체 PO가 발행 취소되면 FK `SET NULL`로 부족 원장이 다시 미복구 상태가 된다. 원 PO→부족 신고→대체 PO 품목 링크는 Case 상세 양쪽에 표시한다. | 실패 복구를 다시 수행할 수 있고 CS가 문서 계보를 추적 가능 |
| D31-7 | 부족 신고는 대체 PO가 연결되지 않았고 원 PO가 발송에 담기기 전까지 협력사가 수정·취소할 수 있다. 두 경로 모두 소유·PO 상태·대체발주·발송 여부를 서버가 다시 검증한다. | 오입력 한 번으로 이후 조달 수량이 고정되는 운영 사고 방지 |
| D31-8 | 대체 후보는 자동 선택하지 않는다. 관리자가 라디오로 명시 선택해야 발행 버튼이 활성화되며, 후보 카드에 출발국·국내 3단계/국외 6단계 물류 방식·대체 발주 금액을 함께 표시한다. | 협력사 선택과 물류 부담을 발행 전에 인지 |
| D31-9 | 원 PO의 박제 `totalAmount`와 부족분을 제외한 서버 계산 `actualSupplyAmount`를 구분해 표시한다. 품목에도 `suppliedQty`와 `suppliedAmount`를 함께 내린다. | 발주 문서 금액을 실제 정산·공급 금액으로 오인하지 않게 함 |

- **화면·API**: 파트너 발주 상세의 [공급 부족 신고], 관리자 Case 발주 패널의
  [잔량 대체발주]. `POST /partner/pos/:poId/shortages`,
  `PATCH|DELETE /partner/pos/:poId/shortages/:shortageId`,
  `GET /admin/bom-quotes/:id/shortages/:shortageId/candidates`,
  `POST /admin/bom-quotes/:id/shortages/:shortageId/recover`.
- **데이터**: 추가형 migration `20260810150000_add_bom_po_shortage_recovery`. 선적 1:PO 1
  제약은 유지한다. 원 협력사의 공급 가능분과 다른 협력사의 잔량이 각각 별도 PO·선적이므로
  D21의 동일 PO 다회 부분 입고 확장과는 다른 문제다.
- **검증**: `journey-bom-shortage-recovery.e2e.test.ts`가 4품목 주문·결제부터 부족 신고
  수정·취소·재등록, 후보 명시 선택과 물류 정보, 원 문서·고객 금액 불변, 정확히 7개
  대체 PO, 실제 13+7 포장 문서, 미복구/입고 1/2 배송 409, 입고 2/2 뒤 배송 완료,
  완료 문서 변경 409와 768/390px 반응형 회귀까지 순서대로 검증한다.

### 6.24 RFQ 미응답 회수·재배정 완주 — D32 (2026-08-11)

견적 검토 중 협력사가 응답하지 않으면 원 Case를 다시 만들거나 수동 대리 회신으로
우회하지 않고, 미응답 RFQ를 회수한 뒤 다른 협력사에 같은 요청을 재배정한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D32-1 | RFQ diff에서 빠진 `requested`만 삭제하고 `quoted`는 보존한다. 회수된 RFQ의 포털 조회와 매직링크는 즉시 404로 끊긴다. | 미응답 요청은 안전하게 회수하되 이미 받은 회신 원장은 유실하지 않음 |
| D32-2 | 재배정 RFQ는 새 256-bit 매직링크를 받고, 재발급하면 구 토큰을 즉시 무효화한다. 관리자 화면은 성공 후 기존 이메일 링크가 무효이며 [링크 복사]로 새 링크를 전달해야 함을 알린다. | 재발급 성공만 표시하고 실제 전달을 빼뜨리는 운영 공백 방지 |
| D32-3 | 무로그인·포털·관리자 대리 회신이 공유하는 품목표에 1280px 미만 가로 이동 안내·좌우 제어를 제공하고, 모든 행 입력에 `품번+필드` 접근성 이름을 부여한다. | 모바일에서 오른쪽 재고·납기·메모 존재를 놓치지 않고 보조기술도 입력 문맥을 구분 |
| D32-4 | 후속 선정·PO는 재배정 협력사의 회신만 참조한다. 회수된 협력사의 RFQ·PO가 0건임을 고객 배송 완료 후에도 재검사한다. | stale 요청이 선정·발주·물류에 재유입되는 것 방지 |
| D32-5 | 공유 회신 폼은 API 계약과 같은 숫자 범위를 전송 전에 검사하고, 오류 품목·필드를 알린 뒤 해당 입력으로 포커스를 이동한다. 공개 링크 조회는 404만 만료로 안내하고 일시 장애는 같은 화면에서 재시도하게 한다. | 입력 오류를 서버 장애로 오인하거나 정상 링크를 만료로 오인하는 복구 실패 방지 |

- **검증**: `journey-bom-rfq-reassignment.e2e.test.ts` 9단계가 요청 스냅샷·검토 시작,
  1차 RFQ 발송·회수, 구 포털/토큰 404, 재배정·토큰 회전, 무로그인 3품목 회신,
  선정·고객 확정, 국제 PO·Invoice/AWB·통관·입고, 고객 배송·완료, stale 참조 0건과
  390px 표 이동·입력 접근성, 잘못된 숫자의 무요청 차단, 503/404 안내 분리를 순서대로 검증한다.

### 6.25 외부공급사 수동 복구·구매 확인 — D33 (2026-08-11)

외부공급사는 사람 협력사처럼 포털에 로그인해 PO를 확인하지 않는다. 자동 실행할 수 없는
발주를 조용히 성공처럼 남기거나, 구매 확인 전 선적을 진행하지 않고 관리자 구매담당자의
명시적 확인으로 복구한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D33-1 | 공급사 PO에 한해 관리자가 실제 주문·결제를 확인한 뒤 `issued → confirmed`로 전이한다. 사람 협력사의 포털 확인 경로는 그대로 유지한다. | 로그인 주체가 없는 공급사 발주를 영원히 발행 상태로 두지 않고 책임 있는 내부 확인 원장을 남김 |
| D33-2 | 실행 가능한 공급사 SKU가 없으면 `NO_SKU_LINES`를 `externalRef` 실패 원장에 실행 시각·건너뛴 품목 수·수동 주문 안내와 함께 저장한다. 재시도는 실제 재실행 가능한 실패에만 제공한다. | 자동 실행 실패를 운영자가 인지하고 불가능한 재시도를 반복하지 않도록 함 |
| D33-3 | 발행 상태의 PO는 관리자 선적 생성·문서 입력·입고를 모두 `PO_NOT_CONFIRMED`로 거절한다. 구매 완료 확인 뒤에만 국제 물류를 열어 준다. | 외부 주문이 없는데 물류와 고객 완료가 앞서가는 상태 모순 방지 |
| D33-4 | 공통 화면은 `조달 발주`·`구매처`처럼 중립 용어를 쓰고, 공급사는 `구매 확인 대기/구매 완료`, 사람 협력사는 기존 확인 상태를 구분해 표시한다. | 공급사 구매와 협력사 회신을 같은 업무로 오해하지 않게 함 |

- **화면·API**: 관리자 Case 조달 발주 패널의 [구매 완료 처리],
  `POST /admin/bom-quotes/:id/pos/:poId/confirm`. 기존 API 계약 스키마와 DB migration은
  변경하지 않고 공급사 PO용 관리자 전이만 추가한다.
- **검증**: `journey-bom-supplier-procurement.e2e.test.ts` 6단계가 공급사 직접 선정·고객 주문,
  결제, SKU 없는 자동 실행 실패 박제, 구매 확인 전 선적 409, 관리자 수동 구매 확인,
  국제 Invoice/AWB·통관·입고와 고객 배송 완료를 순서대로 검증한다. 모든 SKU를 비워 실제
  외부 공급사 API 요청은 발생하지 않으며 390px 조달표도 함께 검사한다.

### 6.26 국내 묶음 거래문서 완주 — D34 (2026-08-11)

D27에서 구현하고 실데이터 검증을 남겼던 협력사 견적서·거래명세서를 국내 묶음 주문의
발주부터 고객 완료까지 연결한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D34-1 | PO별 협력사 견적서는 발행 시점의 협력사·SamplePCB 사업자정보, 품목 거래조건과 금액을 박제하며 관리자와 협력사는 같은 문서 원본을 조회한다. | 기준정보 변경이 이미 발행한 거래문서를 소급 변경하지 않게 함 |
| D34-2 | 묶음 거래명세서는 소속 PO 전체와 Packing List를 조합해 R0 초안→저장 revision→발송 시 확정으로 전이하고, 출고 수량·LOT·Date Code·공급가액·VAT를 실제 포장 기준으로 합산한다. | PO별 주문 조건과 한 박스의 실제 출고 내용을 동시에 추적 |
| D34-3 | 거래문서 레이어는 이름 있는 modal dialog, 포커스 트랩·열기 버튼 복귀·배경 스크롤 잠금을 제공한다. 840px 미만에서는 A4 문서를 왼쪽부터 열고 좌우 안내·버튼을 고정하며 로드 실패 뒤 같은 화면 재시도를 제공한다. | 키보드·보조기술과 모바일에서 문서 일부를 놓치거나 오류 뒤 다시 진입하는 문제 방지 |
| D34-4 | 입고·고객 배송 완료 뒤에도 확정 문서번호·revision·스냅샷은 불변이다. | 종결 후 재조회·재인쇄가 당시 거래 원장과 일치 |

- **화면·API**: 기존 관리자 선적 모달과 협력사 발송 카드의 [견적서]/[거래명세서],
  기존 관리자·협력사 quotation/statement 조회 API를 사용한다. DB schema·migration과 API
  계약 변경은 없다.
- **검증**: `journey-bom-trade-documents.e2e.test.ts` 7단계가 Case 2건 묶음 주문·결제,
  PO 2건 견적서 박제와 역할 간 동일성, 사업자정보 변경 비소급, 국내 묶음 Packing R0→R1,
  출고·입고·고객 완료와 완료 후 문서 불변을 검증한다. 실제 관리자·협력사 화면, 390px
  포커스·좌우 이동, 1회 잘못된 응답 뒤 재시도까지 포함한다.

### 6.27 Case 영구 삭제·초기화 안전 경계 완주 — D35 (2026-08-11)

D14에서 구현한 Case 강제 삭제를 정상 성공만이 아니라 영카트 주문·감사 원장·동시 변경과
접근성 경계까지 실데이터로 검증한다. 삭제 검증용 Case는 주행 중 생성한 대상만 사용한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D35-1 | 삭제 전에 서버가 Case·주문·발주·선적·파일 영향을 다시 계산하고 SHA-256 프리뷰 토큰을 발급한다. 프리뷰 뒤 Case나 연결 원장이 바뀌면 같은 화면 요청도 `STALE_PREVIEW`로 거부한다. | 오래된 화면이 달라진 삭제 범위에 그대로 동의하는 경쟁 조건 방지 |
| D35-2 | `audited`는 관리자·사유·삭제 영향의 최소 SmartBOM 감사행과 단독 주문의 영카트 삭제 백업을 남기며, `reset`은 둘 다 남기지 않는다. 단독 결제 주문은 별도 강제 체크로만 허용하고 공유 주문은 어떤 강제 플래그로도 우회하지 못한다. | 운영 삭제와 테스트·오입력 초기화의 기록 정책을 분리하고 다른 Case 거래 경계를 보존 |
| D35-3 | 삭제 레이어의 영향·최종 확인·결과는 각각 이름 있는 modal dialog이며 단계마다 포커스를 안으로 옮기고 Tab을 가둔다. 배경 스크롤을 잠그고 닫은 뒤 열기 버튼으로 복귀하며, 영향 조회 실패는 같은 화면에서 재시도한다. | 키보드·보조기술 사용자가 위험 범위나 완료 결과를 놓치고 배경을 조작하는 문제 방지 |
| D35-4 | 삭제 성공 시 상세 observer는 즉시 재조회하지 않고 캐시를 stale 처리한다. 결과를 확인하고 목록으로 이동한 뒤 최신 목록을 읽는다. | 정상 삭제 직후 이미 사라진 Case·RFQ·PO를 재조회해 404를 연속 발생시키는 화면 경합 제거 |

- **화면·API**: 기존 `/app/admin/smartbom/cases/:id` 위험 구역과
  `GET /admin/bom-quotes/:id/force-delete-preview`,
  `POST /admin/bom-quotes/:id/force-delete`를 사용한다. API 계약·DB schema·migration 변경은 없다.
- **검증**: `journey-bom-case-deletion.e2e.test.ts` 7단계가 audited 일반 삭제, reset 미입금
  주문 삭제, 결제 주문 이중 확인과 두 감사 원장, 공유 주문 UI/API 차단, stale 프리뷰,
  실패 재시도와 390px 레이어를 검증한다. 최종 주행에서 7/7 통과, 고객·관리자 pageerror와
  예상 밖 HTTP 오류는 모두 0건이었다.

### 6.28 Case 일괄 삭제 혼합 결과 안전 경계 완주 — D36 (2026-08-11)

D35의 단일 Case 삭제 규칙을 진행현황 목록의 혼합 선택으로 확장한다. 일부 Case가 보호되거나
실행 직전에 바뀌어도 삭제 가능한 나머지 작업은 완료하고, 실패 대상의 선택과 원장은 보존한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D36-1 | 일괄 영향 화면은 Case별 결과를 삭제 가능·결제 강제 가능·보호·조회 실패로 나누고, 전체 또는 일부 조회 실패를 같은 레이어에서 다시 확인할 수 있게 한다. | 한 건의 일시적 조회 실패가 전체 선택을 막거나 실패 Case를 삭제 가능 대상으로 오인하는 문제 방지 |
| D36-2 | 결제 주문은 관리자가 별도로 강제 포함하며, 실행 대상 수가 바뀌면 기존 복구 불가 확인을 즉시 해제한다. 공유 주문은 일괄 작업에서도 항상 제외한다. | 확인 당시 범위와 실제 삭제 범위를 일치시키고 다른 Case가 공유하는 거래 원장을 보존 |
| D36-3 | 실행은 Case별 최신 프리뷰 토큰으로 독립 처리한다. 한 Case가 `STALE_PREVIEW`로 실패해도 나머지를 계속 삭제하고 결과에서 성공·실행 실패·사전 제외를 구분한다. | 부분 경합을 전체 실패로 확대하지 않으면서 관리자가 남은 작업을 정확히 재검토하도록 지원 |
| D36-4 | 결과를 닫으면 성공 ID만 목록 선택에서 제거하고 실패·보호 ID는 선택된 채 유지한다. 일괄 레이어도 단계별 이름·포커스 트랩/복귀·배경 스크롤 잠금과 390px 안전 표시를 제공한다. | 재선택 비용과 위험 대상 누락을 줄이고 작은 화면·키보드에서도 작업 맥락을 보존 |

- **화면·API**: 기존 `/app/admin/smartbom` 목록과 Case별
  `GET /admin/bom-quotes/:id/force-delete-preview`,
  `POST /admin/bom-quotes/:id/force-delete`를 재사용한다. API 계약·DB schema·migration 변경은 없다.
- **검증**: `journey-bom-bulk-deletion.e2e.test.ts` 7단계가 일반·결제·stale·조회 재시도·공유
  주문 Case 5건의 혼합 선택, 3건 부분 성공과 2건 유지, 성공 ID만 선택 해제, 감사행 3건,
  결제 주문 백업과 공유 주문 불변, 3단계 접근성 및 390px 표시를 교차 검증한다. 최종 주행에서
  7/7 통과, 고객·관리자 pageerror와 예상 밖 HTTP 오류는 모두 0건이었다.

### 6.29 배송 후 고객 문제 접수·관리자 해결 — D37 (2026-08-11)

영카트 주문 종결과 별개로 누락·파손·오배송·품질 문제를 추적하는 사후 대응 축이다. 고객 접수만으로
주문 취소·환불·재발송을 자동 실행하지 않으며, 관리자가 실제 후속 조치를 별도 운영한 뒤 고객에게
약속한 내용과 상태 전이를 클레임 원장에 남긴다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D37-1 | 고객은 현재 BOM 주문행과 주문 헤더가 모두 `배송` 또는 `완료`이고 행이 취소·반품·품절·삭제가 아닐 때만 접수한다. 서버가 매 요청마다 영카트 상태를 다시 읽고 배송 전 직접 호출도 409로 거부한다. | 부분 상태 변경된 미배송 행의 문의를 배송 사고로 기록하거나 닫힌 주문행을 되살리는 상태 모순 방지 |
| D37-2 | 접수는 Case·주문번호·결제 상태, 선택한 MPN·제조사·주문수량·문제수량을 `sp_bom_claim*`에 박제한다. `activeKey` unique로 같은 현재 주문행에는 `open/reviewing` 한 건만 두고 종결 시 키를 해제한다. | 견적·Case가 이후 바뀌어도 당시 고객 주장과 주문 근거를 보존하고 동시 중복 접수를 DB에서도 차단 |
| D37-3 | 상태는 `접수됨(open) → 검토 중(reviewing) → 해결 완료(resolved) 또는 처리 불가(rejected)` 단방향이다. 관리자 PATCH는 `expectedVersion`을 비교하고 모든 전이를 append-only 이벤트로 남긴다. | 두 관리자의 동시 처리로 답변이 덮이거나 검토 없이 종결되는 사고 방지 |
| D37-4 | 접수와 관리자 답변은 영카트 주문·결제·재고, BOM 확정 견적·PO·선적을 자동 변경하지 않는다. 해결 방식은 대체품 발송·환불 별도 협의·차감/보상 협의·기술 안내라는 운영 약속만 기록한다. | 서비스 응대 기록과 실제 금전·물류 실행의 책임 경계를 분리 |
| D37-5 | 활성 클레임이 있으면 Case 삭제 프리뷰에 `OPEN_CLAIM`을 추가하고 결제 주문 강제 삭제 확인으로도 우회하지 못한다. 종결 클레임은 Case와 FK를 두지 않아 이후 삭제에도 보존한다. | 처리 중 고객 대응 근거 유실 방지와 종결 감사 원장의 독립성 확보 |
| D37-6 | 고객 모바일 하단 진입은 주문 뒤 `주문·문제 접수`로 바꾸고, 접수 모달과 관리자 상세는 이름·포커스 트랩/복귀·배경 스크롤 잠금·390px 안전 표시를 제공한다. 내부 모달 Escape는 바깥 주문 패널을 닫지 않으며, 처리 후 사라지는 버튼 대신 상태 영역으로 포커스를 보낸다. | 사후지원 기능 발견성, 중첩 레이어 키보드 조작, 처리 완료 뒤 포커스 유실 방지 |

- **화면·API**: 고객 `/app/bom/:id`의 배송 후 문제 접수 패널과
...
- **검증**: `journey-bom-claims.e2e.test.ts` 7단계가 배송 전 차단, 배송·완료 자격, 복수 품목
  390px 접수, 중복·Case 삭제 보호, 관리자 순차 전이·stale version, 해결 답변, 다음 접수 자격과
  주문·결제 불변을 UI/API/DB로 교차 검증한다. 독립 재주행 2회 모두 7/7 통과했고 고객·관리자
  pageerror와 예상 밖 브라우저 HTTP 오류는 모두 0건이었다.

### 6.30 고객 BOM 소유권·직접 URL 복구 경계 — D38 (2026-08-11)

고객 Case의 URL을 알게 된 다른 회원이나 관리자라도 고객 API의 회원 소유권을 우회하지 못하게 하고,
삭제·권한·주소 오류와 일시 장애를 고객이 다음 행동을 선택할 수 있는 화면 상태로 구분한다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D38-1 | 고객 BOM 상세·파일·변경·견적요청·주문·클레임·삭제는 매 요청 `mbId` 소유권을 확인하고 타인과 미기록 ID를 같은 404로 은닉한다. 관리자도 고객 API에서는 예외가 아니며 운영 조회는 `requireAdmin` 전용 API만 사용한다. | URL 추측으로 존재 여부·품목·거래 상태가 노출되거나 관리자 권한이 고객 컨텍스트를 오염시키는 사고 방지 |
| D38-2 | SPA는 양의 signed BIGINT 범위가 아닌 `:id`를 API 호출 전에 영구적인 잘못된 주소로 분류한다. 숫자형 404는 삭제·비소유를 구분하지 않고, 5xx는 같은 화면의 명시적 재시도로만 회복한다. | 깨진 링크를 서버 장애로 오인해 무한 재시도하거나 범위 밖 ID가 API 400/500을 만드는 불필요한 오류 제거 |
| D38-3 | 최초 상세 조회 실패는 빈 본문 대신 이름 있는 alert 카드로 표시하고 카드에 포커스를 보낸다. 404·잘못된 주소의 첫 행동은 `내 BOM 내역`, 5xx의 첫 행동은 `다시 시도`이며 390px에서도 문서 가로 넘침이 없다. | 오류 원인과 복구 경로를 인지하지 못한 채 막히거나 모바일에서 보조 행동을 먼저 선택하는 마찰 방지 |
| D38-4 | 익명 고객은 로그인 URL에 원래 `/app/bom/:id`를 보존하고, 비관리자는 관리자 컴포넌트·API를 호출하기 전에 앱 홈으로 복귀한다. | 인증 후 작업 맥락 보존과 클라이언트 가드 단계의 불필요한 권한 요청 차단 |

- **화면·API·DB**: 고객 `/app/bom/:id`의 최초 조회 오류 상태와 공용 BIGINT 라우트 ID 검증을
  추가한다. 기존 고객/관리자 API 소유권을 그대로 검증하며 계약·DB schema·migration 변경은 없다.
- **검증**: `journey-bom-access-control.e2e.test.ts` 6단계가 소유 고객 조회, 타 회원 8개 단건
  경로와 배치 주문 은닉·DB 불변, 390px 404와 비숫자 무호출, 503 재시도, 익명·비관리자 가드,
  관리자 전용 경계를 UI/API/DB로 교차 검증한다. 최종 독립 주행 2회 모두 6/6 통과했고 세 역할의
  pageerror는 0건이며 HTTP 404·503은 시나리오가 의도적으로 만든 응답만 관측했다.

### 6.31 운송수단(항공/해상) — D39 (2026-08-16)

PCB 트랙에 먼저 세운 축(docs/PCB_PARTNER_TRACK.md)을 BOM 선적에도 옮겼다. **공용 사전**
(`SHIPMENT_TRANSPORTS`)을 `bom-po.ts`에 두고 PCB가 그것을 쓴다 — 같은 실물 축을 두 트랙이
다른 어휘로 부르면 협력사가 화면마다 다른 말을 듣는다.

| 결정 | 내용 | 이유 |
| ---- | ---- | ---- |
| D39-1 | `sp_bom_shipment.transport`(air\|sea, NULL 허용, **컬럼 default 없음**) 신설. 국제 전용이며 국내에서 오면 서버가 버린다(`upsertShipment` 의 `mode === 'international'` 게이트 — 기존 shipDate 관례와 동형). | mode(국경)와 직교하는 축이고, default 를 두면 "명시적 항공"과 "이 축 도입 전 데이터"를 영영 못 가른다 |
| D39-2 | 첨부 사전에 `bill_of_lading`(라벨 `B/L`) 추가. 화면은 사전 전체가 아니라 **인보이스 + 수단이 정한 운송서류 1종**만 줄 세운다(`docKinds`). | 해상 발송에 AWB 줄이 함께 서면 "둘 다 내야 하나"로 읽힌다. 이미 올라온 반대편 서류는 수단을 되돌리면 다시 보인다(삭제하지 않는다) |
| D39-3 | 수단이 바뀌면 서버가 `carrier`·`trackingNumber`·`trackingUrl` 을 함께 비운다. 이 정리 스프레드는 carrier 스프레드보다 **앞**에 둔다. | 항공사명 + 해상 B/L 번호 같은 모순 방지. 수단 변경과 새 운송장 입력이 한 요청에 오면 뒤에 오는 새 값이 이겨야 한다 |
| D39-4 | 협력사는 '선적 요청'에서만 고르고(바꾸려면 되돌리기), 관리자는 선적 모달에서 언제든 고친다. | 협력사 쪽은 전이 게이트가 원칙, 관리자 쪽은 임의 조작이 원칙 — 기존 두 화면의 성격을 그대로 따른다 |

- **동반 교정**: `toShipmentFileMeta` 판정을 손으로 나열한 종류 비교에서 **계약 사전 파싱**으로
  바꿨다(PCB 쪽은 그 방식이라 08-15 신설 종류가 `invoice` 로 접혔다 — 같은 함정을 BOM 이 사전
  확장과 동시에 밟을 뻔했다). 모르는 종류는 접지 말고 뺀다.
- **범위 밖**: 상업송장(`BomInvoiceData`)의 `By Air`/`By Sea` 표기. 엑셀 렌더러·초안 저장까지
  파급되고 두 트랙의 송장 양식 변경은 실무 확인이 필요하다 — 지금은 생성기 밖에서 처리한다.
- **화면**: 협력사 카드는 '선적 요청' 폼 맨 위에 라디오(항공 특송·AWB / 해상 선박·B/L),
  관리자 선적 모달은 국제일 때만 셀렉트, 물류 워크큐·카드 요약에 수단 배지.
...
- **선정 공급사 투명성**: RFQ 패널에 DigiKey·Mouser·UniKeyIC 3사를 항상 카드로 놓고,
  선정된 사람 협력사를 같은 횡에 추가한다. 카드의 품목 수·합계와 상세 모달의 MPN·
  제조사·SKU·포장·주문수량·단가·행 합계는 현재 `selectedOffer`와 RFQ offerKey에서 파생하며
  별도 행을 생성하거나 공급사 API를 호출하지 않는다. 카드·상세 모달은 공용 surface/
  ink 토큰으로 라이트·다크를 같이 지원하고, 품목 보기 카드에 hover 이동·색·그림자 효과를
  두지 않는다.
...
### 6.36 협력사 보유 부품 — 재고표 업로드·BOM 후보 주입 (2026-08-23)

협력사가 갖고 있는 부품을 올려 두면 고객 BOM 분석에서 **기존 공급사와 같은 자리에서
뒤순위 후보로** 뜨고, 관리자가 그 행의 견적요청을 걸기 쉬워진다.

- 정본은 **[PARTNER_PARTS.md](PARTNER_PARTS.md)** — 여기서는 RFQ 트랙과의 접점만 적는다.
- **RFQ 제한은 두지 않는다**(사용자 결정). 매칭 협력사에게만 발송을 강제하면 D32(미응답
  회수·재배정, §6.24)와 충돌하고, 품번 대량 업로드로 RFQ 를 선점하는 인센티브 왜곡이 생긴다.
  대신 행 배지(`협력사 보유`)로 **표시만** 하고 발송 판단은 관리자가 한다.
- 협력사 후보는 **가격을 만들지 않는다**(주입 product 의 `offers` 가 비어 있다). 따라서 어떤
  자동 선정도 금액을 만들지 못하고, 값은 기존대로 RFQ 회신(`selectionSource='partner'`,
  `offerKey='rfq:{rfqItemId}'`)이 정본이다.
- capability 는 기존 `part_sale` 을 살려 쓴다 — 정의·라벨·관리자 체크박스가 있는데 어디서도
  검사되지 않던 능력이었다. 포털에서는 모듈이 아니라 **공통 영역**(`tracks.parts`).
...
> 같은 날 견적관리(/shop/quotes, D17 통합 목록)도 Figma 103:2659 로 — 구분선 행(파일명 / 사양[PCB 만] / 유형 | 출고예정 | 접수) +
> 우측 '주문 예상 금액' 패널(주문하기 버튼 포함). VAT포함은 커스텀 툴팁(공급가+부가세), 탭 라벨 `부품`, 날짜 YY.MM.DD, BOM 제목·썸네일이
> /app/bom 링크(PCB 는 고객용 상세가 없어 링크 없음 — 후속). 새 규칙은 `.sp-quotes--fig` 스코프. 보관함(/shop/quotes/archive)도
