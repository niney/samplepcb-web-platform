...
- **ES 유령 문서(DB에 없는 색인) — 2026-08-16 실측**: DB 행이 사라졌는데 ES 문서가 남으면
  같은 부품이 **두 문서**로 검색된다. 실측 사례는 `GRM155C81A225ME44D`(Murata)로, ES 에
  `_id=367`(DB 없음)과 `_id=81178`(DB 있음)이 **같은 점수(8.768374)**로 공존했다. 점수가
  같으니 어느 쪽이 먼저 나올지 비결정적이고, 유령이 먼저 잡히면 부품 상세
  `GET /api/bom/parts/367` 가 **404** 라 구매 조건 패널이 영영 안 열린다(화면은 로딩만).
  - **증상 판별**: "검색 결과는 뜨는데 [구매 조건 보기]를 눌러도 아무 일이 없다" +
    네트워크 탭에 `/api/bom/parts/:id` 404. 재현이 간헐적인 것이 특징이다.
  - **처방**: `pnpm --filter api parts:reindex --recreate`(로컬) — 인덱스를 지우고 DB
    전량을 다시 색인해 고아 문서를 없앤다. DB가 진실원본이라 안전하다.
  - **원인 축**: 인제스트는 upsert 만 하고 **DB에 없어진 문서를 청소하지 않는다**. 하드
    삭제·병합 경로는 ES 삭제를 동반하지만, 그 밖의 경로(운영 sync 반영 등 DB 직접 교체)로
    행이 갈리면 유령이 남는다. 정기 정합 점검이나 삭제 훅은 아직 없다(미착수).
