...
### 히어로(banner 01~05)
- **배너 04 중앙 원 시안 일치(사용자 요청, 2026-09-21)**: 지정 노드 2286:36588과 안쪽 원 36589의 angular stroke·drop shadow가 모두 `visible:false`임을 확인하고, CSS에서 강제로 표시하던 두 색상 테두리를 제거했다. 바깥 278px 원의 흰색 30%·안쪽 220px 흰색을 유지하고, Figma SVG의 실제 필터 값에 맞춰 CSS backdrop blur 20px / 바깥 blur 2.5px / 안쪽 blur 1px로 교정했다. 위치(x1116/y226, 1920 화면)·문구(24px/34px, #6249dd, 900/700)는 유지. CSS 버전 26092111. 1920/1440/1280/1024/768/390/320px에서 원의 크기·중앙 정렬·테두리/그림자 없음·필터·문구 스타일 검증, PC/모바일 캡처 육안 확인 및 PHP 문법 검사 통과.
...
- **SMT Eyes 정밀 검토 아이콘 교정(2026-09-21)**: 지정 행 `2286:1710`이 부품 정보 아이콘을 중복 사용하던 상태를 최신 시안의 체크 표시가 있는 검토 아이콘으로 교체. `2337:39353`의 배경·체크 표시를 포함한 54×54 PNG export를 `ico-smt-review.png`로 그대로 사용하고 기존 행 배치를 유지한다. 1920/390px 캡처에서 아이콘·크기·정렬 확인, PHP 문법 검사 통과.
- Parts의 기능 3행 아이콘은 2행과 동일한 복제 상태를 유지. GIF 에 완전히 덮인 "DFM 분석 화면 3"·SMT iMac 목업은 미사용.
...
- **숫자 카운트업(사용자 요청, 2026-09-21)**: [메가존클라우드 회사소개](https://www.megazone.com/about/company)의 숫자 영역을 브라우저와 공개 번들로 확인하여 반영. 숫자/항목이 60% 보이면 한 번만 재생하며, 2초 카운트업·20px 위로 등장·0.3초 ease-in-out·항목 간 0.1초 간격을 적용한다. 참고 페이지의 CountUp 동작처럼 999 초과 수는 전반부 선형 증가 후 마지막 333에서 지수 감속하며, 작은 수는 전체 구간에서 감속한다.
- `js/home/stats.js`는 PHP가 출력한 최종 문자열을 읽어 쉼표·`+`·`%`를 유지한다. 최종 문자열로 공간을 예약하고 `aria-hidden` 카운터만 겹쳐 너비 흔들림과 스크린리더의 연속 숫자 낭독을 방지한다. 화면 진입 전에는 프레임을 예약하지 않고, 세 항목이 완료되면 관찰·이벤트·프레임을 정리한다. JavaScript/IntersectionObserver 미지원·동작 줄이기에서는 최종값을 즉시 표시하며 재생 도중 동작 줄이기로 바꾸어도 최종값으로 마무리한다. 별도 라이브러리·DB/API·통계 수치 변경 없음. CSS/JS `26092116`/`26092106`.
- 숫자 효과 검증: 1920/1280/768/390/320px에서 화면 밖 대기·진입 후 중간값·정확한 최종값·재진입 시 재생하지 않음·숫자 너비/가로 위치 유지·페이지 가로 넘침 없음 확인. reduced-motion 초기 및 진입 전/재생 중 전환, JavaScript 비활성, IntersectionObserver 미지원의 최종값·표시 검증 통과. pageerror·모듈 로드 오류 0, PC/모바일 중간·완료 캡처 확인, PHP/JS 문법과 `git diff --check` 통과.
- 로고 20종은 2줄 **무한 마퀴**(위 ← 70s / 아래 → 50s, 속도 임의, reduced-motion 정지). 사용자 요청(2026-09-21)으로 마우스오버 정지 규칙을 제거해 두 줄 모두 계속 흐르게 한다. CSS 버전 `26092117`. 피그마 마스크는 페이드가 아니라 하드 클립이라 페이드 없음. **로고 사용 허락 확인**: 유니탑·인지니어스(RADAR by INZINIOUS)·NuEyne·삼성중공업·RORZE·Nordic·INCERASOLUTION·BUGANG·TELECONS·EKPOWER·멀티·글로벌코넷 / 세종대·한양대·성균관대·고려대·서울대·KAIST·POSTECH·단국대. 노드명≠로고("Screenshot … 2.48 PM"=유니탑, "intro"=EKPOWER 등), POSTECH 은 원본 JPEG(불투명 근백색, 피그마도 같음).
