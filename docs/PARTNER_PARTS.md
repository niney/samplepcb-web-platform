...
**① 고객 세션 노출 (P5) — 샘 없음.** 실제 고객 계정 브라우저로 단일검색 → 담기 → 카트까지
가며 본문 텍스트에 조직 이름이 있는지 검사했고, API 응답도 같은 계약으로 확인했다
(`hasPartnerStock` · `partnerStock.partnerCount` 는 보이고 이름은 없다). 화면만 가리고 응답에
남으면 우회로가 열리므로 둘 다 본다.
