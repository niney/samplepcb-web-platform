...
### 낡음 기준일 — 운영 설정 (2026-08-24)

만료를 두지 않기로 한 이상(P4) '낡음'이 **유일한 신호**다. 그런데 재고표 갱신 주기는
협력사·품목군마다 다르다 — 커넥터 재고는 분기로 움직이고 MLCC 는 주 단위로 바뀐다.
그래서 env 상수를 `sp_config` 로 승격했다(`partner_parts` 키, env 는 **기본값**으로만 남는다).

`GET/PUT /api/admin/partner-parts/config` · 화면은 관리자 협력사 보유 부품 상단 카드의
`기준 변경` — 설정 페이지가 아니라 **그 값이 쓰이는 자리**에 둔다(요약 배지가 바로 옆이다).
1~3650일로 막는다: 0일이면 모든 원장이 늘 낡음이 되어 신호가 죽는다.
⚠ 이 값은 **표시 기준일 뿐 원장을 지우지 않는다** — 버튼 툴팁이 그 사실을 말한다.

### 후보 드로어 — 협력사 후보 표기 (2026-08-24)

협력사 후보는 구매 조건이 없어 카드가 밋밋했다. 가격 칸에 "구매 조건 없음"만 뜨면
담당자가 **왜 이 후보가 떴는지** 모른다. `협력사 보유` 배지와 `견적요청 후 확정` 문구를
달아 다음 행동을 가리킨다 — 값은 협력사의 주장이라 여기서 약속하지 않는다.
판정은 `offers.length === 0 && corroboratingSuppliers.includes('partner')` 한 줄이다.
...
- 반영 비용(일괄화 후, 2026-08-24): **콜드 12,175행 19초**(그중 색인 10,506건은 +1초 — bulk 라
  사실상 공짜) · **재실행 1초**(바뀐 게 없으면 쓰기도 색인도 0). e2e 대형 스펙 70초 → 26초.
  청크마다 식별자 일괄 조회 → `createMany` → 구매 조건 일괄 조회 → 신규만 `createMany` ·
  값이 바뀐 것만 개별 update 로 줄였다(재업로드는 대개 update 0건이라 재실행이 특히 싸다).
...
`test_inventory_synthetic_corpus.py` — 엑셀·브로커 시트의 **알려진 병리**를 3본에 섞어 담았다
(병리마다 한 본이 아니라 섞은 이유: 실제 파일이 그렇게 오고, 겹칠 때의 상호작용이 진짜
사고 지점이다 — 헤더가 4행 아래인 데다 품번이 날짜로 변한 시트 같은 것).

| 본 | 담긴 병리 |
|---|---|
| 서식 사고 | 헤더가 1행이 아님(로고·연락처 위) · 엑셀이 품번을 날짜로 변환(`Jan-10`) · 앞자리 0 손실(`402`) · 지수 표기(`1e5`) |
| 표 둘 | 한 시트에 표 2개 · 소계 행 · 구역 제목 · 시트 중간에 다시 나온 머리글 · 수량 표기 3종(`1,000`·`700 pcs`·`1k`) |
| 지저분한 셀 | 병합 셀 · 전각 문자 · 비파괴 공백 · 숨긴 열 |

**이 코퍼스가 바로 결함 둘을 잡았다:**
- `1k` 가 **1** 로 읽혔다 — 숫자만 뽑는 파서가 `k` 를 버렸다. 에러도 안 나는 **조용히 틀린 값**이라
  실물 파일에서도 몰랐을 것이다. 교정: `_parse_int` 가 `k`/`K` 접미를 ×1,000 으로 읽는다.
- 소계 행·다시 나온 머리글이 **검토 표시 없이 부품으로 앉았다**. 재고가 조용히 부풀고 원장에
  정체불명 행이 남는다. 교정: **영숫자가 하나도 없어 조회 키를 못 만드는 값**은
  `mpn_needs_review` 를 단다(버리지는 않는다 — 무유실). `소계`·`품번`·`[ 신규 입고 ]` 가 한 규칙에 걸린다.

⚠ **합성 코퍼스는 회귀 방지용이지 발견용이 아니다.** 여기 담긴 것은 우리가 상상한 병리뿐이라
추출기와 같은 가정을 공유한다. "한 파일로 설계한 추출기는 다음 파일에서 깨진다"의 그 지점은
실물 두 번째 파일이 와야 드러난다.

### 남은 것

- ⛔ **실물 협력사 파일 2~3본**(`local-corpus`) — 파일이 있어야 진행. 고객 파일·PII 가 있는
  본은 gitignore 된 `fixtures/local/` 에만 둔다(거버 zip 이 쓰는 관례와 같다).
- 선택: RFQ 회신에 "내 원장도 이 값으로 갱신" 체크 — 회신이 가장 정확한 최신 정보다.
  다만 회신 값(견적가)과 원장 값(보유 재고)은 성격이 달라 무엇을 덮을지 정해야 한다.

### 끝난 것 (2026-08-24)

- ~~대형 원장 반영 지연~~ → 콜드 19초 · 재실행 1초(§1.5 실측)
- ~~후보 드로어 협력사 표기~~ → `협력사 보유` 배지 + `견적요청 후 확정`
- ~~관리자 설정에 낡음 기준일 노출~~ → `sp_config` 승격 + 상단 카드 `기준 변경`
- ~~단일검색·[부품 추가]~~ → 카탈로그 투영 + `협력사 보유` 결과표 + `partner_stock` 담기
- ~~회귀 코퍼스~~ → 합성분 완료(위). 실물분은 파일 대기.
