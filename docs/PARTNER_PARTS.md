...
| P5 | 고객 단일검색 Distributor에는 **협력사명 노출** — 연락처·조직 ID·협력사별 정확 재고는 계속 비노출. BOM 분석/견적 상세는 기존처럼 곳 수·기준일만 | 2026-08-27 사용자 정책 변경. 검색 단계에서는 보유 주체를 명확히 하되 RFQ 대상 선정과 개별 상업 조건은 관리자 경계에 둔다 |
...
| 칸 | 값 |
|---|---|
| Distributor | 현재 승인·`part_sale` 협력사명. 3곳 이상은 `A · B 외 N곳`, 전체 이름은 툴팁 · 아래줄에 `협력사 재고표 · N일 전 기준` |
...
견적요청 뒤 관리자 `검토 시작`에서는 이 명시적 협력사 선택을 exact MPN 공급사 비교의
시작점으로 사용한다. 단일검색에는 업로드 분석 component가 없으므로 현재 행으로 최소 영속
분석을 만들고 기존 sp-engine 3사 강제 라이브 검색을 재사용한다. 협력사 선택은 자동 교체하지
않고 DigiKey·Mouser·UniKeyIC 결과만 비교 후보로 박제한다. 기존 reviewing Case도 비교 버튼을
열면 같은 경로로 준비되므로 별도 DB 백필은 없다.

노출은 P5 정책 변경을 따른다. ES에는 이름을 넣지 않고 단일검색 응답을 만들 때 현재 원장의
활성 행과 승인·`part_sale` 조직을 DB에서 일괄 조인해 `partnerStock.partnerNames`로만 내린다.
이름 변경·정지는 재색인 없이 다음 조회부터 반영한다. 조직 ID·연락처·협력사별 재고는 보내지 않고,
검색 바구니와 BOM 견적 상세는 특정 협력사를 선택한 상태가 아니므로 기존 `협력사 보유` 표기를 유지한다.
...
| 단일검색 Distributor 협력사명 | **노출**(`sp_partner.name`, 승인+`part_sale`+활성 원장) | 노출 |
| BOM 분석/견적 상세 조직 이름 | **비노출**(`toItemDto`가 `partnerIds` 제거) | 협력사 보유 부품 화면에서 품번으로 조회 |
| 조직 ID·연락처·협력사별 정확 재고 | **비노출** | 노출 |
...
## 8.5 고객 여정 — 관리자 토큰으로는 안 보이던 자리 (2026-08-24)

여기까지의 검증이 전부 **관리자 토큰**이었다. 이번 기능의 결함이 전부 *경계*에서 나왔으므로
(왕복·화면·정리 경로) 남은 경계 셋을 한 편으로 묶어 태웠다 —
`e2e/specs/journey-partner-parts-customer.e2e.test.ts`.

**① 고객 세션 노출 (P5, 2026-08-27 정책 변경 반영).** 실제 고객 계정 브라우저로 단일검색 →
담기 → 카트까지 간다. 단일검색 Distributor와 API의 `partnerStock.partnerNames`에는 현재 승인
협력사명이 보이고, 카트·BOM 견적 상세는 아직 특정 협력사를 선정한 상태가 아니므로 일반
`협력사 보유`만 유지한다. API에는 조직 ID·연락처·협력사별 재고가 없는지도 함께 확인한다.

**② 주문 게이트 — 유지.** 금액 없는 협력사 행이 담긴 견적으로 주문을 시도하면 확정 전에는
409 다. 견적요청을 넣은 뒤에도 막힌다 — 게이트는 **라인 단위가 아니라 관리자 확정가**
(`status='answered' ∧ confirmedTotal !== null`)이기 때문이다. 0원 행이 조용히 합계에 섞이는
경로는 없고, 확정가를 넣는 사람에게는 그 행의 금액이 비어 있는 것이 보인다.

**③ 동시 반영 — 통과, 다만 코드 갭을 닫았다.** 두 협력사가 겹치는 61 품번을 동시에 반영해도
부품은 하나·구매 조건은 둘이다. 다만 일괄 경로에 **조용한 건너뜀**이 있었다 — 다른 협력사의
미커밋 생성 때문에 재조회가 놓치면 그 행의 구매 조건이 소리 없이 빠졌다. 이제 개별 확보로
떨어지고, `ensureCatalogPart` 는 유니크 충돌을 '남이 먼저 만들었다'로 읽어 다시 조회한다
(인제스트 `upsertWithRaceRecovery` 와 같은 결).

⚠ 같은 편에서 드러난 **비대칭 하나**: 단일검색으로 **수동 담기한 행에는 `matchEvidence` 가
없다**(그건 엔진 검색 투영이 만든다). 관리자 Case 화면의 `협력사 보유 · 이름` 칩은 evidence 가
아니라 `/admin/bom-quotes/:id/partner-stock` 을 품번으로 조회하므로 **여전히 보인다** —
그래서 결함은 아니다. 다만 같은 행이 출처에 따라 근거를 달리 갖는다는 점은 알고 있어야 한다.

## 8.6 주문 완주 여정 (2026-08-24)

앞 여정은 "확정 전에는 막힌다"까지였다. 이제 **열리는 것**을 본다 —
`e2e/specs/journey-partner-parts-order.e2e.test.ts`.

가격 있는 실공급사 부품과 가격 없는 협력사 부품을 **한 카트에 섞어** 담고, 견적요청 →
관리자 검토 → 품목 확인 → 확정가 → 고객 주문까지 한 줄로 간다. 섞어서 보는 이유는
협력사 행만 있으면 "금액이 없으니 막힌다"만 확인되기 때문이다. 진짜 질문은 **섞였을 때
실공급사 금액은 살고 협력사 행은 0 으로 남는가**이고, 그 상태로 주문이 서는가다.

결과: 선다. 담긴 두 행 중 실공급사 행만 `lineTotalKrw` 가 있고 협력사 행은 null,
장바구니에 실린 금액은 **관리자 확정가의 부가세 포함액**이다 — 0원 행이 합계에 섞이지 않는다.
재주문해도 같은 행을 갱신한다(중복 담기 없음).

⚠ 이 편에서 배운 함정 셋:
- 서명 토큰에는 `cartId` 클레임이 없어 주문이 `NO_CART_ID` 로 먼저 끊긴다 — 화면이 쓰는
  `/spcb/api/me` 세션→JWT 브리지로 진짜 토큰을 얻어야 무엇이 막았는지 흐려지지 않는다(여정 10호).
- **금액은 `ct_price` 가 아니라 `io_price`(옵션가)** 에 실린다. 본체가는 관례상 0 이라
  `ct_price` 를 보면 "금액이 안 실렸다"로 오독한다.
- 템플릿 상품 id 는 `sp-bom-parts` 다. 정리 쿼리를 `it_id LIKE 'bom%'` 로 짚으면 **아무것도
  안 지워지고 실제 장바구니 행이 남는다**(실제로 두 번 남겼다). 견적별 `io_id='bom-{quoteId}'` 로 짚는다.

### ⚠ 전체 스위트에서만 터진 동시성 결함

여정 스펙을 **단독으로** 돌리면 통과했는데, 협력사 스펙 전체를 함께 돌리자
`P2034 Transaction failed due to a write conflict or a deadlock` 로 반영이 500 이 났다.
두 협력사가 겹치는 신규 품번을 동시에 만들면 유니크 인덱스(`mpnNorm, manufacturerNorm`)
갭 락에서 교착이 난다. 유니크 충돌(P2002)은 `ensureCatalogPart` 가 '남이 먼저 만들었다'로
흡수하지만 **교착은 흡수할 수 없다** — 트랜잭션 자체가 죽으므로 다시 시도하는 것만이 답이다.
교정: `withWriteRetry`(지수 백오프 4회, 판정은 인제스트의 `isTransientPartIngestError` 재사용)
로 부품·구매 조건의 일괄/개별 쓰기를 감쌌다.

**교훈**: 한 번 통과했다고 경합이 없는 게 아니다. 동시성은 **부하가 있을 때만** 드러나므로
단독 실행 결과를 근거로 삼으면 안 된다.

## 8.7 BOM 분석 경로로 주문 완주 (2026-08-24)

단일검색 담기(§8.6)와 **BOM 업로드**는 협력사 부품이 견적에 오르는 기계가 다르다 —
전자는 카탈로그 `partner_stock` 담기, 후자는 검색 잡의 `local_products` 주입이다. 그래서
같은 질문을 BOM 경로로 다시 물었다: `e2e/specs/journey-partner-parts-bom-order.e2e.test.ts`.

실물 EUREKA 품번으로 만든 혼합 BOM(흔한 부품·협력사만 가진 단종품·아무도 없는 품번)을
실계정으로 올려 검색을 끝내고, 고객 화면 → 견적요청 → 관리자 확정 → 주문까지 갔다.

| 행 | 근거 | 금액 |
|---|---|---|
| STM32F030F4P6 (실공급사) | 자동 선정 | 있음 |
| 88PW886·ADUC7020 (협력사만) | `partnerStock` 붙음, `partnerIds: []` | **없음** |
| ZZ9-NOSUCHPART (아무도 없음) | 없음 | 없음 |

장바구니 금액은 관리자 확정가 ×1.1 — 협력사 0원 행이 합계에 섞이지 않는다. 고객 견적 상세
화면 본문에 협력사 이름이 없고, 같은 견적의 관리자 `partner-stock` 조회에는 이름이 있다.
협력사 스펙 7본 38케이스를 함께 돌려도 통과한다(§8.6 의 교착 재시도가 부하에서 버틴다).
