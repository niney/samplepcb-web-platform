...
## 6-D. migrate:sync 사용법 (운영 직결 쿡북)

**1) 접속 설정** — `.env.migration`의 `LEGACY_DATABASE_URL`만 운영으로 교체(나머지 불변):

```bash
# 운영 직결(읽기 전용 — legacy-db.ts 가 SELECT 외 거부). 비밀번호 특수문자는 URL 인코딩(! → %21 등)
LEGACY_DATABASE_URL="mysql://hyoh9150:<비밀번호>@www.samplepcb.co.kr:3306/hyoh9150"
# 로컬 폴백(덤프 임포트본) — 직결 불가 시 이 줄로 전환
#LEGACY_DATABASE_URL="mysql://root@127.0.0.1:3306/samplepcb_legacy_full"
```

JDBC 스타일 쿼리 파라미터(useSSL 등)는 불필요(`?` 뒤 무시, zero-date 는 dateStrings 처리).
실파일은 gitignore — 커밋 금지. 형식 참고는 `.env.migration.example`.

**2) 명령**

```bash
pnpm migrate:sync -- --dry-run    # 미리보기(타깃 무변경) — 델타 규모 확인. (a)/(b)가 같은 신규 라인을
                                  #   중복 카운트할 수 있음(실행 모드에선 (a)가 먼저 삽입하므로 한 번만 처리)
pnpm migrate:sync                 # 실제 반영. 기본 --window=90(주문 재대조 최근 생성 창 — 비종결·지문
                                  #   상이는 창과 무관하게 항상 후보)
pnpm migrate:sync -- --final      # 컷오버 마지막 1회 전용: 노이즈 컬럼(mb_today_login·mb_login_ip)까지 반영
pnpm migrate:verify               # 전량 검증(--light = 행수+금액 항등만)
```

평소엔 원할 때 `migrate:sync` 한 번(반복 안전 — 무변경이면 no-op). 매회 게이트가 먼저 돌므로
운영 스키마가 변하면(컬럼 추가 등) 위반으로 중단된다 → §4 요령대로 처분 확정 후 재실행.
...
**4) 운영 직결 실증(2026-07-07, 20260702 덤프 기준 닷새치)**: 게이트 드리프트 0 · 신규 주문 43/
라인 57/spec·quote 56/회원 19/포인트 125행/1:1 1/글 2 · 재대조 갱신 헤더 109/라인 123/**사양
드리프트 12**(운영 가격확인 실발생)/회원 62/프로필 8(회사명 덮음 2 리포트) · 갱신분 금액 항등
109/109 · verify 전량 그린 · 재실행 no-op · 1회 소요 2~3분(원격 왕복 포함).
...
- **admin 회원 상세 400 (2026-07-07)**: 계약 `MbIdParams` 가 표준 아이디 가정(max 20자)이라
  **20자 초과 이메일 아이디 838명**의 상세/편집 API 가 검증 단계에서 400. 전 회원(6,246) 스윕으로
  실측 후 max(191)(sp 한도 정합)로 완화 — 스윕 재실행 0 실패·드로어 실브라우저 실증.
  교훈: "mb_id ≤ 20자" 가정은 코어 필터·컬럼 폭·계약 3층에 숨어 있었다(§8 로그인 건과 동일 뿌리).
...
| 스키마 | `prisma/migrations/20260707010000_legacy_migration_profile_bizinfo` · `20260707040000_widen_mbid_mediumtext` · `20260707120000_widen_profile_bizzip` |
| env | `.env.migration.example` (실파일 `.env.migration`은 gitignore — 운영 직결 URL·비번 포함) |
