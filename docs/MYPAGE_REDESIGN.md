...
## 공용 사이드바 갱신 (2026-09-15 — Figma 2243:2386)

현재 사이드바 기준은 카드형이다. 포인트 전용 `.is-point` 덮어쓰기를 제거하고 홈·주문·견적·
제조확인·A/S·포인트 등 공용 메뉴 전체에 적용한다. 아래 08-25 기록의 평면 레이아웃과 활성
메뉴 돌출 스타일은 이 기준으로 대체한다.

- 레이아웃: 사이드바 211px + 간격 29px, 테두리 `#d9d9d9` 1px, 모서리 12px.
- 사용자 요청으로 Figma의 불균일한 메뉴 간격을 통일: 행 최소 높이 36px(한 줄), 행 사이 4px,
  그룹 제목 아래 12px, 프로필 아래·그룹 사이 32px. 선택 여부에 따라 크기·여백이 바뀌지 않는다.
- 960px 이하에서 상단 3열, 640px 이하에서 1열로 전환하며 같은 간격 규칙을 유지한다.
- `G5_CSS_VER` 26091501로 캐시 갱신. 메뉴명·링크·배지 데이터는 기존 공용 파일을 유지한다.
- 검증: 로컬 로그인 브라우저의 홈·포인트·견적관리에서 폭 211px·메뉴 8개 높이 36px 확인.
  실제 사이드바 HTML을 별도 렌더링한 1440/1024/768/390px 검사에서 메뉴 높이 동일·가로 넘침 0.
  포인트 실제 화면·390px 사이드바 캡처 육안 확인, 변경 PHP 2파일 문법 검사 통과.
