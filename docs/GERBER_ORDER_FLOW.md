...
| `g5_shop_cart` | 영카트 코어 | sp-node 는 **INSERT · 파생 SELECT · ct_select UPDATE(주문 선택 ④) · 견적 행 UPDATE(재견적 동기화 io_id/io_price/ct_option ⑥) · 견적 행 DELETE(ct_id 단위 빼기 ⑥) · ⑬ 상태 전이 ct_status(change_status 미러)·재고 loop ct_stock_use · ⑮ 카트행 취소/반품/품절(orderformcartupdate.php 이식 — ct_status=취소류·ct_stock_use=0·ct_point_use=0·ct_history CONCAT, 원자 가드 WHERE ct_id AND ct_status; `setOrderItemsStatus`) · ⑯ 임의 상태 변경(orderformcartupdate 정상 분기 — 주문 라인(쇼핑/삭제 제외, **취소류 포함** = un-cancel) ct_status=target·ct_history CONCAT, claim-first 원자 가드; `setOrderForceStatus`) · ⑰ 제작 8단계 전이(⑯ force-status 확장 — ct_status=제작단계(가격확인~A/S), `FORCE_STATUS_LINE_IN` 에 제작 8 포함, 무부수효과) · ⑲ 재능마켓 계약 결제(2026-07-08) — 계약 카트행 INSERT(①insertCartRow 재사용, io_id=contractKey)·멱등 청소 DELETE(`deleteCartRowsByIoId`, '쇼핑' 행만)·버킷 판정 read(`getCartRowByCtId`)** (한정 예외, `lib/g5-db.ts` 에 명시) |
| `g5_shop_item_option` (견적 옵션 행) | 영카트 코어 | sp-node 는 견적 옵션 행(io_id=quoteId) **INSERT + 보상 DELETE** (한정 예외 확장, 기법 #3) · ⑬ 배송 전이 재고차감(order_update_delivery 미러 — io_stock_qty-=ct_qty, per-quote 옵션 행이라 9999999 버퍼 감소, 멱등 ct_stock_use 가드) · ⑮ 취소/반품/품절 재고 **복원**(add_io_stock 미러 — io_stock_qty+=ct_qty, ct_stock_use=1 행만) · ⑯ 임의 전이 재고 가감(adjustStock — 배송/완료 진입 -ct_qty·주문 역방향 +ct_qty) · ⑲ 재능마켓 계약 옵션 행(io_id=contractKey) 동일 INSERT/보상·정리 DELETE |
