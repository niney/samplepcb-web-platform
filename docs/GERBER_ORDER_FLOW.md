...
| `g5_shop_cart` | 영카트 코어 | sp-node 는 **INSERT · 파생 SELECT · ct_select UPDATE(주문 선택 ④) · 견적 행 UPDATE(재견적 동기화 io_id/io_price/ct_option ⑥) · 견적 행 DELETE(ct_id 단위 빼기 ⑥) · ⑬ 상태 전이 ct_status(change_status 미러)·재고 loop ct_stock_use · ⑮ 카트행 취소/반품/품절(orderformcartupdate.php 이식 — ct_status=취소류·ct_stock_use=0·ct_point_use=0·ct_history CONCAT, 원자 가드 WHERE ct_id AND ct_status; `setOrderItemsStatus`) · ⑯ 임의 상태 변경(orderformcartupdate 정상 분기 — 주문 라인(쇼핑/삭제 제외, **취소류 포함** = un-cancel) ct_status=target·ct_history CONCAT, claim-first 원자 가드; `setOrderForceStatus`)** (한정 예외, `lib/g5-db.ts` 에 명시) |
