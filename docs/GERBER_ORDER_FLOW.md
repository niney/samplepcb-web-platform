...
> 갱신 2026-07-05 · **주문 상세 편집 + 입금 조정 + 인쇄**(orderformupdate.php·orderformreceiptupdate.php 이식). ⑭ 신설(updateOrderInfo 화이트리스트·updateOrderShopMemo·updateOrderReceipt 무통장 3필드+미수금 재계산). 인쇄 GET .../print = 상세(⑫)+발신처(⑦) 조합. 입금 조정은 무통장·3필드로 좁혀 이식(코어의 배송/에스크로/재고/상태전이/메일 부수효과는 스코프 밖 — WP3 전이가 담당)
> 갱신 2026-07-05 · **카트행 단위 취소/반품/품절**(orderformcartupdate.php 이식, 무통장 한정). ⑮ 신설(setOrderItemsStatus — ct 단위, 재고 복원 add_io_stock·ct_history·전량취소→od_status='취소'·it_sum_qty·미수금/취소금액 재계산 recomputeOrderMoneyOnItemChange). 무통장 409 로 PG 부분취소 제외. 포인트 복원 no-op(PCB ct_point=0, ct_point>0 은 HAS_POINT skip)·send_cost/쿠폰 저장값 재사용(WP3 갭 동일). sp 견적행 무접촉
> 갱신 2026-07-05 · **주문 임의 상태 변경**(orderformcartupdate.php 정상 상태 분기 이식, ⑮ 취소류의 짝). ⑯ 신설(setOrderForceStatus — 주문 라인(쇼핑/삭제 제외, **취소류 포함** — 취소류→정상 = un-cancel) ct_status=target+od_status=target, 역방향 포함). 스톡 앵커 resolveForceStatusStock: 배송/완료 진입 차감·주문 역방향/취소 복귀 무차감. 코어 정상 분기에 결제수단 가드·운송장 요구·od_mod_history append **없음**(스톡이 안 하는 부수효과 미창작). ct_point>0 은 409 HAS_POINT
> 갱신 2026-07-05 · **PCB 제작 8단계 주문 상태 도입**(가격확인·파일검사·EQ·생산시작·생산중·품질시험·생산완료·A/S — 레거시 lib/common.lib.php `get_order_status_list` 이식). '준비'와 '배송' 사이 커스텀 상태로 **od_status/ct_status 재사용**(신규 컬럼·Prisma 마이그레이션 없음 — 공유 DB reset 제약 회피). 관리자 전환은 **force-status(⑯) 확장**으로만(선형 전이 체인 무변경) — 제작 단계는 무부수효과(재고차감·송장은 '배송' 진입에서만, `resolveForceStatusStock` 자동 none). ⑰ 신설(**`ACTIVE_ORDER_STATUSES` 상수 SSOT** — 정상합계/부분취소/카운트 IN절 리터럴 6곳 통합; g5-db `OrderTab`·`OrderForceStatusTarget`·`FORCE_STATUS_LINE_IN`·counts SQL + 계약 `AdminOrderTab`/`AdminOrderCounts`/force target enum + FE 탭16·배지 variant·i18n ko/en). 고객 주문조회(sp-lite/shop/orderinquiry.sub.php) 배지 switch 에 제작단계 case(default '주문취소' 오작동 방어, `status_03` 재사용·CSS 무변경·테마 오버라이드라 코어 무수정). HANDOFF:72 "제작 상태 나중 고려(기록만)" 해소. 알림 미포함(force-status 무알림 — 필요 시 order-notify 후속). typecheck·135 test·lint 통과, e2e 실구동 미검증
> 갱신 2026-07-05 · **관리자 설정 > 사업자정보**(영카트 쇼핑몰설정 사업자정보 11필드 이식 — g5_shop_default de_admin_*). 카탈로그 ⑱ 신설(getBusinessInfo/updateBusinessInfo — 11컬럼만 UPDATE, 코어 ~150컬럼 일괄 대비 무관 설정 미훼손). 저장 규칙 3종(check_vaild_callback·대표자명 공백 가드·clean_xss_tags) → `lib/shop-config.ts`(isValidCallback·cleanXssTags 순수함수 + 단위테스트) + `routes/admin-settings.ts`. 사이드바 "설정" 1-depth + 페이지 내부 탭(첫 탭 사업자정보, 결제/배송/알림 확장 뼈대). 번호 ⑰은 병존 미커밋 PCB 제작단계 작업이 ACTIVE_ORDER_STATUSES SSOT 로 선점 → 사업자정보는 ⑱
> 갱신 2026-07-05 · **제작 상태 선형 전이로 개정**(위 force-status-only 방침 대체 — 리뷰/실사용 피드백 반영). 제작 7단계(가격확인~생산완료)를 **정방향 선형 체인에 편입** — '다음 단계 처리' 버튼이 준비→가격확인→…→생산완료→배송→완료 순차(g5-db `TRANSITION_REQUIRED_STATUS`·`setOrdersStage`, 계약 `AdminOrderStatusRequest.target`, FE `TAB_ACTION`/`NEXT_ACTION`). 배송 실무(운송장·재고차감·배송알림)는 '준비'→'생산완료'로 이동(배송 전이 from, 운송장 입력 탭 `OrdersTable`/`AdminOrders`, `getDeliveryExcelRows` 동기). **A/S 는 선형 제외 force-status 전용**. 요청 "준비 처리→가격확인 순차" + 배송알림 소실(force 무알림) 동시 해결(생산완료→배송이 선형이라 알림 브리지 자동). 관리자 드로어 **상태 스텝퍼**(`OrderStatusStepper` + `ORDER_PIPELINE`) 추가. F3(완료→A/S it_sum_qty)는 이번 스코프 제외(레거시 동작 유지). typecheck·136 test·lint 통과.
> 갱신 2026-08-09 · **PCB·SmartBOM 관리자 Case 고객 정보 통일** — 주문 전에는 견적 신청 회원(g5_member)+회사명 프로필/문서 스냅샷을 표시하고, 주문 뒤에는 영카트 주문 당시 `od_name`·`od_email`·`od_hp`/`od_tel`·`mb_id`를 우선한다. `getOrdererContactByOdId` 최소 read-only 조회를 카탈로그 ⑩에 추가하고 `sp_order_biz_info.companyName`을 주문 회사명 1순위로 사용한다. DB 마이그레이션 없음.
> 갱신 2026-08-10 · **SmartBOM 고객 배송 수신처·메일 정합화** — 배송 워크큐가 주문 시점 `od_name`·`od_email`과 `od_b_name`·`od_b_tel`/`od_b_hp`·`od_b_zip1/2`·`od_b_addr1~3`를 `getOrderHeadersLite` 배치 조회로 표시한다. `force-status` 배송 진입은 명시적 `sendMail` 선택 시 기존 PHP 주문 알림 브리지/영카트 메일 양식을 재사용하고 발송 결과를 `sp_mail_log`에 남긴다. 신규 DB 마이그레이션 없음.
> 갱신 2026-08-09 · **PCB 주문·결제 취소** — `/app/admin/pcb/orders`와 PCB Case에서 공용 취소 미리보기·실행 모달 제공. 영카트 `orderformcartupdate.php`의 ⑮ 카트행 취소를 재사용하되 미입금(`od_status='주문'`)·수납액 0·무통장·협력사 발주 0건만 즉시 허용한다. 실행 시 주문 헤더를 `FOR UPDATE` 잠그고 같은 조건을 재검사한 뒤 카트행·금액·헤더·변경이력을 한 트랜잭션으로 반영하며 취소 사유를 `od_mod_history`에 남긴다. 입금/PG는 로컬 상태를 먼저 바꾸지 않고 원본 영카트 승인취소·환불 화면으로 위임한다. 부분취소 행은 PCB 취소 탭에 따로 집계하고 해당 스펙의 후속 RFQ·발주도 라인 상태로 차단한다. DB 마이그레이션 없음.
> 갱신 2026-08-12 · **다건 PCB 주문·결제 단위 정합화** — 같은 `od_id`의 PCB 여러 줄을 주문 1건으로 카운트·페이지네이션하고 한 페이지에서 함께 펼친다. 입금확인·주문 결제 블록은 주문당 1회, PCB 행은 `io_type`·수량을 반영한 줄 금액을 표시한다. 계약에 주문 결제대상·현금수납·포인트·환불·순결제·미수·동반 PCB 수를 명시하고 Case도 `이 PCB 주문금액`과 `주문 전체`를 분리한다. PHP 주문 저장은 일반 상품 `distinct it_id` + 커스텀 스냅샷 `ct_id` 건수로 `od_cart_count`를 기록하고, 고객·통합 관리자 주문목록은 같은 집계로 기존 오저장 주문도 표시 시 보정한다. 고객 목록 결제액은 수납+포인트-환불 순액, 상세 완불 판정은 저장된 `od_misu`를 정본으로 사용한다. DB 마이그레이션·신규 쓰기 경로 없음. 실제 PHP 주문→동시 입금→부분취소·환불 E2E 3회 통과.
...
| `g5_shop_cart` | 영카트 코어 | sp-node 는 **INSERT · 파생 SELECT · ct_select UPDATE(주문 선택 ④) · 견적 행 UPDATE(재견적 동기화 io_id/io_price/ct_option ⑥) · 견적 행 DELETE(ct_id 단위 빼기 ⑥) · ⑬ 상태 전이 ct_status(change_status 미러)·재고 loop ct_stock_use · ⑮ 카트행 취소/반품/품절(orderformcartupdate.php 이식 — ct_status=취소류·ct_stock_use=0·ct_point_use=0·ct_history CONCAT, 원자 가드 WHERE ct_id AND ct_status; `setOrderItemsStatus`) · ⑯ 임의 상태 변경(orderformcartupdate 정상 분기 — 주문 라인(쇼핑/삭제 제외, **취소류 포함** = un-cancel) ct_status=target·ct_history CONCAT, claim-first 원자 가드; `setOrderForceStatus`) · ⑰ 제작 8단계 전이(⑯ force-status 확장 — ct_status=제작단계(가격확인~A/S), `FORCE_STATUS_LINE_IN` 에 제작 8 포함, 무부수효과) · ⑲ 재능마켓 계약 결제(2026-07-08) — 계약 카트행 INSERT(①insertCartRow 재사용, io_id=contractKey)·멱등 청소 DELETE(`deleteCartRowsByIoId`, '쇼핑' 행만)·버킷 판정 read(`getCartRowByCtId`) · ⑪ SmartBOM 배타 결제 주문 강제삭제 — 프리뷰의 ct_id/it_id/io_id/ct_status 일치+형제 0을 `FOR UPDATE`로 재검증한 정확한 행만 물리 DELETE** (한정 예외, `lib/g5-db.ts` 에 명시) |
| `g5_shop_item_option` (견적 옵션 행) | 영카트 코어 | sp-node 는 견적 옵션 행(io_id=quoteId) **INSERT + 보상 DELETE** (한정 예외 확장, 기법 #3) · ⑬ 배송 전이 재고차감(order_update_delivery 미러 — io_stock_qty-=ct_qty, per-quote 옵션 행이라 9999999 버퍼 감소, 멱등 ct_stock_use 가드) · ⑮ 취소/반품/품절 재고 **복원**(add_io_stock 미러 — io_stock_qty+=ct_qty, ct_stock_use=1 행만) · ⑯ 임의 전이 재고 가감(adjustStock — 배송/완료 진입 -ct_qty·주문 역방향 +ct_qty) · ⑲ 재능마켓 계약 옵션 행(io_id=contractKey) 동일 INSERT/보상·정리 DELETE · ⑪ SmartBOM 결제 주문 강제삭제에서 ct_stock_use=1이면 io_stock_qty+=ct_qty 복원 후 옵션 정리 |
| `g5_shop_item` (템플릿) | 영카트 코어 | sp-node 는 SELECT(배송정책 스냅샷) + ⑬ **UPDATE**: 완료 전이 it_sum_qty(전 주문 '완료' SUM(ct_qty) 판매 통계) · 배송 전이 it_stock_qty 차감(cart 행에 io_id 없을 때만 — 견적 행은 옵션 행 경로) · ⑮ 취소류 it_stock_qty 복원(io_id 없을 때)·it_sum_qty 재계산 · ⑯ 임의 전이 it_stock_qty 가감(io_id 없을 때)·it_sum_qty 재계산(target 주문/완료만, 코어 조건 미러) · ⑲ 마켓 앵커 `sp-market-svc` read-only SELECT(`getMarketAnchorItem` — it_price=0·it_sc_type=1 무료배송 시드 `seed-market-anchor-item.ts`) · ⑪ SmartBOM 결제 주문 강제삭제에서 옵션 없는 차감 재고 복원 및 대상 상품 it_sum_qty 재계산 |
| `g5_shop_order` (주문 헤더) | 영카트 코어 | sp-node 는 **관리자 API 한정**. read-only SELECT: ⑩ 견적 완전삭제 프리뷰(주문됨 견적이 묶인 주문의 결제상태·수납액·PG거래·형제 cart 파악 — od_status·od_receipt_price·od_cart_price·od_settle_case·od_tno·od_pg·od_misu, `getOrderInfoByCtId`) + PCB 관리자 상세 주문 세액 표시(`getOrderHeadersLite`의 od_tax_mny·od_vat_mny·od_free_mny; 견적 역산값이 아닌 주문 저장값) + PCB·SmartBOM Case 주문자 표시(`getOrdererContactByOdId`의 mb_id·od_name·od_email·od_hp·od_tel, 주문 당시 스냅샷) + SmartBOM 배송 워크큐 수신처 표시(`getOrderHeadersLite`의 od_name·od_email·od_tel/od_hp·od_b_name·od_b_tel/od_b_hp·od_b_zip1/2·od_b_addr1~3) · ⑫ 관리자 주문내역(adm/shop_admin/orderlist.php 이식 — 목록/상세/배타 counts/누적주문수, 컬럼 화이트리스트, **민감 컬럼 od_pwd·od_cash·od_cash_info 는 SELECT 자체 배제**, 날짜는 KST native 문자열, WHERE 는 파라미터 바인딩(qField·정렬 컬럼은 화이트리스트 상수), `searchOrders`/`getOrderRow`/`getCartRowsByOdId`/`getMemberOrderCounts`). **UPDATE**: ⑬ 관리자 상태 전이(adm/shop_admin/orderlistupdate.php 이식 — 주문→입금(무통장, od_receipt_price/od_misu/od_receipt_time)→준비→배송(od_delivery_company/od_invoice/od_invoice_time)→완료, 원자 가드 WHERE od_id AND od_status. 전이 후 미수금·과세 재계산 od_misu/od_tax_mny/od_vat_mny/od_free_mny/od_send_cost(get_order_info 산식 미러 — send_cost·쿠폰은 상태 전이 불변이라 저장값 재사용, get_sendcost/쿠폰테이블 포트 미이식 갭). od 단위 독립 처리 processed/skipped. `setOrdersReceipt`/`setOrdersPreparing`/`setOrdersDelivery`/`setOrdersComplete`). **DELETE**: ⑪ 미입금 주문 완전삭제(od_status='주문'만 — 백업(serialize)→cart ct_status='삭제'→order DELETE, 코어 orderlistdelete.php 이식, `deleteUnpaidOrder`/`deleteOrders` 배치). ⑭ 주문 상세 편집(adm/shop_admin/orderformupdate.php·orderformreceiptupdate.php 이식): `updateOrderInfo`(주문자/받는분/배송지/희망일 화이트리스트 동적 SET — jibeon 패스스루, 상태 무관)·`updateOrderShopMemo`(od_shop_memo 평문)·`updateOrderReceipt`(무통장 입금 조정 od_receipt_price/od_receipt_time/od_deposit_name — 원자 가드 WHERE od_settle_case='무통장' + recomputeOrderMoney; 코어 receiptupdate 의 배송/에스크로/재고/상태전이/메일 부수효과는 스코프 밖). ⑮ 카트행 취소/반품/품절 헤더 반영(orderformcartupdate.php 이식 — od_cart_price/od_cart_coupon/od_cancel_price/od_send_cost/od_misu/od_tax_mny/od_vat_mny/od_free_mny 재계산(활성·취소류 카트 집계, **send_cost·od_coupon/od_send_coupon 저장값 재사용** WP3 갭 동일)·전량 취소류면 od_status='취소'+od_mod_history CONCAT. **recomputeOrderMoneyOnItemChange**(WP3 전이 경로 recomputeOrderMoney 와 별개), `setOrderItemsStatus`). ⑯ 임의 상태 변경(orderformcartupdate 정상 분기 — od_status=target·recomputeOrderMoneyOnItemChange 재사용·target='배송'+delivery 시 od_delivery_company/od_invoice/od_invoice_time. **결제수단 가드·od_mod_history append 없음** — 스톡 앵커만 미러. `setOrderForceStatus`; SmartBOM/PCB 배송 화면의 명시적 `sendMail`은 기존 PHP 주문 알림 브리지로 배송 메일을 발송하고 `sp_mail_log`에 기록). 메일/SMS 는 sp-node 가 아니라 **PHP 브리지 재사용**(order-notify.php, 아래 8장). ⑲ 재능마켓 계약 paid 승격 라인 검증 read-only — `getOrderInfoByCtId` additive 확장(자기 카트행 rowCtStatus/rowIoId/rowIoPrice)+`PAID_ORDER_STATUSES` export(ACTIVE−'주문' — '부분취소'는 od_status 값이 아니라 행 단위라 라인 검증이 정본). ⑰ **PCB 제작 8단계 주문 상태**(가격확인·파일검사·EQ·생산시작·생산중·품질시험·생산완료·A/S — 레거시 lib/common.lib.php `get_order_status_list` 이식). od_status=제작단계(신규 컬럼 없음), **선형 전이 편입**(setOrdersStage — 준비→가격확인→…→생산완료→배송→완료)+A/S 는 force-status 전용. 재고차감·운송장·배송알림은 '생산완료→배송' 진입에서만. **`ACTIVE_ORDER_STATUSES`(표준5+제작8) 상수 = 정상합계/부분취소/counts IN절 SSOT**(리터럴 6곳 통합). `OrderTab`/`OrderForceStatusTarget`·counts SQL + 계약 `AdminOrderTab`/`AdminOrderCounts`/force target + FE 탭16·배지·i18n·sp-lite 고객 배지 매핑. `lib/g5-db.ts` |
| `g5_shop_order` SmartBOM 배타 삭제 확장 | 영카트 코어 | ⑪의 일반 관리자 주문 삭제는 계속 `od_status='주문'`만 허용한다. 별도 2차 `forceDeletePaidOrder=true`가 있고 `PAID_ORDER` 외 차단이 없는 SmartBOM 단일 cart 주문에 한해서만 결제 헤더도 로컬 물리 DELETE한다. 주문/cart/보조 원장/포인트/재고를 한 InnoDB 트랜잭션으로 처리하며 외부 PG 승인 취소·환불은 호출하지 않는다. audited는 최신 `g5_shop_order_delete` 1건, reset은 0건을 남긴다. `deleteUnpaidOrder`의 `deleteExclusiveCart`+`allowPaymentEvidence` 경로. |
| 주문 보조 원장 (`g5_shop_order_data`·`g5_shop_coupon_log`·`g5_shop_coupon`·`g5_shop_personalpay`·`g5_shop_order_post_log`·`g5_shop_inicis_log`·`g5_shop_order_delete`) | 영카트 코어 | ⑩ SmartBOM 삭제 프리뷰가 od_id/oid/de_key 직접 연결 건수를 read-only 집계한다. ⑪ 결제 강제삭제 트랜잭션이 해당 주문 연결 행을 DELETE한다. 외부 PG 원장은 대상이 아니다. |
...
> 지운다. 쇼핑 cart의 임시 `od_id`는 과거 주문번호와 충돌할 수 있으므로 주문 헤더 존재만으로
> 주문됨을 판정하지 않으며, checkout/연결변경으로 상태가 바뀌면 0행으로 중단한다. ⑩
> 공용 `getOrderInfoByCtId`는 안정적인 기본 주문정보만 읽고, 삭제 전용
> `getOrderDeletionInfoByCtId`가 수납 포인트·쿠폰과 주문 보조 원장/포인트/기존 백업 건수를
> 추가로 읽도록 분리했다. ⑪ `deleteUnpaidOrder`의 일반·관리자 배치 경로는 이전처럼
> `od_status='주문'` + 현금 수납·포인트 사용·쿠폰 적용 0 + `od_tno=''`만 삭제하고 cart를
> soft-delete한다. SmartBOM 배타 하드삭제는 선택적 `exclusiveCart`(ct/상품/옵션/**행 상태**)를
> 받아 주문과 전체 cart를 `FOR UPDATE`로 잠그고 형제 0과 프리뷰 상태 일치를 다시 확인한다.
> 기본은 결제 흔적에서 롤백하며, 2차 경고의 `forceDeletePaidOrder`가 있을 때만 로컬 결제 주문과
> 보조 원장·포인트를 함께 지운다. 차감 재고와 판매수량을 복구하고, audited는 최신 주문 백업
> 1건, reset은 백업 0건을 남긴다. 외부 PG 승인취소/환불은 이 경로의 스코프 밖이다.
...
| 관리자 주문내역 API (읽기+쓰기) | `…/apps/api/src/routes/admin-orders.ts` (가드: `requireAdmin` · g5 접근: `g5-db.ts` 예외 ⑫ 읽기·⑬ 상태 전이/삭제·⑭ 상세 편집 · 순수 로직 테스트 `g5-db.test.ts`·`order-edit.test.ts`: WHERE 빌더·`computeOrderMoney`·`orderTransitionGuard`·`matchDeliveryRows`·`phpRound`·`resolveOrderSort`·`orderInfoBodyToFields`·`resolveItemCancelSkip`·`resolveForceStatusStock`·바디 refine). 엑셀 배송: `GET/POST /orders/delivery-excel`(업로드 파일 파트명 `file` 고정). 상세 편집: `PATCH /orders/:odId/info·/memo·/receipt`(무통장 409), 인쇄 `GET /orders/:odId/print`(상세+발신처). 카트행 취소/반품/품절: `PATCH /orders/:odId/items/status`(무통장 409, ct 단위 processed/skipped, ⑮). 임의 상태 변경: `PATCH /orders/:odId/force-status`(target+delivery?, HAS_POINT 409, ⑯) |
| PCB 주문·결제 워크큐·취소 | 목록 `GET /pcb-orders`는 `od_id` 단위 카운트·페이지네이션 후 같은 주문의 PCB 줄을 펼친다. `g5-db.ts`가 카트 `io_type/io_price/ct_price/ct_qty` 줄 금액과 주문 `od_cart_price/od_cancel_price/쿠폰/배송비/od_receipt_price/od_receipt_point/od_refund_price/od_misu`를 읽어 주문 결제대상·순결제를 파생한다. 취소는 `GET /pcb-orders/:specId/cancel-preview`·`POST /pcb-orders/:specId/cancel`; `lib/pcb-order-cancel.ts`가 결제·발주 정책을 판정하고 ⑮를 `requireUnpaidBankTransfer`·`historyReason` 옵션으로 재사용한다. 화면은 `AdminPcbOrders.vue`·`AdminPcbCase.vue`와 공용 `PcbOrderCancelModal.vue`. PG/수납 주문은 `/adm/shop_admin/orderform.php`로 위임한다. |
| 주문 편집 매퍼(순수) | `…/apps/api/src/lib/order-edit.ts` (`orderInfoBodyToFields` — AdminOrderInfoBody→OrderInfoFields 화이트리스트, jibeon 패스스루) |
