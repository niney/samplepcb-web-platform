...
- **웹**: 선적·배송 워크큐에 '고객 배송 대기'·'고객 배송 중' 탭 — 발송 대기 탭이 발주서
  축인 것과 같은 이유로 여기서는 **주문 축**으로 모수를 바꾼다(입고확인은 협력 축의
  종점이라 그 다음 일은 od 를 모수로 잡아야 보인다). 공용 `PcbCustomerShipModal`(미입고
  n/m 경고+confirm). Case 상세의 '배송 처리 대기' 배지(P3.8 — 보이게만 하던 신호)를
  **버튼으로 승격**해 같은 모달을 그 자리에서 연다. 배지 pcbShipmentPending = 발송 대기
  + 관리자 차례 + **고객 배송 대기** 3항 합산(D12).
