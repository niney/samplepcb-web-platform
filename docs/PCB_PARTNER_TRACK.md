...
### P4.1-b 구현 기록 (2026-08-25 — 고객 진입점: 마이페이지 "확인 요청 > 제조 확인")

**D16 의 구멍 하나를 막는다.** 확인 요청의 진입점이 **메일 딥링크뿐**이었다 — 메일을 놓친
고객은 주문을 하나씩 열어보기 전까지 자기 차례인 줄 모르고, 그동안 발주는 멈춰 선다.
고객용 조회 API 도 `odId` 필수라 "내 대기 목록"이라는 모수 자체가 없었다.

- **어휘 — "EQ" 를 화면에 쓰지 않는다.** 이 축은 **스텐실(메탈마스크) 발주도 그대로 쓴다**
  (고객확인 라우트·lib 어디에도 track 분기가 없다). 갈리는 건 말뿐이라, 메뉴는 트랙 중립어
  **"제조 확인"** 이고 상위 그룹이 **"확인 요청"** 이다. 화면에 EQ 를 하드코딩하면 스텐실
  고객에게 없는 단어가 나온다(계약 `pcbEqEventLabel` — 2026-08-17 확정 결함과 같은 유형).
  ⚠ 혼동 주의: 스텐실의 **"고객문의사항"은 이 축이 아니다**(협력사→관리자, PO 전이 note).
- **API**: `GET /api/pcb-eq-reviews/mine?scope=open|all` — 주문을 가로지르는 내 확인 요청.
  소유권은 `po.spec.mbId` **관계 필터**로 건다(회원의 spec 을 통째로 읽어 id 목록을 만들면
  이관분이 많은 회원에서 수천 행을 헛돈다). `openCount` 는 scope 와 무관하게 열린 것만 세며
  사이드바 배지와 같은 수다. `odId` 는 g5 영역이라 **라우트**가 붙인다(lib 은 sp_ 만).
  `requested` 가 곧 유효한 대기인 근거: EQ 전이가 열린 요청을 닫는다(`closeOpenEqReviews`).
- **화면**: `spcb/pages/eq.php`(`/shop/eq`, 루트 `.htaccess` 3번 규칙) — 견적관리와 같은 셸.
  기본 탭 **확인 대기**, `?scope=all` 이 이력. **승인·반려 폼은 두지 않는다** — 행은 주문
  상세 딥링크(`orderinquiryview.php?od_id=…#eq-{id}`, 메일이 쓰는 바로 그 링크)로 보낸다.
  결정 UI 를 복제하면 첨부·기한·확인 모달·"반려 후 새 파일 없음" 경고가 두 곳에서 갈린다.
- **사이드바**: `_account_nav.php` 에 그룹 신설. 배지 건수만은 **DB 직접 count**(브리지
  `sp_pcb_eq_open_count`) — 사이드바는 마이페이지·장바구니·포인트·쪽지 등 **모든 계정
  페이지**에서 렌더되므로 API 를 태우면 그 전부에 HTTP 왕복이 붙는다(주문내역 배지와 같은
  이유). 세기일 뿐 판정이 아니다 — 목록·결정은 여전히 sp-node 정본.
- **모바일**: 별도 대응 없음. 테마가 `G5_THEME_DEVICE='pc'` 라 코어가 `G5_IS_MOBILE=false`
  를 강제하고(모바일 분기 자체가 실행되지 않는다), 계정 셸은 960/640 폭 기반 반응형이다.
  iPhone 12(390px) 실측으로 사이드바 1열 전개·가로 오버플로 0 확인.
- **CSS**: `default_shop.css` 에 `.sp-eqm` 섹션 추가(주문 상세의 `#sod_fin .sp_eq_*` 와 시각
  문법만 공유하고 셀렉터는 나눈다). **`G5_CSS_VER` 를 26082501 로 올렸다**(§7 함정).
- **검증**: e2e `customer-eq-menu` **6/6**(모수 분리 · openCount · 소유권 격리 · 메뉴 2단·활성·
  배지 · **목록에 결정 폼 0개** · 딥링크가 승인 버튼에 착지 · 390px 가로 오버플로 0 ·
  다른 계정 페이지에서 같은 배지). api 961 · typecheck 통과.
...
### W3 후속 — 관리자 발주 UI의 협력사 선정 선행 안내 (2026-08-12)

서버의 `RFQ_NOT_SELECTED` 가드는 옳았지만 Case 화면이 그 규칙을 미리 설명하지 않았다. 발주
모달은 선정 행이 없어도 첫 협력사를 기본값으로 잡고, 가격이 있는 미선정 회신까지 승계 대상으로
인정해 서버 409까지 진행시켰다. 일반 관리자 발주 흐름을 서버 규칙과 다음처럼 맞췄다.

- 현재 회차에 선정된 협력사가 없으면 버튼을 `협력사 선정 필요`로 표시한다. 클릭은 막힌 버튼이
  아니라 안내 동작이다 — `?from=pos` 때문에 접힌 협력사 견적요청 섹션을 자동으로 펼치고,
  회신 완료·회신 대기·미배정 상태에 맞는 설명을 보여 준 뒤 `[선정]`·`[대리 회신]`·견적요청
  버튼 순으로 실제 다음 조치에 포커스를 옮긴다.
- 발주 모달의 협력사는 드롭다운이 아니라 **현재 회차 선정 행 읽기 전용**이다. 요청에도 해당
  `rfqId`를 반드시 보내므로 화면에서 미선정 회신이나 다른 협력사로 바꿔 발주할 수 없다.
- 모달을 연 뒤 다른 세션이 선정을 바꾸어 서버가 `RFQ_NOT_SELECTED`를 반환하는 경합도 RFQ를
  다시 조회하고 같은 선정 안내로 복귀한다. 전역 오류만 띄워 모달 뒤에서 놓치는 경로는 없앴다.
- `rfqId` 없이 조건을 직접 넣는 **백엔드 수동 발주 API는 유지**한다. 긴급·데이터 이관용 계약을
  없앤 것이 아니라, 협력사 회신을 보고 발주하는 표준 관리자 UI에서 선정 단계를 필수화한 것이다.

회귀: `pcb-po-selection-guide.e2e.test.ts`가 발주 워크큐 진입(`?from=pos`)에서 미선정 발주
클릭 → 모달 미노출 → RFQ 자동 펼침 → 문맥 안내 → `[선정]` 포커스 이동을 검증하고,
선정 뒤 모달에서는 협력사 드롭다운이 사라지고 선정 조직이 읽기 전용으로 고정되는 것까지 본다.
...
### P4.14 상태 표시 교정 — 관리자 상태 ↔ 고객 표기 실측 매트릭스 (2026-08-25)

관측 러너 `e2e/specs/status-matrix.e2e.test.ts`(`pnpm -F e2e e2e:status-matrix`, 시드 직삽입·
자동 정리)로 **관리자가 바꾼 상태가 고객에게 어떻게 보이는가**를 od 13종+취소 3종 · 협력 트랙
7단계 · BOM 견적 6상태 · BOM od 축 전수로 찍었다(`e2e/output/status-matrix.md`). PCB 에서 확정된
결함 9건을 **od 무접촉(D6 유지)** 원칙 그대로 표시 층에서 고쳤다.

- **두 축 단절(①)** — 협력 트랙이 입고까지 가도 고객 목록·줄 배지는 '입금완료', 관리자 드로어
  스텝퍼도 2/12 '입금'. → 진행 파생(lib/pcb-customer-progress)을 **목록·줄·드로어가 함께** 쓴다:
  `POST /api/pcb-progress/batch`(주문마다 가장 느린 줄) + PHP `sp_pcb_progress_batch()` → 목록
  배지 / 단건 응답에 `ctId`·`shortLabel` 추가 → 상세 줄 배지 / 관리자 상세 `items[].pcbProgress`
  → 드로어 줄 배지 + '협력 트랙 진행' 줄. 우선순위는 PHP SSOT `sp_order_status_customer($status,
  $progress)`: **결제 뒤·배송 전**(입금~생산완료)에서만 진행이 배지를 덮고, '주문'(미입금)은 돈이
  먼저라 '입금확인중' 그대로. od 는 여전히 안 바꾼다.
- **제작 8단계 원문+단색(②)** — 가격확인→'상품준비중'(레거시), EQ→'제조 확인(EQ)', A/S→'A/S 진행
  중', 나머지는 고객 어휘 유지 + 단계별 색 `status_03_1~5`(확인·생산·품질·완료·A/S).
- **A/S 파이프라인 밖(③)** — 드로어 다음 단계 `A/S → 배송`('A/S 재배송 처리', 운송장·알림) +
  스텝퍼 안내문 전용화. 상태 자체는 여전히 가지(ORDER_PIPELINE 밖).
- **반품·품절 → '주문취소' 뭉갬(④)** — 줄은 제 이름('반품'/'품절'), 헤더는 줄 집계
  `sp_order_status_customer_order()`(전량 반품이면 '반품' — 코어는 od_status='취소' 하나로 저장).
- **부분취소 목록 무흔적(⑤)** — 헤더 배지 옆 보조 배지 '일부 취소 n/m'.
- **od 배송 진입 시 카드 소멸(⑥)** — 카드 접힘 규칙(배송·완료·취소, 재점검 08-10 #4)은 **유지**.
  맥락은 줄·목록 배지가 잇는다(제조 확인→생산→입고→상품배송→배송완료가 한 배지 축).
- **EQ 3상태 1문구(⑦)** — `PCB_PROGRESS_STAGES` 에 `eq_pending`(issued)·`eq_done` 추가:
  '제조 확인(EQ) 준비 중' / '진행 중' / '확인 완료 — 생산 준비 중'(스텐실·직송 어휘 동형).
- **카드 단색(⑧)** — `sp_stage_<stage>` 단계색.
- **입금 안내 배너(⑨)** — od 입금 이후 미수 잔액은 "일부 확인 — 남은 금액" 문구(배지 '입금완료'와
  딴말 방지).

시드 규율: 관리자 API 는 od_id 를 Number 로 다뤄 **2^53(9.007e15) 초과면 반올림** — 실 od_id
(2026…)는 안전하나 e2e 합성 id 는 8-접두 16자리로.

같은 날 후속(P4.15): 진입점을 트랙 공용 `GET/POST /api/order-progress[/batch]` 로 넓혀 BOM 조달·물류
파생을 같은 배관에 태우고, 주문 상세에 **트랙별 스텝퍼**(PCB 8칸 `입금확인→입금완료→제조 확인→생산→
생산완료→입고 완료→상품배송→배송완료`)를 세웠다 — 정본은 `docs/SMARTBOM_PARTNER_RFQ.md` §6.36.
`/pcb-progress` 는 PCB 전용 그대로(기존 여정 호환).

### P4.16 고객 주문 상세 — Figma 103:4561 단일 컬럼 재편 (2026-08-26)

시안은 "무통장·입금완료·PCB 1줄" 한 장면만 그렸고 상태값은 예시라, **골격은 시안대로·상태는 P4.14/P4.15
사다리대로·기능은 빼지 않는다**(사용자 결정 14건 + 추가 체크 6건)로 재편했다. 코어 무수정 — 테마
`orderinquiryview.php`·`default_shop.css` `#sod_fin` 블록·`shop.head.php` 제목 조건·`extend/sp_order_status.extend.php`.

- **골격(시안대로)**: 제목 `상세주문내역` 36px → 머리줄(주문일자 `Y.m.d` — 시안 `2026.0801` 은 오타로 처리·주문번호
  파랑은 링크 아닌 강조) → 스텝퍼(90px 원 + 꺾쇠, **현재 칸만 검정·지난 칸도 회색**, 칸 수가 트랙마다 달라
  남는 폭은 균등 분배) → `주문 상품 N개` 표(헤더 무배경·검정 상단 룰·썸네일 74) → 배송지정보(주문자|수령자
  2열 격자, 전화/휴대폰 한 줄 — 시안의 '전화번호/전화번호' 라벨 교정, **수령자 E-mail 은 영카트에 필드가 없어
  행 삭제**) → 결제정보 2열 격자 → 주문금액 박스(검정 테두리 522px). 옛 2열(좌 정보 / 우 sticky 요약) 폐기.
- **시안에 없어도 남긴 기능(자리만 정함)**: 입금 안내 배너(머리줄 아래·스텝퍼 위) · 단계 설명(옛 '상태설명보기' →
  스텝퍼 아래 토글, **트랙별 문구** `sp_order_customer_step_notes($track)` — PCB 8칸/부품 7칸/일반 5칸 + A/S·취소류) ·
  진행 카드·제조 확인(EQ)·A/S 접수(주문 상품 아래 같은 제목 양식) · 희망배송일·전하실 말씀·운송장(배송지정보
  전폭 행, 운송장은 배송 시작 뒤에만) · 승인번호·영수증·현금영수증·포인트사용·환불(결제정보 격자에 이어 붙임,
  미입금 결제일시는 `—`) · 배송비·쿠폰·취소금액(주문금액 들여쓰기 블록, 총계 위) · 미결제액 행·결제액은 코어 로직
  (완불이면 `(완불)`) · 주문 취소(박스 아래 텍스트 버튼) · 취소 주문은 스텝퍼 대신 배지+종료 문구(시안 숨김 레이어
  '주문취소' 원은 채택 안 함).
- **진행 카드(⑩ 보류 → 두 변형 비교)**: 줄 배지·스텝퍼가 같은 진행을 보이므로 카드는 **좌표파일이 있거나 줄이 둘
  이상일 때만**(`$sp_progress_card_mode='auto'`). 항상 렌더하되 숨길 땐 `#sp_progress_wrap.is-quiet` — 러너가 CSS 한
  줄로 `*-card-always.png` 변형을 같이 남겨 사용자가 보고 고른다. `'always'` 로 바꾸면 예전처럼.
- **반응형(시안 없음 — 이쪽 판단)**: ≤640 격자 1열·스텝퍼 가로 스크롤(64px)·상품표 가로 스크롤.
- 검증: 러너 ① 6/6(표 diff = 카드 열이 1줄 진행 주문에서 '(카드 없음)' 으로 바뀐 것뿐)·② 16/16, 스크린샷
  `e2e/output/status-matrix/*-customer-detail{,-card-always,-mobile,-legend}.png`. 러너 셀렉터는 그대로
  (`th_itst`·`#sp_progress_wrap:not(.is-quiet)`·`.sp-steps__item.is-done/.is-current`).
...
### 운송수단(항공/해상) — 국제 발송의 두 번째 축 (2026-08-16)

`mode`(international|domestic)가 "국경을 넘는가"라면 `transport`(air|sea)는 **"무엇으로
나르는가"**다. 두 축은 직교하고, 해상은 리드타임이 항공과 자릿수로 다르며 **운송서류가
아예 다른 문서**(AWB ↔ B/L)라 문자열 `carrier` 안에 녹일 수 없다. "머스크"라고 적어도
시스템은 그게 해상인 줄 모르고, `SF Express` 처럼 양쪽 다 하는 회사에서 역추론은 깨진다.

- **사전 위치**: 값·라벨·헬퍼는 **BOM 과 공용**이다(`bom-po.ts` `SHIPMENT_TRANSPORTS`·
  `shipmentTransportOf`·`shipmentTransportDocType`). 같은 실물 축을 두 트랙이 다른 어휘로
  부르면 협력사가 화면마다 다른 말을 듣는다 — BOM 이관은 docs/SMARTBOM_PARTNER_RFQ.md §6.31.
- **원장**: `sp_pcb_shipment.transport VARCHAR(8) NULL`. **컬럼 default 를 두지 않는다** —
  "명시적으로 항공을 골랐다"와 "이 축이 없던 시절의 데이터"를 가를 수 있어야 한다.
  표시·게이트는 읽는 쪽에서 `pcbTransportOf`(null→'air')로 접고, **응답은 null 을 지킨다**
  (`asPcbTransport`). 응답에서 접으면 고른 적 없는 것이 화면에서 고른 것으로 굳는다.
- **적용 범위**: 국제 전용. 국내는 택배 체인이라 이 축이 없고, 계약은 같은 body 를 두
  모드가 공유하므로 **서버의 `mode === 'international'` 게이트가 유일한 방어**다
  (BOM `upsertShipment` 의 shipDate 게이트와 같은 관례).
- **서류 사전 확장**: `PCB_SHIPMENT_FILE_TYPES` 에 `bill_of_lading`(라벨 `B/L`). 업로드
  라우트는 이미 `PcbShipmentFileType.safeParse` 라 서버 코드 변경 없이 열렸다. Case ID
  갈래의 '선적' 게이트는 `pcbTransportDocType` 으로 찾을 종류를 유도하고, 없으면
  항공 `MISSING_AWB_FILE` / 해상 `MISSING_BL_FILE` 로 갈라 답한다 — 문구가 AWB 로
  남으면 화면에 **없는 버튼**을 가리킨다.
- **전환 시 정리**: 수단이 바뀌면 앞서 박힌 `carrier`·`trackingNumber`·`trackingUrl` 을
  서버가 함께 비운다(항공사명 + 해상 B/L 번호 같은 모순 방지). 화면 watch 도 같은 정리를
  하지만 진실은 서버다 — 관리자 프롬프트·재시도가 화면을 안 거칠 수 있다. ⚠ 이 정리
  스프레드는 `carrier` 스프레드보다 **앞**이어야 한다: 수단 변경과 새 운송장 입력이 한
  요청에 오면 뒤에 오는 새 값이 이겨야 한다.
- **"한 번 정하면 못 고치나"**: 아니다. `preparing` 중엔 화면 상태라 자유롭고,
  `requested` 이후엔 협력사 ↩되돌리기로 준비 단계에 복귀해 바꾼다(첨부는 유지된다).
  Case ID 갈래는 관리자가 부킹 주체라 '선적' 프롬프트에서 확정한다. 완전 초기화는
  **전부 꺼내기 → 박스 소멸**(기존 `detachPcbShipmentPo` — 첨부 실파일까지 삭제)이며,
  되돌릴 수 없는 조작이 다른 줄의 [꺼내기]와 생김새가 같으면 사고라서 **마지막 한 건을
  꺼낼 때만** 확인창을 세웠다(서류 건수를 문구에 싣는다).
- **화면**: 포털 체크리스트를 ①인보이스 → **②운송수단** → ③발송 방식 → ④출고예정일로
  재배열했다. ②가 ③보다 앞인 것은 순서가 곧 의존이기 때문 — 이 값이 정해져야 ③ 블록의
  서류 버튼이 AWB 인지 B/L 인지 정해진다. 항공은 정식 표기 셀렉트(`PCB_INTL_CARRIERS`)
  + 직접입력, **해상은 프리셋 없이 직접입력만**(선사보다 포워더 이름을 적는 실무).
  번호 라벨도 수단을 따른다(`AWB No.`/`B/L No.`) — 단 국내엔 서류 라벨을 쓰지 않는다
  (폴백이 새어 택배 송장이 'AWB No.'로 보인다). 관리자는 대행 '선적 요청'과 Case ID
  '선적' 프롬프트에서 수단을 받고(`allowCustom:false` — 사전 밖 문자열 차단), 선적
  워크큐 SH 셀에 수단 배지를 세운다.
- **동반 교정 — 첨부 종류 접힘(구 결함)**: `toFileView` 가 사전에 없는 종류를 전부
  `invoice` 로 접고 있었다(08-15 `test_report` 추가 때 매핑이 안 따라옴). 성적서만 올린
  발송이 화면에서 "① 인보이스 ✓ 첨부됨"으로 보이고 [선적 요청]이 열린 뒤 서버가
  `MISSING_INVOICE_FILE` 로 막는, **카드가 피하려던 409 왕복 그 자체**였다. 판정을 계약
  사전 파싱으로 바꾸고(BOM `toShipmentFileMeta` 관례) **모르는 종류는 접지 말고 뺀다** —
  남의 종류로 위장하면 게이트가 거짓 통과한다. 기존 회귀가 못 본 이유는 그 스펙이 API
  응답이 아니라 **DB 를 직접 읽어서**다(`demo-md-ship-keep`).
- **검증**: `pcb-transport` e2e 4케이스(해상 요청 박제+세 종류 첨부가 응답에서 안 접힘 →
  되돌리기 후 항공 전환 시 선사명·B/L 번호 소거 → 해상 Case ID 갈래가 AWB 로는 409
  `MISSING_BL_FILE`·B/L 로 통과 → 국내엔 수단이 안 박힘), 3패키지 typecheck·ESLint 0건.
...
### P5-b 구현 기록 (2026-08-25 — 고객 진입점: 마이페이지 "문의 > A/S 접수")

**P5 의 구멍을 막는다.** 접수 폼이 주문 상세 안에만 있어, 새로 접수하려면 주문내역에서 배송
완료 주문을 찾아 상세를 열어야 했고 낸 접수의 진행 상태도 그 주문을 다시 열어야만 보였다.
"내 A/S 목록" API 도 두 트랙 모두 없었다(EQ 와 같은 구멍 — P4.1-b 와 짝).

- **어휘 — "요청"이 아니라 "접수".** 확인 요청(P4.1-b)은 자사→고객, 이쪽은 고객→자사라
  방향이 반대이고, 결과(재생산·환불·안내)는 관리자 판정이라 이름이 결과를 정하지 않는다.
  화면 동사도 이미 전부 "접수"였다(접수됨·A/S 접수하기·주문·문제 접수). "A/S" 는 유지 —
  이 도메인에선 환불 요청도 A/S 로 내는 게 고객이 찾는 말이다. 주문 상세 섹션 제목
  `PCB A/S` → **`A/S 접수`** 로 중립화(메탈마스크 주문에도 같은 섹션이 선다).
- **트랙은 한 메뉴 안의 탭(PCB / 부품 BOM), '전체' 탭 없음.** 같은 상태가 트랙마다 다른
  말이라(`resolved` = 처리 완료 / 해결 완료) 한 목록에 섞지 않는다. 메뉴를 둘로 쪼개지 않은
  이유: 부품 BOM 배송 라인은 전체 25건(PCB 19,701건)이라 대부분 고객에겐 빈 메뉴가 되고
  배지도 갈린다. **부품 탭은 그 회원에게 부품 축(주문된 견적·접수 이력)이 있을 때만** 선다.
- **"접수할 주문" 섹션을 둔다** — 없으면 메뉴 이름이 "A/S 내역"이어야 맞다. 배송·완료
  주문행 중 스펙/견적이 있고 진행 중 접수가 없는 것. **최근 50 주문행 상한** — 회원 한 명이
  4,172 행을 가진 경우가 실재해(레거시 이관분) 행마다 판정 함수(쿼리 3회)를 돌리면 1만 회다.
  `g5-db.getDeliveredCartRowsByMember` 가 배송·완료를 SQL 에서 걸러 주므로 남는 판정은
  활성 클레임 유무뿐, 트랙별 라우트가 스펙·견적·활성을 배치 쿼리 3회로 붙인다. 밖의 주문은
  "주문내역에서 찾기" 안내(정책 '무기한 접수'는 그대로).
- **API**: `GET /api/pcb-claims/mine` · `GET /api/bom/claims/mine` (`scope=open|all`,
  `claimable`·`claimableTruncated`·`claims`(take 200)·`openCount`). 하나로 합치지 않는다 —
  계약·lib 이 트랙별로 갈라진 구조(어휘 격리 관례)를 지키고 PHP 가 탭별로 하나씩 부른다.
  소유권은 두 테이블 모두 행에 박제된 `mbId`(인덱스 `[mbId, submittedAt]` 이 "고객 내역"
  용도로 이미 있었다). `odId` 도 행에 있어 EQ 처럼 뒤에 붙일 필요가 없다.
- **접수 폼은 두지 않는다.** 접수하기 = 주문 상세 `#sp_as_wrap`, 내용 보기 = 접수 행 앵커
  `#as-{id}`(이번에 추가). 부품 행은 `/app/bom/:id`(클레임 패널은 데이터 로드 뒤 렌더라
  해시 앵커가 안 먹어 페이지 링크만). 폼을 복제하면 사진·수량 검증·확인 모달이 두 곳에서 갈린다.
- **배지 = 진행 중(open|reviewing) PCB+BOM 합산, 회색.** EQ 의 `requested` 는 고객 차례라
  빨간 배지였지만 A/S 의 진행 중은 **관리자 차례**다 — 같은 빨간 배지면 "당신이 할 일 N건"
  오신호. "답변 도착 안 읽음"은 읽음 기록(`readAt`)이 없어 못 만든다(후속 후보). 세기는 DB
  직접 count(`sp_pcb_claim_active_count`·`sp_bom_claim_active_count`, 사이드바 규약).
- **파일**: `spcb/pages/as.php`(`/shop/as`) · `extend/sp_pcb_claim.extend.php`(+mine·count) ·
  **`extend/sp_bom_claim.extend.php` 신설**(BOM 사전 PHP 사본 — PCB 와 함수 분리) ·
  `_account_nav.php` "문의" 그룹(1:1문의는 넣지 않는다 — 사용자 결정) · CSS 는 `.sp-eqm`
  공용 블록 확장 · `G5_CSS_VER` 26082502.
- **검증**: e2e `customer-as-menu` **6/6** — 두 트랙 `/mine` 의 접수할 주문↔접수 내역 배타 ·
  openCount scope 무관 · 남 격리 · 메뉴 2단·활성·**회색 배지 합산** · 탭 2개(전체 없음) ·
  **페이지에 폼 0개** · 접수하기→`#sp_as_wrap` · 내용 보기→`#as-{id}` 착지 · 부품 행→
  `/app/bom/:id` · 390px 오버플로 0 · 다른 계정 페이지에서 같은 배지. 시드는 e2e-customer 의
  실 배송 완료 주문행에 고객 경로로 접수(PCB multipart 사진·BOM JSON)하고 접수만 지운다.
  api 961 · typecheck · PHP lint 5파일.
- **여정 44호 `journey-pcb-as-mypage`(9/9)** — 거버 → RFQ → 주문·입금 → 발주 → EQ 고객확인
  (**/shop/eq 에서 출발**, PHP 승인) → 생산 → 국제 발송 → 입고 → 배송 → 완료까지 1호 경로를
  밟은 뒤, **마이페이지 A/S 접수 진입점으로** 접수→종결을 완주한다: 배송 전엔 접수할 주문에
  없음(게이트) → 완료 뒤 등장 → [접수하기] 딥링크 → 주문 상세 폼에 **실 브라우저로** 사진
  동반 접수 → 접수 내역·회색 배지 1·접수할 주문에서 빠짐·[내용 보기] 앵커 착지·주문 상세
  "새 접수 닫힘" → 관리자 검토(마이페이지 '검토 중') → 판정(재생산) → 케이스 → 협력사 수락
  (고객 사진 열람) → 회차 1 발주 → 회차 EQ·생산·발송·입고 → 종결 뒤 진행 중 0·배지 없음·전체에
  '처리 완료·답변 있음'·**재접수 개방**·주문 상세 답변 본문·**고객 pageerror 0**. 정리는
  force-status '주문'(재고 복원) → g5 → sp_* 순, `JOURNEY_KEEP=1` 이면 남긴다.
  1차 주행이 잡은 결함 2건(교정 완료): ① A/S 접수 버튼이 EQ 스크립트의 `.sp_eq_btns button`
  셀렉터에 걸려 `.sp_eq_decision` null TypeError(제출은 됐지만 콘솔 오류 — `.sp_eq_form` 으로
  좁힘) ② 증상 설명 textarea 가 `.sp_eq_form textarea` 규칙만 타서 A/S 폼에선 인라인 기본
  크기로 찌그러짐(`.sp_as_form textarea` 추가, 폼 폭 90% 이상 어서션).

### 메탈마스크 트랙 구현 기록 (2026-08-16 — EQ 를 고객문의사항 + 좌표파일로 대신한다)

메탈마스크(스텐실)에는 제조 확인 질의를 주고받을 것이 없다. 그 자리를 **협력사가
고객문의사항을 적고 좌표파일을 올리는 것**으로 대신하고 관리자는 확인만 한다. 나머지
단계(생산·발송·송금·A/S)는 전부 그대로다(사용자 결정 2026-08-16).

**핵심 판단 — status 5단계를 포크하지 않았다.** 요청을 단계로 풀면 칸의 개수도 주체도
순서도 EQ 와 같다. 갈리는 것은 ①그 칸에서 무엇을 반드시 내야 하는가 ②뭐라고 부르는가,
둘뿐이다. 상태를 새로 만들면 화면 8곳의 라벨 사전·워크큐 탭 6종 카운트·파생 12단계
`pcbStepOf`·고객 진행표시·선적 게이팅·MD 미러·메일 4종·e2e 31본이 통째로 두 벌이 된다.

- **트랙 파생(계약 정본)** — `resolvePcbPoTrack(spec.category)` → `'eq' | 'stencil'`.
  판정은 **서버 한 곳**(lib/pcb-po `loadPcbPoTracks`)이 하고 화면·PHP·메일은 실려 온
  `track` 을 읽기만 한다. `PCB_PO_STATUS_LABELS` 는 **트랙별 2벌**로 바꿨다 — flat 사전을
  남기면 새 화면이 그걸 집어 스텐실에 'EQ 승인요청'을 써도 타입이 통과한다(조용히 틀린다).
  `pcbEqForwardLabel`·`pcbEqRevertLabel` 도 같은 이유(전이 사전 `PCB_EQ_FORWARD/REVERT`
  자체는 주체·순서의 정본이라 **트랙으로 갈리지 않는다** — 갈리면 서버 검증이 두 벌이 된다).
- **좌표파일** — `PCB_EQ_FILE_TYPES` 에 `'coord'` 추가(`sp_file.file_type` 은 VarChar 라
  **마이그레이션 없음**). 잠금은 협력사 산출물과 동일(`canEditPcbEqFile` — issued 에서만).
  ⚠ `lib/pcb-po.ts` 의 `asEqFileType` 에 새 종류를 안 더하면 그 파일이 조용히 'eq' 로
  뭉개진다(주석이 경고하던 함정 — 실제로 밟을 뻔했다).
- **고객문의사항** — **새 컬럼 없이** `eqHistory` 의 note 다(`advancePcbPoEq` 가 `note:
  null` 로 하드코딩하던 자리). 타임라인이 이미 note 를 말풍선으로 그려 화면 작업이 0 이다.
  `lastPcbEqRejection` 은 `eq_requested → issued` 만 보므로 `issued → eq_requested` 인
  여기와 안 부딪힌다. ⚠ 바디는 **nullish** 로 받는다 — 이 라우트는 원래 바디가 없었고,
  필수로 두면 본문 없는 기존 호출이 `Expected object, received null` 로 죽는다(실측).
- **제출 게이트(서버 강제)** — `pcbStencilSubmitBlockers(note, files)` 를 화면과 서버가
  공유한다(서버가 정본, 화면은 버튼을 미리 잠그는 용도). 문의사항 2자 이상(D16 하한과
  동일) + coord 1건 이상. **관리자 대행(D11)도 예외가 아니다** — 대행은 "누가 누르는가"의
  문제이고, 좌표 없이 확인 완료로 넘어간 건은 나중에 고객 화면에서 빈자리가 된다.
- **고객 열람 = 통보 없는 열람**(사용자 결정) — 메일도 확인 요청(D16) 행도 만들지 않는다.
  `/api/pcb-progress` 응답의 `coordFile` 에 실려 주문내역 '제작 진행 상황' 줄에 링크로 뜬다.
  제약 셋: ①**관리자 확인(eq_done) 뒤에만**(그 전엔 보완 요청으로 파일이 바뀔 수 있고,
  반려된 좌표로 고객이 SMT 설비를 잡는 것이 이 기능의 최악이다 — 확인 뒤로는 첨부가 영구
  잠기므로 내려받은 것이 안 달라진다) ②**최신 1건만** ③**이름 중립화**(`부품좌표_{프로젝트}
  .{ext}` — 협력사가 붙인 원본 파일명에 협력사명이 박혀 있을 수 있다, 여정 43호 축).
  다운로드는 `/api/pcb-progress/coord-files/:fileId` + PHP 브리지 `spcb/api/coord-file.php`
  (eq-file.php 동형). **종류(coord)가 곧 공개 권한**이고 단계·소유권은 서버가 다시 판정한다.
- **화면** — 협력사 상세는 스텐실이면 문의사항 textarea + 좌표파일 버튼만 열고(EQ 질의서·
  Working 칸을 내지 않는다), 못 보내는 이유를 버튼 옆에 적는다. 관리자 Case 는 [EQ 승인]
  → [확인 완료]·[반려] → [보완 요청], 안내문에 "확인하면 고객도 내려받는다"를 명시. 고객
  진행 라벨은 '제조 확인(EQ) 진행 중' → '제작 전 확인 중'.
  ⚠ **반려는 스텐실에도 남긴다** — `eq_requested` 에서 뒤로 가는 다른 문은 협력사의 '요청
  취소'뿐이라, 빼면 관리자가 잘못 온 좌표를 돌려보낼 수단이 아예 없어진다.
- **실측 근거** — metalMask 견적 1,289건(전체 20,805 중 3위)인데 **발주 0건**이고 e2e
  픽스처도 0개였다. 이관 걱정 없는 신규 경로라 지금이 가장 싼 시점이었다.
