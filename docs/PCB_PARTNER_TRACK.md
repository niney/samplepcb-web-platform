...
- **삭제 진입점(08-06 추가)**: 삭제는 원래 **통합 관리 → 견적 관리**에만 있었다(목록 체크박스
  ...
  상세라 모듈을 오가야 했으므로, **Case 상세 헤더에 [견적 삭제]** 와 **진행현황 목록의 체크박스
  + [선택 삭제]**(배치)를 추가해 같은 모달을 연다 — 창구만 셋, 판정·모달은 하나다. 상세에서
  삭제하면 진입 워크큐(`?from=`)로 되돌리고, 목록은 선택을 비운다. 전체선택은 견적 관리와 같은
  규칙(현재 페이지 범위만 · 탭·검색·페이지 이동 시 선택 해제).
- **모달 재설계(08-06, 사용자 요청 "SmartBOM과 비슷하게")**: `DeleteQuoteModal` 을 SmartBOM Case
  삭제와 같은 **3단 위험 레이어**로 바꿨다 — ① 1차(연빨강 헤더): 통계 4칸(선택/삭제 가능/결제
  강제 가능/보호됨) + 합산 영향 4칸(파일·RFQ/발주/선적·협력 첨부·주문/장바구니) + 건별 카드
  (상태 배지·이관 태그·차단 사유 전부) + 주문 그룹·경고 ② 2차(진빨강 헤더): 대상 목록 + 결제
  강제 체크 + 사유 + 복구 불가 확인 ③ 결과: 삭제·차단·실패를 한 화면에서. **견적 관리와 PCB가
  같은 컴포넌트를 쓰므로 두 화면에 동시에 적용된다.** i18n(ko/en)은 유지하고 키를 확장했다
  (SmartBOM 모달은 한국어 리터럴이지만 이 모달은 원래 i18n 이라 회귀시키지 않았다).
