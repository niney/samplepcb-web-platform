...
- **삭제 모달 디자인 통일(08-06)**: 단층 모달이던 것을 **SmartBOM Case 삭제와 같은 2단계**로
  맞췄다(사용자 결정) — 1차 = 영향 확인(차단 사유 전부 · 함께 사라지는 것 집계 그리드 · 삭제
  대상 목록 · 주문 연결 · 경고), 2차 = **붉은 헤더의 최종 확인**(강제 해제 체크 · 사유 · 복구
  불가 확인 · [N건 영구 삭제]). 되돌릴 수 없는 배치 작업이라 "보고 나서 한 번 더 결심"하는
  층을 두는 것이 요지다. 강제 해제 체크를 켜면 대상 집합이 바뀌므로 1차로 되돌려 다시 보게 한다.
