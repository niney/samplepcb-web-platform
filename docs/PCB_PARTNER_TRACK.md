...
**일부러 안 고친 것 2가지**: ① 상업송장 품목 description 을 PO 번호로 바꾸는 것은 통관 서류
기본값 변경이라 정책 판단이 필요하다(수량·단가·합계는 원래 정확했다) ② 견적관리 `NEW`
배지는 다른 상태 견적이 모수일 수 있어 '의심'으로 남겼다.
...
**별건 red(이번 변경과 무관)**: `journey:md`(여정 4호)가 첫 케이스에서 실패한다 — 협력1 을
MD 로 전환할 때 기대값이 `PARENT_HAS_ACTIVE_POS`(409)인데 실제는 `PARENT_IS_CHILD`(400)다.
원인은 **상설 관계 `마스터딜러상사 → 협력1`**(MD 3편이 만든 픽스처, 관계 #12)이라 협력1 이
이미 남의 하위여서 다른 가드가 먼저 걸린다. 픽스처 드리프트에 따른 기대값 노후화이며,
고치려면 여정 4호가 자기 전용 조직을 쓰게 하거나 어서션을 현 픽스처에 맞춰야 한다.
