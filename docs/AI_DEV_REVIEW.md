...
### 13.9 의뢰하기·상세 재설계 — 타입 스케일 한 단계 위 · 폭 · 레이아웃 (2026-09-04 밤, 사용자 확정)

정적 시안(claude.ai artifact `0ed05488`, 스크래치 `market-redesign-mock.html`)을 확인받고 컴포넌트로 옮겼다. 로직·API·계약은
손대지 않았고(`why` 한 필드만 레지스트리에 추가), 관리자(apps/web)는 별도 컴포넌트라 영향 없다.

| # | 결정 | 근거 |
|---|---|---|
| 40 | **타입 스케일 6단계**를 `style.css` `@theme` 토큰으로: `text-micro` 12 · `text-label` 13 · `text-body` 14 · `text-lead` 15 · `text-title` 18 · `text-h1` 28. 의뢰하기·상세·검토서·구성도 섹션은 이 여섯만 쓴다 | 기존은 본문 12px·라벨 10~11px(상세 43곳). Tailwind 기본 `text-xs` 를 전역으로 키우면 헤더·홈까지 번지므로 역할명 토큰으로 두 페이지만 옮겼다. 다음 조정은 토큰 한 곳 |
| 41 | 분야 색 5종 `--color-area-{circuit,pcb,firmware,app,server}`(`@theme static` — 인라인 var() 로만 읽어 유틸리티 미사용 변수 제거에 걸린다) — **`AreaIcon` 타일**(색 배경 + 흰 선 아이콘 5종, md 40/sm 32)에만 싣는다. 1스텝 분야 카드·2스텝 분야 카드 헤더·검토서 분야 카드가 같은 타일. 왼쪽 색 띠는 사용자 피드백으로 폐기 | 분야가 늘면 토큰 하나 + 아이콘 하나 |
| 42 | **의뢰하기 1280px**(`max-w-7xl`) = 폼 + 320px sticky 사이드 `WizardAside`(①AI 안내·예상 시간 ②공통 조건 n/6 항목별 값 ③최종 요약 — 옛 3스텝 하단 "최종 의뢰 내용" 블록이 여기로) + **하단 고정 액션 바**(이전 · 공통 조건 n/6 · 답변 n · 첨부 n · 다음/등록). 스텝 인디케이터는 진행 바 3칸. 2스텝은 블록마다 자기 카드라 바깥 카드 없음 | 14px 본문에 폼 폭 약 900 이 읽기 한계. 2스텝이 길어져 "다음"이 멀어진 것을 액션 바가 해결. 트레이는 위저드에서 `bottom-24` 로 올린다 |
| 43 | 1스텝 분야 선택 = **3×2 아이콘 카드**(5 분야 + 6번째 점선 카드 "잘 모르겠어요 — 전부 맡길게요", 선택 시 "5개 분야 모두"). 선택 = 카퍼 테두리 + 연한 카퍼 배경 + 우상단 체크(색 반전 없음 — 설명이 계속 읽힌다). 옛 5열 카드·별도 칩 버튼 폐지 | 5열은 카드 150px 에 설명이 3줄로 접혔고 선택이 카드를 남색으로 뒤집어 설명이 죽었다 |
| 43-1 | 문항에 **"왜 묻나요" 한 줄**(`MarketQuestionDef.why`, 고객용 — LLM 용 `promptHint` 와 별개) 17문항. 칩 36px·13px, 탈출구는 점선 | 참고 사이트의 질문 부제 |
| 44 | **상세 1440px** = 헤더(종이 위, h1 28·메타 한 줄에 조건 3 포함) → **sticky 섹션 내비**(의뢰 내용 · AI 사전 검토서 · 시스템 구성도 · 첨부 n · 받은 견적 n, IntersectionObserver 로 현재 표시) → 본문 + 360px sticky 사이드. 본문 텍스트 블록 최대 900px | 답변 표가 1000px 을 가로지르던 문제 |
| 45 | **답변 표는 "의뢰 내용" 한 곳에만** — `DevReviewView` 의 "고객 의뢰내용" 섹션 삭제(검토서는 요약·핵심 요구사항부터). 의뢰 내용 = 설명(15px) · 프로젝트 조건 타일 3 · 답변 표 · 희망 툴 행 | 같은 표가 두 번 보였다(상세 설명 카드 + 검토서). 위저드 3단계도 폼이 곁에 있어 불필요 |
| 46 | 검토서 분야 카드 3열 → **2열**(왼쪽 분야 색 띠), 상의 항목은 번호 2열, 명세서는 [분야명 150px | 항목 2열] 행. 구성도 생략·미생성은 점선 카드 안내(500자·첨부 조건 문구) | 14px 에서 3열은 좁다 |
| 47 | 사이드 카드: 소유자 액션(조기 마감·취소) · 전문가 액션 · 구성도 상태 요약(메타 있을 때) · 안전거래(잉크 카드) | 소유자 액션이 헤더에서 떨어져 있던 것을 상단으로 |
| 48 | **첨부 = 드롭존**(`FileDropZone.vue`, `variant: panel \| slot`). 1스텝 참고 자료는 전폭 **한 줄** 패널(높이 74px — 36px 아이콘 + 문구 2줄 + 오른쪽 "파일 선택" 알약. 세로로 큰 첫 시안은 사용자 피드백으로 축소). 굵은 첫 줄은 **허용 형식**(PDF · Word · 엑셀 · 이미지 · 손그림 사진 — 회로도·거버는 2스텝 분야 슬롯 몫이라 1스텝에서 뺐다), 둘째 줄이 드롭 안내 — 무엇을 올릴 수 있는지가 어떻게 올리는지보다 먼저다(드래그 중에는 첫 줄만 "여기에 놓으세요"로 바뀐다), 2스텝 분야 슬롯은 **크기 그대로** 두고 드롭만 받는다. 드래그 중에는 카퍼 테두리 + "여기에 놓으세요"(dragenter/leave 깊이 카운터로 깜빡임 방지). 기본 `input` 은 `sr-only`(1px 박스라 Playwright `setInputFiles` 가 그대로 잡는다 — `display:none` 금지) | 옛 화면은 드롭 핸들러가 아예 없어 카드에 놓으면 브라우저가 파일을 열어 작성분이 날아갔다. 슬롯은 분야 5 × 최대 2 = 10칸이라 패널 크기로 키우면 2스텝이 스크롤 지옥 |
| 48-1 | 첨부는 **누적**(같은 `name+size+lastModified` 만 중복 제거) + 파일별 ✕ 삭제. 지우기 목록은 `<label>` **밖**에 둔다 | 옛 `pickAttachments`/`pickSlotFiles` 는 배열을 교체해 두 번째 선택이 먼저 고른 파일을 조용히 지웠다. 드래그앤드롭은 나눠 놓는 것이 정상 동작이라 교체면 사고가 잦아진다. ✕ 가 label 안에 있으면 클릭이 파일 선택창을 연다 |
| 48-2 | 위저드 페이지 전체에 **드롭 가드**(`window` 의 `dragover`·`drop` 기본 동작 차단, `onMounted`/`onBeforeUnmount`) | 드롭존을 빗나간 드롭 하나로 의뢰 작성분이 통째로 사라지는 것을 막는다. 드롭존 자신의 핸들러는 그 전에 이미 실행된다 |
...
### 13.11 3스텝 생성 중 화면 + 개발 전용 상태 미리보기 (2026-09-04 밤)

**① 분석 카드** — 옛 화면은 "검토서 작성 중… 경과 12초" 한 줄뿐이라 30초~3분이 빈 화면으로 흘렀다. 완성될 검토서 자리를
미리 차지하는 카드로 바꿨다(`StepReview`).

| 요소 | 내용 |
|---|---|
| 헤더 | 카퍼 링이 퍼지는 아이콘 + `AI 가 자료를 읽고 있습니다` / `AI 가 검토서를 쓰고 있습니다`(단계에 따라) + 한 줄 설명 + `경과 n초 / 보통 30초~3분` |
| 진행 바 | 불확정(왼→오른쪽 이동). 남은 시간을 모르는 작업이라 퍼센트를 지어내지 않는다 |
| 단계 | **서버가 실제로 보내는 2단만**(`stage`: attachments → review) = `자료 읽기` · `검토서 작성`. 지난 칸 ✓(초록), 현재 칸 카퍼 + 점 펄스. 3단계로 보이게 없는 진척을 만들지 않았다 |
| 스켈레톤 | 요약 1줄 + 본문 2줄 + 분야 카드 2개 셔머 — 완성 레이아웃과 같은 골격이라 검토서가 들어와도 높이가 튀지 않는다 |
| 90초 초과 | "조금 더 걸리고 있습니다 — 자료가 많으면 3분까지" 한 줄(멈춘 것으로 읽히는 구간) |
| 구성도 | 같은 카드 안에 병렬 상태 한 줄(생성 중 5~10분 · 생략 사유). 옛 화면은 안내 문단 속에 묻혀 있었다 |
| 탈출구 | `검토서 없이 바로 등록` 을 카드 안 마지막 줄로(옛 별도 박스) |

애니메이션 3종(`ai-bar`·`ai-ring`·`ai-skel`)은 `prefers-reduced-motion: reduce` 에서 전부 정지한다.

**② 개발 전용 미리보기** — `/market/dev/review-states` (`apps/market/src/pages/dev/ReviewStates.vue`).

- 라우터가 `import.meta.env.DEV` 일 때만 등록한다 → 운영 빌드·운영 URL 에 없다.
- **진짜 `StepReview`** 를 가짜 잡(`DevReviewJob` 과 같은 모양의 computed 묶음)으로 렌더한다. 실 LLM·API 호출이 없어
  로딩 화면을 손볼 때 위저드를 완주하지 않아도 되고, 고친 스타일이 곧 실제 3스텝에 반영된다(같은 컴포넌트).
- 스위처: 상태 6종(자료 읽는 중 · 검토서 작성 중 · 90초 초과 · 완료 · 완료+오래됨 · 실패) × 구성도 3종(생성 중 · 생략 · 없음)
  + 경과 초 슬라이더(0~180). 완료 상태는 표본 검토서(급식기 제어 보드, 4분야)로 검토서 뷰 전체를 그린다.
- 여는 법: `pnpm dev` 상태에서 <https://local-web.samplepcb.co.kr/market/dev/review-states> (nginx 없이 vite 직결이면
  <http://127.0.0.1:5176/market/dev/review-states>). 로그인 불필요.

검증(2026-09-04): 상태 6종·구성도 3종 전환 실브라우저 확인(완료 상태는 요구사항·상의 항목·개발명세서까지 렌더), 콘솔 오류 0,
market 타입체크·eslint 0.

### 13.12 구성도 생성 중 화면 (2026-09-04 밤)

검토서 카드(§13.11)와 같은 언어(불확정 바·경과·액자)를 쓰되 **두 가지는 일부러 다르게** 했다. `DevDiagramSection` 한 곳을
고쳤으므로 3스텝 미리보기·의뢰 상세·관리자 화면에 동시에 반영된다.

| | 검토서(§13.11) | 구성도(여기) |
|---|---|---|
| 단계 목록 | 2칸(서버 `stage` 가 실제로 2단) | **없음** — 잡 상태가 `queued`/`running` 둘뿐이라 칸을 만들면 없는 진척을 지어내는 셈. 대신 `대기 중 — 곧 시작합니다` / `구성도를 그리는 중입니다` 두 문구로 가른다(프로세스 내 큐가 동시 1이라 대기는 실제 상태다) |
| 주 메시지 | "곧 끝납니다"(30초~3분) | **"기다리지 않으셔도 됩니다"**(5~10분) — 카드에서 가장 크게 읽히도록 종 아이콘 + 15px 굵은 제목의 파란 배너로 세웠다(사용자 요청). 등록 뒤 알림·메일로 오고, 지금 등록해도 계속 만들어진다는 것까지 적는다 |
| 경과 | 잡 시작부터 클라이언트 타이머 | `meta.requestedAt` 기준이라 **화면을 다시 열어도 이어진다**. 12분 초과(실측 최대 581초)면 "예상보다 오래 걸리고 있습니다 — 최대 15분" |
| 액자 | 검토서 골격 스켈레톤 | **220px 청사진 액자**(사용자 확정: 작게) — 격자 위에 블록 4 + 연결선 4 가 순서대로 나타났다 사라진다. 완성 미리보기(320~720px)보다 작지만 "도면 자리"는 잡아 준다 |

애니메이션(`dg-bar`·`dg-draw`)은 `prefers-reduced-motion: reduce` 에서 정지하고 블록은 그대로 보인다.

개발 미리보기(`/market/dev/review-states`)에 **구성도 섹션 단독 블록**을 더했다 — 대기 중 · 그리는 중(3분) · 그리는 중(13분) ·
완성 · 실패 · 생략 · 없음 7종. 위저드 경로에서는 `html` 이 언제나 null 이라(본문은 등록 뒤 상세에서만) **완성 모양은 여기서만**
볼 수 있다. 표본 도면 SVG 를 넣어 액자 크기 대비도 같이 본다.
