...
- **엔진 조달 판단 투영(`engine-procurement-projection-v8`, 2026-07-21)**:
...
- **판단 단일 설명 원본(2026-07-21)**: 후보의 관계·선택 자격·기술/검토 순위·사전 선정과
  ...
  `manual_review + preselect`는 `검토 권장` 임시 선정으로만 강조한다. 사용자 확인 전에는
  `selectedCandidateKey`나 견적 금액에 반영하지 않는다.
...
- **검토 표현**: `automatic`만 자동 적용한다. `manual_review + preselect + review_recommended`는
  가장 유력한 후보·오퍼를 알려주되 사용자 확인을 요구하고, `candidate_only`는 일반 검토 후보,
  `exclude`는 선택 불가로 표시한다. 후보 패널의 검증률은 confidence를 그대로 `100%`로 표시하지
