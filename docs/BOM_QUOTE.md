...
사이드바 메뉴·홈 토글의 "단일 검색"을 실라우트로 활성화(1단계 — 카탈로그 열람 전용).
- `pages/bom/BomSearch.vue`: 토글(단일 검색 활성)+필요수량 입력+`BomPartSearchPanel`
  **browse 모드**(신규 prop — 부품 변경 CTA 숨김, 검색→상세→공급 포장·오퍼 비교만).
  `?q=` 초기 질의 지원. 검색 API 는 기존 `/api/bom/parts-search`·`/bom/parts/:id` 그대로.
- 환율은 견적 문맥(`usdKrwRateUsed`)에만 있어 단일 검색은 원통화 표시(usdKrwRate=null).
