...
- **저항·커패시터 행 검색조건 보완(2026-07-24)**: 후보 상세에서 원본 추출값을 수정하지 않고
  ...
  자동선정에 필요한 조건이 비어 있으면 엔진은 후보를 수동검토로 유지한다. 정격전력은 사용자가
  지정한 저항에서만 필수 비교하고, TCR은 이 계약 범위에서 제외한다. 변경 시 해당 component만
  별도 공급사 실행으로 재검색하고 그 행의 기존 명시 선택도 새 조건으로 다시 판정하며, 다른 행의
  후보·선택·검색 trace는 보존한다.
