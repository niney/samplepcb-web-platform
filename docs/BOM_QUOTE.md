...
- `sp_bom_quote_candidate`(SpBomQuoteCandidate): quoteItemId로 안정 행에 연결하며 엔진의 `identity_key`가 묶은
  ...
  검색은 sp-engine이 기술 판정 후 공급사별 기술 상위 3개와 가격 상위 2개 그룹의 합집합으로 제한하고
  행당 최대 10개 그룹을 저장한다. sp-node도 구형·과대 응답을 10개로 방어 제한하되 현재 명시 선택,
...
| 현재 엔진 결과: sp-engine이 기술 1순위를 감사 근거로 보존한다. 저항·커패시터 동급 안전 후보 및 제조사 미기재 정확 MPN 검토 후보는 **실효 총액 최저**를 적용하고, 그 밖에는 기술 1순위를 유지한다. 기술 1순위가 구매 불가하면 다음 안전 후보군을 실제 적용 후보로 분리한다. 각 후보군 안에서는 **구매 가능성→과다주문 위험→실효 총액→주문수량**으로 오퍼를 정한다. sp-node는 적용 후보 키와 안정 `offer_key`, 주문수량·가격구간·환산단가를 검증 후 그대로 저장 | 신규 — 기술·구매 판단을 엔진으로 일원화 |
...
- **비용 게이트**: preflight 예상 호출은 관측·경고용이다. 예상치가 작업 한도를 넘어도
  ...
  상단에 강조 경고를 지속 표시한다. 작업 호출 상한과 공급사 자체 쿼터를 구분하고 실제 호출 수·상한,
  영향 행 필터를 제공하며 각 행에도 `호출 상한 미검색` 배지를 표시한다. 이미 확인된 후보·금액은
  그대로 사용할 수 있게 하고, 구형 요약의 오집계는 영속 search trace로 복구한다.
...
- **부품 유형별 자체 카탈로그 우선 조회 provenance(2026-07-26)**: R/C는 SamplePCB 스펙
  카탈로그를, connector는 exact MPN 자체 커넥터 카탈로그를 외부 공급사 검색 전에 조회한다.
  공급사명이 아니라 sp-engine의 부품 유형 판정을 기준으로 하며, 로컬 후보가 엔진의
  `automatic_selected`를 통과한 행만 외부 검색에서 제외한다. 이 단계는 외부 공급사 trace로
  위장하지 않고 검색 실행의 `preflight` JSON에 카탈로그 유형·정규 검색어·로컬 후보 수·엔진
  판정 수·선정 결과·소요시간을 별도 저장한다.
  후보 비교의 `검색 과정`에서는 항상 첫 단계로 표시하고 `API 0회`를 명시하며, 이후 실제
  DigiKey·Mouser·UniKeyIC 시도는 기존 순서대로 이어서 보여준다. 구형 실행에는 이 기록이 없어
  `localCatalogTrace=null`로 읽기 호환하고 SamplePCB 전용 v1 기록은 현재 v2 계약으로 투영한다.
...
- **저항·커패시터 행 검색조건 보완(2026-07-24)**: 후보 상세에서 원본 추출값을 수정하지 않고
  `sp_bom_quote_item.search_requirements`에 사용자 조건과 수정자·시각을 별도 저장한다. 저항은
  저항값+패키지, 커패시터는 정전용량+패키지+종류가 검색 최소조건이며, 공차·전압·전력·유전체를
  모르면 비워 둔 채 후보 검색을 진행할 수 있다. 다만 저항 공차, 커패시터 공차·전압, MLCC 유전체처럼
  자동선정에 필요한 조건이 비어 있으면 기본적으로 후보를 수동검토로 유지한다. 사용자는 견적 단위
  확인 창에서 저항 공차와 MLCC 공차·최소 정격전압을 한 번 승인할 수 있고, 유전체는 1nF 이하 C0G·
  초과 X7R 보수 정책으로 적용한다. 승인 기본값은 누락 필드에만 적용하고 BOM·행별 명시값을 덮지
  않으며, 전해·탄탈·필름 커패시터에는 적용하지 않는다. 엔진은 이 값도 hard requirement로
  품번 대체·파라메트릭 후보의 실제 사양과 검증해, 미기재·불일치 후보는 계속 검토/차단한다.
  승인값은 공급사 실행 옵션에 저장되어
  이후 행 재검색에도 상속된다. 정격전력은 사용자가 지정한 저항에서만 필수 비교하고, TCR은 이 계약
  범위에서 제외한다. 행별 변경 시 해당 component만 별도 공급사 실행으로 재검색하고 그 행의 기존
  명시 선택도 새 조건으로 다시 판정하며, 다른 행의 후보·선택·검색 trace는 보존한다.
...
- **정확 MPN 우선 자동 선정(2026-07-22)**: `supplier-candidate-decision-v3`는 정규화 제조사 품번이
  ...
  조건 불일치는 계속 차단한다. 제조사가 비어 있는 원본 품번이 여러 구조화 제조사에서 동시에 발견되어도
  제조사별 identity는 분리하되 공급사 간 제조사 표기 차이를 충돌이나 수동검토 사유로 만들지 않는다.
  sp-node는 현재 v3와 저장된 v1·v2 결정을 모두 수신하고 이 정책을 재추론하지 않는다.
- **완료 반영과 카탈로그 분리(2026-07-21, 무인 복구 2026-07-23)**: 폴러는 엔진 결과를 한 번 읽어 견적 후보·선정
  오퍼·가격 스냅샷과 `done`을 먼저 커밋한다. 사용자가 기다리지 않아도 되는 전역 카탈로그 DB/ES
  동기화는 그 뒤 백그라운드에서 수행한다. 공급사 원문은 gzip 영속 작업으로 먼저 보존하며 30초
  복구 워커가 연결 종료·DB 경합·ES 장애·프로세스 재시작 후에도 자동으로 이어간다. 원문이 없는
  기존 견적의 명시적 재시도는 영속 분석에서 공급사 검색을 다시 시작한다. 새 부품의 `partId`는 잠시 null일 수 있지만 후보 비교·
...
- **완료 반영 경로 3중**: ① 결과 폴러의 견적 우선 반영 ② 결과 조회 카탈로그 백업 훅 ③ **게으른 치유** —
  ...
  completed 면 엔진 판정 반영을 즉발하고 카탈로그는 영속 백그라운드 작업으로 넘긴다(고객의 3초 폴링이 곧
...
- **엔진 조달 판단 투영(`engine-procurement-projection-v13`, 2026-07-28)**:
  ...
  명시한다. 저항·커패시터 파라메트릭 후보와 제조사 미기재 정확 MPN 검토 후보는 선택 자격·매칭 관계·
  필수조건 판정·충돌·누락·수명주기 상태가 모두 같은 안전 밴드에서 실효 총액 최저 그룹을 적용하고
  `price_optimization_used=true`를 명시한다. 정확한 제조사+MPN과 안전 밴드가 다른 후보는 가격으로
  교체하지 않으며, 가격 최적과 구매 불가 기술 fallback 사유는 동시에 참일 수 없다.
  필요수량 대비 절대 초과량(기본 100개)과 초과비율(기본 50%)이 **모두** 한도를 넘는 오퍼는 구매 가능·
  사용자 수동 선택 상태는 유지하되 추천 대상에서는 제외한다. 이 때문에 기술 사전 선정 그룹이
  1→5,000 같은 과다수량 오퍼만 가지면 다음 구매 가능 기술 그룹을 평가하고, 대안이 없으면 미선정으로
  남긴다. sp-vue는 선택된 레거시/수동 오퍼도 같은 기준으로 `필요/주문/초과/배율` 경고를 표시한다.
  ...
  - **샘플/양산 조달 모드**: 견적은 `procurementMode=sample|mass`를 저장한다. `sample`은 기존처럼
    안전한 구매 가능 오퍼 중 실효 총액을 우선하고, `mass`는 같은 기술 판단·재고·과다주문 게이트를
    유지한 채 명확한 제조사 Reel(`Tape & Reel` 포함)→재포장 서비스(`Digi-Reel`/`MouseReel`)→
    기타 포장 순으로 구매 적합도를 정한다. `Reel, Cut Tape, MouseReel`처럼 한 레코드에 여러 선택지가
    섞인 값은 Reel로 단정하지 않는다. 안전한 Reel이 없으면 기존 구매 가능 오퍼로 축퇴하고
    `mass-production-reel-unavailable` 근거를 남긴다. 모드 전환은 공급사 API를 다시 호출하지 않고
    저장 후보를 `supplier-selection-application-v4`로 배치 재평가한다. 고객이 직접 고정한 후보·오퍼는
    바꾸지 않으며 기술 사전 선정의 의미(MPN·제조사·기술 근거)도 불변이다. 구버전 후보 키는 현재
    정규화 규칙으로 바뀔 수 있으므로 검증기는 원시 키 변경과 기술 의미 변경을 구분한다.
...
- **판단 단일 설명 원본(2026-07-21)**: 후보의 관계·선택 자격·기술/검토 순위·사전 선정과
  ...
  - **후보 shortlist(2026-07-24)**: 스펙 검색은 DigiKey·Mouser에 10건을 요청하고, 크기를 지정할 수
    없는 UniKeyIC 응답은 전체 기술 판정 후 상위 10개 그룹만 조달 판단에 사용한다. 정확 MPN 경로의
    기존 20건 상한은 유지한다. sp-engine은 조달 판단 후
    공급사별 기술 상위 3개와 실효 총액 상위 2개 identity/evidence 그룹의 합집합을 전달한다.
    기술 사전 선정·실제 적용 그룹과 한 공급사가 선택한 동일 그룹의 타 공급사 오퍼를 함께 남겨
    교차 가격 비교를 보존하고, 원본 결과 수는 trace에 유지한다. sp-node의
    행당 10개 영속 상한은 구형·비정상 과대 결과에 대한 저장 안전장치이며 별도 기술 판단은 아니다.
  ...
  - **구버전 결과 처리**: 1.3 조달 결정이 없거나 v1/v2 적용 계약인 저장 후보는
  sp-node의 옛 구매 규칙으로 복원하지 않는다. 원본 엔진 후보가 있으면 위 배치 재평가로 현재 v4
...
- 고객 결과는 `매칭 / 가격 확인 필요 / 검토 필요 / 미매칭`과 함께 적용 단가·선택 출처·선정
  ...
  구체적인 문구로 표현한다.
  구매 가능한 최적 오퍼가 없는 후보는 후보 단위 선택 버튼도 비활성화하고 같은 사유를 버튼에 표시해,
  오퍼별 `선택 불가` 판정과 후보 선택 동작이 어긋나지 않게 한다.
...
- 관리자는 견적 라인의 [후보·근거]에서 고객과 동일한 후보 스냅샷·현재 선택·변경 이력을 확인해,
  고객 선택이 자동 추천과 달라진 이유를 추적한다. SmartBOM Case 상세의 검색·후보·구매조건 비교는
  RFQ·주문·PO 여부와 관계없이 항상 열어 둔다. `requested|reviewing`, build `ready`, 공급사 검색
  비실행, 장바구니·주문·PO 없음이며 해당 품목이 어느 RFQ에도 포함되지 않은 일반 변경은 바로
  적용한다. 업무가 진행된 품목도 관리자가 영향 경고와 체크를 확인하면 강제 변경할 수 있다.
- 관리자 교체 API는 클라이언트의 가격·재고·합계를 받지 않는다. 후보 키 또는 영속 `partId`와
  공급사·SKU 정체성만 받고 부모 견적을 잠근 트랜잭션에서 상태·RFQ·주문·PO와 `updatedAt`을 다시
  검사한 뒤 서버 스냅샷으로 재계산한다. 일반 선택 이벤트는 `source=admin`/`admin-choice`, 강제
  변경은 `admin-force-choice`로 구분한다. 강제 변경 시 해당 품목의 기존 RFQ 회신 행은 삭제하고
  RFQ를 `requested`로 되돌리되 문서·요청 범위·매직링크는 유지해 새 부품으로 다시 회신받는다.
  고객 회신·종료 견적은 `reviewing`으로 되돌리고, 기존 주문·PO는 발행 당시 스냅샷을 보존한다.
  모든 변경은 확정 운송료·관리비·총액을 해제하며 원본 BOM과 선택 이력을 유지한다. 기존 JSON/문자열
  필드를 사용하므로 DB 마이그레이션은 없다.
...
- **관리자/고객 판정 일치 실증**: `(박용한정렬)EnvMainV5_BOM241209.xlsx` 첫 시트 108행을
  ...
  v3 정책에 따라 정확 제조사+MPN은 기술 1순위를 유지하고, 저항·커패시터 동급 안전 밴드에서는
  실효 총액 최저 후보를 적용한다.
...
- `layouts/BomLayout.vue`: 상단바(로고·견적별 샘플/양산 조달 모드·타이틀·프로필) + 좌측(BOM 분석/단일 검색
...
- `pages/bom/BomHistory.vue`: 파일·견적명 검색, 상태 필터, 페이지 이동, 현재 페이지 선택,
  ...
  DB 인프라 레버(buffer pool 증설·운영 배포 체크리스트)는 docs/DB_TUNING.md.
...
- **시안 대비 미구현(표시만) 리스트**: 프로필 메뉴 ·
  프로모 카드 링크(튜토리얼/Gerber Eyes) — 표시 요소는 존재, 동작은 후속.

### 단일 검색 `/bom/search` (2026-07-20, 2026-07-23 하이브리드 개편)
...
- **수량·환율 단일 문맥**: sp-node가 요청마다 기존 BOM 실효 환율 스냅샷을 한 번 확정해
  ...
  사실값으로 저장하고 원화 가격은 검색 시점 파생값으로만 취급한다. 환율이 없으면 서로 다른
  미환산 통화를 숫자만으로 비교하지 않는다.
