...
- **정확 MPN 우선 자동 선정(2026-07-22)**: `supplier-candidate-decision-v3`는 정규화 제조사 품번이
  ...
  조건 불일치는 계속 차단한다. 제조사가 비어 있는 원본 품번이 여러 구조화 제조사에서 동시에 발견되면
  `manufacturer_source_conflict`로 분리해 exact MPN이라도 수동검토로 내린다. sp-node는 현재 v3와
  저장된 v1·v2 결정을 모두 수신하고 이 정책을 재추론하지 않는다.
