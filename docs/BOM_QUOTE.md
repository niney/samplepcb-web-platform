...
- 관리자는 SmartBOM Case 품목 툴바의 [부품 추가]에서 세트당 BOM 수량을 입력하고 카탈로그
  부품·공급사 오퍼를 선택해 `sourceSheetIndex=null`인 수동 행을 추가할 수 있다. 클라이언트는
  `partId`, 공급사·SKU, BOM 수량만 보내며 서버가 현재 세트·예비 수량과 오퍼 원장으로 필요수량,
...
- 관리자 카탈로그 검색은 로컬 결과를 먼저 보여준 뒤 정확 MPN을 포함해 공급사 캐시/API 추가
  ...
  실행하며 세트당 수량 편집은 이미 확인한 오퍼에 MOQ·주문배수를 다시 적용할 뿐 외부 호출을
