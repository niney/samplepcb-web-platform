...
- **기술·가격 하이브리드 자동 선정(`engine-hybrid-transparent-v2`, 2026-07-20)**:
  ...
  스펙 행도 기본은 기술 1순위다. 저항/커패시터/인덕터/크리스털의 카테고리별 필수 스펙이
  **전부 검증**되고 충돌·필수값 누락이 없으며 재고가 충분한 후보에 한해, 기술 1순위 대비
  **10% 이상이면서 500원 이상** 절감될 때만 다른 MPN을 가격 추천한다. 기술 1순위가
  NRND/EOL이고 활성 후보가 있으면 수명주기를, 가격/재고가 없거나 부족하면 구매 가능성을
  ...
  고객이 명시한 후보/공급사 오퍼는 유지한다. 추천 유형·기술 순위·검증 수·절감액/비율·이유 코드는
