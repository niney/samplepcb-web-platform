...
> 관련: `docs/PARTS_SEARCH.md`(부품 카탈로그 — 매칭·구매 조건의 원천), `AGENTS.md`.
...
고객(/app/bom, 회원 전용)                sp-node(/api/bom)                sp-engine(:8400)
 ...
 검토(수량·구매 조건·포함) → 1s 자동저장 ─▶ PATCH(draft, ID 기반 수정 명령)
...
- **상태**: `draft → requested → reviewing → answered → closed` (+ requested/reviewing의
  ...
  주문·결제·발주·배송까지 끝났다는 뜻이 아니다. 전이는 서버 검증
  (`lib/bom-quote.ts QUOTE_TRANSITIONS`), requested 이후 고객 수정 불가(409).
- **회신 공개 경계**: 검토 중 저장한 확정가·고객 회신 메모는 관리자 초안이다. 고객 DTO에는
  `answered|closed` 이후에만 공개하며, 고객 회신 확정 시 기본 선택된 이메일을 해제할 수 있고 확정
  뒤에는 같은 내용으로 이메일만 다시 보낼 수 있다. 받는 주소는 회원 이메일을 기본으로
  표시하되 관리자가 발송별로 수정할 수 있으며, 수정 주소는 회원정보에 저장하지 않는다.
- **견적서 참고 이미지**: 화면 미리보기에는 선정 카탈로그→선정 후보→엔진이 검증한 정확 일치
  순서의 부품 이미지를 MPN과 함께 표시한다. 인쇄/PDF에서는 관리자가 이미지 포함 여부를 선택할 수
  있고, 이미지가 공급 부품의 정본으로 오인되지 않도록 제조사·MPN이 실제 공급 기준임을 명시한다.
- **견적서 전용 담당자**: 관리자 `BOM 견적` 설정의 담당자명·이메일을 고객·관리자 BOM 견적서에만
  사용한다. 전용 값이 빈 쌍이면 사업자정보의 정보관리책임자를 폴백으로 사용하며, PCB 견적·주문
  문서·협력사 알림에는 적용하지 않는다. 회사·대표·주소·전화·계좌는 계속 영카트 사업자정보가 정본이다.
...
- `sp_bom_supplier_search_trace`: 공급사 검색 실행별·엔진 componentId별 `supplier-search-trace-v1`
  원본 JSON을 박제하고 rowIdx·최초/fallback 검색어·시도 수만 열로 승격한다. 후보/구매 조건마다 중복하지
...
- `sp_bom_quote_item`(SpBomQuoteItem): **id(영속 행 식별자)**·rowIdx(표시 순서)·
  ...
  **selectedOffer Json(구매 조건 스냅샷 박제 — 가격구간 사다리 포함·pinned)**·lineTotalKrw·sourceRow(원본 근거)·
  sourceSheetIndex/sourceSheetName
- `sp_bom_quote_candidate`(SpBomQuoteCandidate): quoteItemId로 안정 행에 연결하며 엔진의 `identity_key`가 묶은
  부품 후보의 견적 문맥 스냅샷. 엔진 선택 자격·기술 순위·구매 조건/가격구간·검증 근거를 보존해 엔진 인메모리 잡이
  사라져도 고객과 관리자가 동일한 후보를 비교한다. 스펙은 기술 순위 최상 후보를 정본으로 삼고
  공급사별 값의 임의 필드 병합은 하지 않으며, 동일 부품의 공급사 구매 조건만 한 후보 아래 통합한다. 신규
...
- `sp_bom_quote_selection_event`(SpBomQuoteSelectionEvent): quoteItemId 기준으로 고객 명시 선택의
  이전/선택 후보·MPN·구매 조건·행 금액·이유를 누적한다. 행 갱신과 분리되어 선택 감사 이력이 보존된다.
...
| 현재 엔진 결과: sp-engine이 기술 1순위를 감사 근거로 보존한다. 저항·커패시터 동급 안전 후보 및 제조사 미기재 정확 MPN 검토 후보는 **실효 총액 최저**를 적용하고, 그 밖에는 기술 1순위를 유지한다. 기술 1순위가 구매 불가하면 다음 안전 후보군을 실제 적용 후보로 분리한다. 각 후보군 안에서는 **구매 가능성→과다주문 위험→실효 총액→주문수량**으로 구매 조건을 정한다. sp-node는 적용 후보 키와 안정 `offer_key`, 주문수량·가격구간·환산단가를 검증 후 그대로 저장 | 신규 — 기술·구매 판단을 엔진으로 일원화 |
| `pickDefaultOffer`: 엔진 후보가 아닌 카탈로그 직접 선택·구매 조건 갱신에만 사용 | 추천 판단과 분리 |
| `pinned`(사용자 명시 선택): 수량·환율 변경 시 저장한 엔진 후보를 무호출 재평가 API로 다시 계산하고, 요청 구매 조건이 여전히 허용될 때 유지 | 레거시 "선택 pkg 내 탐색" 일반화 |
| 통화: 구매 조건 원통화 보존. USD는 한국수출입은행 자동 환율(매매기준율/TTS 선택)+안전계수로 KRW 예상 환산. 장애 시 수동값→마지막 정상 캐시 순 폴백, 모두 없으면 uncosted 경고 | 신규 |
| samplepcb 파생 구매 조건은 견적 선정 후보 제외(자기 선택 순환 방지) | — |
| 납기: "확정 시 안내" 문구 | '2주' 하드코딩 제거 |

**서버 재계산 원칙**: 합계는 항상 서버가 스냅샷에서 재계산(클라 금액 불신). 검색 완료 후
스냅샷 구매 조건·주문수량·적용 가격은 엔진 결정을 서버가 검증해 저장하며, sp-node는 저장 스냅샷의
...
- **필요 조건**: 엔진 판정(`matchEvidence`)이 없는 업로드 라인 OR included 미매칭 라인 존재
  OR 구매 조건 갱신 후 경과 시간 > `freshnessHours`(기본 24h). 카탈로그가 신선해도 최초 업로드는 반드시 엔진
...
- **입력 충돌·조달 가능 상태 보존(2026-07-22)**: sp-engine 추출 계약 1.2·검색 계약 1.2는 값·패키지·
  ...
  제외한다. `PCB/FPCB`와 층수·두께가 함께 명시된 강한 기판 제작 행은 `RING_REV01` 같은 보드 리비전
  식별값이 품번 열에 있어도 전자부품 identity로 승격하지 않고 `pcb_feature`로 제외한다. 참조번호 개수와
  선언 수량이 다르거나 수량이 없으면 기술 후보 검색은 가능하지만
...
- **정확 MPN 우선 자동 선정(2026-07-22)**: `supplier-candidate-decision-v3`는 정규화 제조사 품번이
  ...
  필수조건 배지·툴팁과 충돌 상세에 계속 표시한다. 워크벤치 행에서는 일반 `Matched`와 같은 중립 배경을
  사용하고 별도 정보 확인 배지를 표시하지 않는다. 포장 접미사 등 변형 품번과 파라메트릭 후보의 실제
...
- **완료 반영과 카탈로그 분리(2026-07-21, 무인 복구 2026-07-23)**: 폴러는 엔진 결과를 한 번 읽어 견적 후보·선정
  구매 조건·가격 스냅샷과 `done`을 먼저 커밋한다. 사용자가 기다리지 않아도 되는 전역 카탈로그 DB/ES
  ...
  backfill**한다. 이때 후보·구매 조건·가격·사용자 선택은 변경하지 않으며, 제조사가 확인된 행은
...
- **완료 반영 경로 3중**: ① 결과 폴러의 견적 우선 반영 ② 결과 조회 카탈로그 백업 훅 ③ **게으른 치유** —
  ...
  나머지는 엔진의 안전 후보 판정과 선택 구매 조건을 한 저장으로 교체한다.
- **엔진 조달 판단 투영(`engine-procurement-projection-v13`, 2026-07-28)**:
  ...
  구매 적합 순위와 `automatic|manual_review|none` 추천을 계산한다. 기술 1순위에 적용 가능한 구매 조건이 없으면
  ...
  필요수량 대비 절대 초과량(기본 100개)과 초과비율(기본 50%)이 **모두** 한도를 넘는 구매 조건은 구매 가능·
  사용자 수동 선택 상태는 유지하되 추천 대상에서는 제외한다. 이 때문에 기술 사전 선정 그룹이
  1→5,000 같은 과다수량 구매 조건만 가지면 다음 구매 가능 기술 그룹을 평가하고, 대안이 없으면 미선정으로
  남긴다. sp-vue는 선택된 레거시/수동 구매 조건도 같은 기준으로 `필요/주문/초과/배율` 경고를 표시한다.
  기술 순위·`preselect`는 감사 근거로 그대로 남고 실제 선정·금액만 적용 후보를 따른다.
  sp-node는 1.4 계약의 기술/적용 후보군·구매 조건 키·
  ...
  - **샘플/양산 조달 모드**: 견적은 `procurementMode=sample|mass`를 저장한다. `sample`은 기존처럼
    안전하게 적용 가능한 구매 조건 중 실효 총액을 우선하고, `mass`는 같은 기술 판단·재고·과다주문 게이트를
    ...
    섞인 값은 Reel로 단정하지 않는다. 안전한 Reel이 없으면 기존 적용 가능한 구매 조건으로 축퇴하고
    `mass-production-reel-unavailable` 근거를 남긴다. 모드 전환은 공급사 API를 다시 호출하지 않고
    저장 후보를 `supplier-selection-application-v4`로 배치 재평가한다. 고객이 직접 고정한 후보·구매 조건은
...
- **판단 단일 설명 원본(2026-07-21)**: 후보의 관계·선택 자격·기술/검토 순위·사전 선정과
  구매 조건의 가격/구매적합 순위·자동/검토 추천은 모두 sp-engine이 결정한다. sp-node는 정책 버전,
  ...
  명시한다. `provisional_selected`도 엔진이 지정한 후보·구매 조건을 `selectedCandidateKey`와 예상금액에
  반영하지만 `selectionSource=auto`인 동안은 결과 집계부터 `Review`와 `선정됨 · 검토 대기`로 표시한다. 사용자가 확인하면
  기존 명시 선택 API가 같은 후보·구매 조건을 `selectionSource=customer`로 기록하며, sp-node가
  ...
  - **후보 shortlist(2026-07-24)**: 스펙 검색은 DigiKey·Mouser에 10건을 요청하고, 크기를 지정할 수
    ...
    기술 사전 선정·실제 적용 그룹과 한 공급사가 선택한 동일 그룹의 타 공급사 구매 조건을 함께 남겨
    교차 가격 비교를 보존하고, 원본 결과 수는 trace에 유지한다. sp-node의
    행당 10개 영속 상한은 구형·비정상 과대 결과에 대한 저장 안전장치이며 별도 기술 판단은 아니다.
  - **구매 조건 키 v2(2026-07-21)**: 검색 스키마 1.6부터 신규 공급사 구매 조건은
...
- **검토 표현**: 엔진의 `automatic_selected`는 바로 선정 완료, `provisional_selected`는 후보·구매 조건·
...
- 라인 구매 조건에 "기준 N일 전" 나이 배지(데이터 정직성 — 방금 조회처럼 보이지 않게).
- 라이브 검증: 카탈로그 미보유 STM32F030F4P6 업로드 → build 미매칭 → 고객 개입 0으로
  자동 검색→적재→실 Mouser 구매 조건(₩3,042) 매칭·합계 갱신 확인. 2026-07-19 초기화 상태
  (카탈로그 3건)에서 3부품 CSV 재검증 — 업로드 즉시 "확인 중" 모드, 완료 후 3/3 매칭
  (Mouser·Digikey 실구매 조건)·합계 갱신·버튼 활성.

## 비용 정책 (sp_config `bom_quote` — 관리자 설정 승격)

`/app/admin` 설정 → BOM 견적 탭: BOM 견적서 전용 담당자(빈 쌍이면 정보관리책임자 폴백)·
기본 운송료(30,000)·기본 관리비(25,000)·USD→KRW 적용 방식
...
담당자 전용 값은 비용·검색 설정과 분리된 `sp_config.bom_estimate_contact` JSON에 저장하므로
스키마 마이그레이션이 없고, 사업자정보의 개인정보 보호책임자 값도 변경하지 않는다.
...
- 업로드 분석 견적과 단일검색 견적은 `BomQuoteRequestModal` 하나로 **견적명 확인 → 요청 중 →
...
- 고객 결과는 `매칭 / 가격 확인 필요 / 검토 필요 / 미매칭`과 함께 적용 단가·선택 출처·선정
  ...
  MOQ·재고·적용 단가·행 총액을 함께 보여주며 안전 후보와 특정 공급사 구매 조건을 명시 선택할 수 있다.
  엔진 대표 사유가 `out_of_stock|insufficient_stock|stock_unverified|catalog_inquiry`이면 후보 영역 상단에 필요수량과
  함께 `재고 없음/부족/확인 필요` 경고를 표시한다. 각 후보의 모든 구매 조건이 같은 재고 제약이면 최적
  구매 조건 요약도 구체적인 재고 문구로 바꾸며, 펼친 공급사 구매 조건은 재고 0을 `재고 없음`, 양수지만
  ...
  주문수량 과다 건수로 집계해 보여주며, 모든 집계 숫자에는 `건` 단위를 붙인다. 현재 선정 구매 조건이
  정상이라면 후보 카드 대표값은 그 선정 단가·합계·MOQ를 우선 표시하고, 다른 포장 조건의 과다 주문은
  후보 전체 오류로 올리지 않고 펼친 해당 공급사 조건에서만 구체적인 필요/주문/초과 수량으로 표시한다.
  ...
  엔진 후보 밖의 카탈로그 직접 검색/구매 조건 선택은 같은 패널의 보조 경로로 유지한다.
- 관리자는 견적 라인의 [후보·근거]에서 고객과 동일한 후보 스냅샷·현재 선택·변경 이력을 확인해,
  고객 선택이 자동 추천과 달라진 이유를 추적한다. SmartBOM Case 상세의 검색·후보·구매 조건 비교는
...
- 관리자 교체 API는 클라이언트의 가격·재고·합계를 받지 않는다. 후보 키 또는 영속 `partId`와
  ...
  고객 확인 대기·마감 견적(`answered|closed`)은 `reviewing`으로 되돌리고, 기존 주문·PO는
  발행 당시 스냅샷을 보존한다.
  모든 변경은 확정 운송료·관리비·총액을 해제하며 원본 BOM과 선택 이력을 유지한다. 기존 JSON/문자열
  필드를 사용하므로 DB 마이그레이션은 없다.
- 관리자는 SmartBOM Case 품목 툴바의 [부품 추가]에서 세트당 BOM 수량을 입력하고 카탈로그
  부품·공급사 구매 조건을 선택해 `sourceSheetIndex=null`인 수동 행을 추가할 수 있다. 클라이언트는
  `partId`, 공급사·SKU, BOM 수량만 보내며 서버가 현재 세트·예비 수량과 구매 조건 원장으로 필요수량,
  MOQ·주문배수, 환율, 행·견적 합계를 다시 계산한다. 수동 행은 업로드 원본과 구분해 표시하고 그
  행만 제거할 수 있으며, 원본 분석 행 제거 요청은 서버가 차단한다.
- 관리자 카탈로그 검색은 로컬 결과를 먼저 보여준 뒤 정확 MPN을 포함해 공급사 캐시/API 추가
  확인을 기본 1회 실행하고, 카탈로그 반영이 끝나기 전에는 선택을 잠근다. 명시 검색어가 바뀔 때만
  실행하며 세트당 수량 편집은 이미 확인한 구매 조건에 MOQ·주문배수를 다시 적용할 뿐 외부 호출을
  반복하지 않는다. 실패·일일 한도 초과 시 경고를 남기고 기존 카탈로그 선택은 계속 허용한다.
  결과는 현재 검색 세션 기준 `기존 카탈로그 / 공급사 최신 확인 / 공급사 신규 발견`으로 그룹화한다.
  이는 발견 경로이며 카드의 `구매 채널`(Mouser·DigiKey 등)과 별도 개념이다.
- 일반 추가·제거는 교체와 같은 최신 버전·build/enrich·업무 상태 게이트를 사용한다. 관리자가
  강제 추가하면 `requestedItemIds=null`인 전체 범위 RFQ만 새 행을 자동 포함해 `requested`로
  되돌리고 문서 합계·납기·메모를 해제한다. 기존 행별 회신은 보존한다. 명시 ID 배열인 부분 RFQ는
...
- **후보 패널 실데이터 E2E**: `(박용한정렬)EnvMainV5_BOM241209.xlsx` 첫 시트 108행 견적(#63)의
  ...
  행에서 차순위 부품 선택→현재 선택/차액/이유 갱신→특정 DigiKey 구매 조건 고정→동일 부품 최저
  실효가 Mouser 구매 조건 복원→선택 이력 누적을 Chrome 실화면으로 확인했다. 콘솔 경고·오류 0건.
  동일 `EEE-1HA221P`의 `Panasonic`/`Panasonic Industry` 표기를 별칭으로 통합해 후보 2개→1개,
  공급사 구매 조건 4개가 한 카드에 표시되는 것도 확인했다.
- **대형 후보 저장 내성**: 후보/스펙 JSON 443건의 단일 `createMany`가 MariaDB 패킷 한도로
  연결을 끊는 것을 실측하고 20건 단위 트랜잭션 배치로 교정했다. 같은 데이터 440건 저장 345ms,
  엔진 재반영 뒤 고객 명시 선택 후보 키·구매 조건 키가 새 그룹 키로 재연결되는 것까지 확인했다.
- **관리자/고객 판정 일치 실증**: `(박용한정렬)EnvMainV5_BOM241209.xlsx` 첫 시트 108행을
  `componentId`로 대조. `spec_compatible 33 + verified_exact 26 = 자동 선정 59`(구매 조건 57,
...
- 카탈로그 직접 검색의 selectedOffer 스냅샷은 클라 제출값(조작 가능하나 RFQ 모델이라 이득 없음 —
  관리자 확정가가 정본). 엔진 후보 선택은 후보/구매 조건 키만 받아 DB 스냅샷으로 서버 재계산한다.
...
- **시안 대비 미구현(표시만) 리스트**: 프로모 카드 링크(튜토리얼/Gerber Eyes) — 표시 요소는 존재,
  동작은 후속.
...
- **하이브리드 검색**: `GET /api/bom/parts-search` 로 DB/ES의 로컬 결과를 조회한다. 단일 검색은
...
- **완료 후 최종 목록(2026-09-17)**: 단일 검색·상세 견적 추가 작업대는 `waitForCatalog=true`로 공급사
  확인과 카탈로그 반영까지 기다린다. 로컬에 가격 있는 부품이 있어도 완료 전에는 진행 상태를 표시한다.
  sp-node는 공급사 후보 자체의 품번·제조사 키로 실제 `partId`를 연결하고 구매 조건·엔진의 `searchMatch`
  판정을 반환한다. 원래 검색어로 재조회한 결과가 0건이어도 다른 품번 후보를 버리지 않는다.
  sp-vue는 공급사 후보와 로컬 결과를 부품 ID 기준으로 합치고 최종 목록과 완료 문구에 같은 개수를 쓴다.
  구매 조건이 없는 후보도 열람 목록에 남으며 담기는 제공하지 않는다. 일부 공급사 조회 실패는 별도
  안내하고, 전체 공급사 확인 실패 시에는 실패 안내와 함께 저장된 카탈로그 결과를 제공한다.
  같은 검색어의 재검색도 새 확인을 수행하며, 이전 검색의 늦은 응답은 현재 결과를 바꾸지 않는다.
  회귀 검증: `e2e/specs/bom-single-search-results.e2e.test.ts`(API 대체, 공유 DB·공급사 호출 없음).
...
- **FE 테이블**: `BomSearch.vue`(풀폭, 검색+필요수량 폼) + `BomSearchRow.vue` — BomQuoteRow
  ...
  BOM 문맥 배지는 없음). [구매 조건] 클릭 시 행 확장으로 전체 구매 조건·공급 포장 비교.
...
- 테이블: 공급사 배지(라이트 필+파비콘+이름, 로고·이미지 열 76px 정사각 정렬)·구간가(상위 4+
  ...
  단순화했다. 리스트에서는 정상 `선정품 생산 중`과 `스펙 대체 후보` 배지를 숨기고, EOL·단종·NRND
  같은 수명주기 주의만 유지한다. 전체 대체 근거·가격구간·부품 변경·공급사 구매 조건 상세는 후보 비교
  패널에서 처리한다.
...
- 2026-08-02에는 가변 높이 행을 실측하는 가상 스크롤(`@tanstack/vue-virtual`, overscan 6)을
  적용해 표시 구간만 DOM에 둔다. 이에 따라 브라우저 찾기가 아직 렌더되지 않은 행을 찾지 못하므로
  매칭 결과 헤더에 `현재 BOM에서 찾기`를 제공한다. MPN·원본값·제조사·설명·REFDES·시트/행·
  현재 공급사/SKU를 전체 클라이언트 품목에서 먼저 검색하고, 시트·판정·재고·검색 한도 필터와
  교차 적용한 결과만 가상 스크롤에 전달한다. 공급사 API나 DB 검색은 호출하지 않는다.
...
- 열: 항목·Excel 원본(sticky) + 공급사 열(Mouser/DigiKey/UniKeyIC 고정 + 발견 공급사) —
  같은 제조사+MPN 후보 아래 통합한 공급사 구매 조건에서 재고·MOQ·최저 단가·수명주기 표시
