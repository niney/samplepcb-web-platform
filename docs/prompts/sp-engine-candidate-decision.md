...
```json
{
  ...
  "selection_mode": "exact | variant | spec-compatible | review",
  "auto_eligible": true,
...
- 정규화 MPN이 정확히 같고 실제 스펙·패키지·부품 종류·실장 방식·치수 충돌이 없으면 제조사 불일치만으로 후보를 차단하지 않는다.
- 위 경우 제조사 별칭이 검증되지 않았다면 `manual_review`, `auto_eligible=false`, `manual_selectable=true`로 반환하고 `manufacturer_confirmation_required`를 근거에 넣는다.
- 검증된 제조사 별칭은 불일치가 아니며 자동 선택 자격을 가질 수 있다.
- MPN, 전기 스펙, 패키지, 부품 종류, 실장 방식, 직경 등 실제 기술 조건이 충돌하면 `blocked`, `manual_selectable=false`다.
- 정확 MPN은 공급사에 일부 상세 스펙이 없어도 동일 주문 코드라는 정체성이 확인되면 자동 자격을 유지할 수 있다. 다만 실제 충돌이 있으면 반드시 차단한다.
...
동일 `identity_key`의 여러 공급사가 서로 다른 실장 방식이나 허용 오차를 넘는 직경을 주장하면 엔진에서 `mount_style_source_conflict` 또는 `diameter_mm_source_conflict`로 차단한다. 한 공급사에 값이 없고 다른 독립 공급사에 일관된 값이 있으면 확인된 값을 사용할 수 있다.
...
- 정확 MPN + 제조사 불일치만 존재: `manual_review`, 수동 선택 가능.
- 정확 MPN + 제조사 불일치 + 상세 스펙 누락: 수동 선택 가능.
- 정확 MPN + 실제 스펙 또는 물리 조건 충돌: `blocked`.
...
- 동일 부품 공급사 간 실장/직경 불일치: 그룹 전체 차단.
