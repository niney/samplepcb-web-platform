...
engine_sync()  { step "sp-engine uv sync";     (cd "$ROOT/samplepcb-parts-engine" && uv sync --frozen); }
engine_restart(){ step "sp-engine 재시작";      sudo systemctl restart sp-engine
  sleep 1; curl -fsS http://127.0.0.1:8400/health >/dev/null && echo "  health OK" || echo "  ⚠ health 실패 — journalctl -u sp-engine 확인"; }
...
case "$choice" in
  ...
  9) case_9 ;;
  10) case_10 ;;
  *) echo "잘못된 선택: '$choice' (1-10)"; exit 1 ;;
esac

step "완료 — 서비스 상태"
for u in sp-api sp-engine php8.1-fpm nginx mariadb; do
