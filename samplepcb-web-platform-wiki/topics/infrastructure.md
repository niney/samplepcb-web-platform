...
samplepcb 웹 플랫폼의 로컬/운영 라우팅·배포·인프라 계층. 두 코드 영역(sp-php = 그누보드5/영카트 `samplepcb-web/`, sp-vue·sp-market·sp-node = `samplepcb-web-mono-app/`)을 **같은 도메인**에서 함께 돌리는 것이 핵심 목표다 — 같은 도메인이어야 PHPSESSID 쿠키가 공유되어 인증 브리지(그누보드=IdP)가 성립한다. 로컬은 nginx 4경로 분기(`/`·`/app`·`/market`·`/api`)가 모두 가동 중이고, **운영은 centrafab.co.kr 실서버 배포 런북·배포 스크립트(`deploy.sh`)까지 확립됐다**(2026-07, `docs/DEPLOY_CENTRAFAB.md`). 인프라 설정은 메타 repo 가 추적하는 `ops/` 폴더에, 실파일 저장은 외부 파일서버 `file.samplepcb.kr` 에 위임한다.
...
/market/ → 127.0.0.1:5176  Vue (Vite dev+HMR)  ← samplepcb-web-mono-app/apps/market (sp-market, base:'/market/') ★2026-07-08 신설
/        → 127.0.0.1:8888  PHP (XAMPP Apache)  ← samplepcb-web (그누보드/영카트)     ← 루트=PHP
```

- location 순서 규칙: 구체 경로(`/api`·`/app`·`/market`)를 먼저, catch-all `/`을 마지막에. `/app`·`/market`·`/api`는 그누보드가 점유하지 않는 **예약 경로**. `/spcb`(인증·알림 브리지)는 별도 location 없이 catch-all 로 PHP 가 처리.
...
- `/app`·`/market`은 각각 dev(Vite 프록시 + WebSocket Upgrade HMR)와 운영(빌드 `dist/` alias + `try_files … /{app,market}/index.html` SPA fallback) 블록 중 **택1** — 운영 블록은 스니펫에 주석으로 준비돼 있다.
...
- E2E: `ops/scripts/e2e-market.mts` — 재능마켓 전 플로우 E2E 91케이스(매칭~계약·결제·검수·정산).
...
- 운영 nginx 설정은 리포 미추적(서버에서 직접 생성, 런북 STEP 9 인라인) — `.htaccess` 번역 포함: data/ PHP 실행차단(RCE 방지)·`/spcb` 무확장 라우팅+Bearer 패스스루·루트 짧은URL(`/슬러그`→`spcb/pages/`)·lib 차단.
- Node는 systemd 유닛 `sp-api`(`ExecStart=/usr/bin/node --env-file=.env dist/server.js` — **시스템 node 절대경로 필수**, fnm 임시경로는 203/EXEC). MariaDB는 charset utf8 + **`sql_mode=''` 영구화**(strict면 그누보드 쓰기 실패).
- **재배포 = 루트 `deploy.sh`** (운영 서버 전용, 메뉴식 8케이스): sp-api만/+DB마이그레이션/web만/market만/풀 재배포/sp-php만(git pull+php-fpm reload=opcache 비움)/nginx reload/.env만(sp-api 재시작). `pull` 단계에 **스테일 emit 정리 가드**(`clean_stale`) 내장 — 과거 `vue-tsc -b`가 src/에 emit한 untracked `.js`·`.tsbuildinfo`를 제거(vite 확장자 해석이 .js > .ts라 옛 .js가 .ts를 가려 MISSING_EXPORT 빌드 실패 재발 방지). prisma는 `migrate deploy`만(reset/dev 금지), 파괴적 마이그레이션 여부를 물어 sp-api 중단/무중단 선택.
...
| 구성요소 | 상대 | 방식 |
|---|---|---|
| nginx edge (로컬) | sp-node(3333) · sp-vue(5173) · sp-market(5176) · sp-php(8888) | 경로 기반 리버스프록시, `X-Forwarded-*`/`Host` 전달 |
| nginx edge (운영) | sp-api(3333, systemd) · web/market `dist/` 정적 · php-fpm unix socket | Cloudflare 뒤 :80, `X-Real-IP=$http_cf_connecting_ip` |
...
운영 배포 명령 표면(`deploy.sh` 케이스 1–8): api / api+DB / web / market / 풀 / php / nginx / .env — 정적 앱(web·market)은 dist 교체만으로 즉시 반영(서비스 재시작 불필요), sp-api는 `.env`·코드 변경 시 `systemctl restart sp-api` 필수(systemd 는 시작시점 env 고정).
...
- **같은 도메인 경로 분기** (도메인 분리 대신): PHPSESSID 공유 → 인증 브리지가 CORS/서드파티쿠키 문제 없이 성립. `/app`·`/market`·`/api` 를 예약 경로로 확보.
...
- **pnpm 필터명은 스코프 없음** — `--filter api`·`web`·`market`(`@sp/api` 아님). `@sp/*` 는 패키지 쪽.
...
- **레퍼런스 스니펫 ≠ 라이브 그대로**: 라이브 `/app`·`/market` 엔 `X-Forwarded-Proto` 한 줄 더, 부가 호스트(local·local2·local3)는 repo 미추적.
...
- location 순서: 구체 경로를 catch-all 보다 먼저 두지 않으면 `/api`·`/app`·`/market` 이 PHP 로 흘러간다.

## Sources [coverage: high — 10 sources]

- [ops/README.md](../../ops/README.md)
- [ops/nginx/local-web.conf](../../ops/nginx/local-web.conf) — 4경로 레퍼런스 스니펫(/market 포함)
- [docs/DEPLOY_CENTRAFAB.md](../../docs/DEPLOY_CENTRAFAB.md) — centrafab.co.kr 운영 배포 런북(신설)
- [deploy.sh](../../deploy.sh) — 운영 재배포 스크립트(케이스 8종·스테일 emit 가드)
