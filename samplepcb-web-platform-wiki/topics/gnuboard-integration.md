...
- **nginx 통합 호스트** `local-web.samplepcb.co.kr`: `/api`→Node(3000) · `/app`→Vue(5173) · `/`(catch-all)→PHP Apache(8888). `/app`·`/api` 는 그누보드 예약 경로가 아니라 안전. `/spcb` 는 별도 location 없이 catch-all 로 PHP 가 처리. 레퍼런스: `ops/nginx/local-web.conf` (실구동은 repo 밖 `D:\nginx\conf\nginx.conf`).
