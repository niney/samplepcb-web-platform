...
- **핵심 화면 구성**:
  - `RequestWizard` — 의뢰 등록 마법사. STEP2 "전문 기술·도구"는 **분야 종속 동적 스텝**(`MARKET_AREA_TOOL_GROUPS`·`MARKET_AREA_SPECIALTIES` 합집합, 질문 그룹 없는 분야만이면 스텝 자체 생략=4스텝). "설명·자료" 뒤 **AI 스텝도 동적** — 인터뷰 유스케이스가 활성이면 선택적 설명·첨부 선분석→질문 폼(정책 뱅크에서 최대15, 한 화면5)→명세 요약 카드+AI 추가질문→결정적 구성도 생성→ROC·포스팅 카드 생성(선택), 아니면 legacy 단발 diagram UI, 셋 다 비활성이면 스텝 없음.
...
- **코드 사전·한글 라벨 정본은 `packages/api-contract/src/schemas/market.ts`**(`MARKET_*`·`MARKET_*_LABELS`) — sp-market·sp-vue·sp-node 메일 빌더 3곳 공유, DB 에는 코드만 저장. 질문 뱅크도 계약 데이터(정책 77 + SW 보완 3, 한 번 5개·전체 최대15)이며 선택적 선분석은 명시 근거가 있는 답변만 재충전 없이 제외한다.
...
2. **2026-07-12/16 — LLM 산출물 렌더 격리·첨부 분석 고지**: 구성도 HTML 은 반드시 **sandbox iframe(srcdoc)**(`DiagramViewer`), ROC 마크다운은 **라인 파서 렌더·v-html 금지**(`RocViewer`). 첨부 구조화는 고지 후 제한 추출 텍스트·래스터를 보내며, 레거시 구성도는 첨부를 보내지 않는다.
