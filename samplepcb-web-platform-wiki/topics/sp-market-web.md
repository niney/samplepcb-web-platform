...
- **코드 사전·한글 라벨 정본은 `packages/api-contract/src/schemas/market.ts`**(`MARKET_*`·`MARKET_*_LABELS`) — sp-market·sp-vue·sp-node 메일 빌더 3곳 공유, DB 에는 코드만 저장. 질문 뱅크도 계약 데이터(정책 77 + SW 보완 3, 한 번 5개·전체 최대15).
