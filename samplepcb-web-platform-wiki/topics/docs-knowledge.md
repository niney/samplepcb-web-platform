...
| [AI_DIAGRAM.md](../../docs/AI_DIAGRAM.md) | AI 시스템 구성도 자동 생성(Ollama) — **범용 AI 유스케이스 계층**(`sp_config`+`sp_ai_usecase`+비동기 잡, stream 필수), sandbox iframe 렌더(XSS), §6 정책 질문 뱅크 80문항·5개씩/최대15·첨부 문서/이미지 분석(`qwen3.5:cloud`)·spec JSON 피벗·ROC·포스팅 | 2026-07-16 | 새 AI 유스케이스를 추가할 때. 위저드 구성도/인터뷰/첨부 분석·관리자 AI 연동·Ollama 연결을 만질 때 |
