...
Total topics: 11 | Total sources: 44 knowledge files (+ 토픽별 경량 코드 스캔)

## Topics

| Topic | Also Known As | Sources | Last Updated | Status |
|-------|--------------|---------|-------------|--------|
| [sp-node-api](topics/sp-node-api.md) | sp-node, Fastify API, 담기 API, 가격 엔진, 관리 API, g5 접근 카탈로그 ①–⑳, 재능마켓 백엔드, AI 유스케이스, BOM 견적, sp_bom_quote, /api/bom, 부품 카탈로그, sp_part, sp-parts ES, engine-client, 환율 스냅샷, requireAdmin, 카탈로그 임포트 parts:catalog, Walsin, 연호, offer_kind, 문의 견적, SpPartIngestRun, 검색 원장, bom-local-catalog | 21 | 2026-07-27 | active |
| [sp-vue-web](topics/sp-vue-web.md) | sp-vue, /app, Vue SPA, 관리자 화면, admin, /app/bom, BOM 워크벤치, BomQuoteRow, BomCandidateDrawer, BomSearch, BomHistory, localCatalogTrace, 문의 견적 표기, AdminParts, AdminMarket, AdminSeo, AdminSlides, AI 설정, GerberPricingForm, requiresMember | 36 | 2026-07-27 | active |
...
| [parts-engine](topics/parts-engine.md) | sp-engine, samplepcb-parts-engine, bom-extraction-engine, supplier-search-engine, smartbom_engine, FastAPI :8400, uv workspace, Mouser, DigiKey, UniKeyIC, 판단 단일 원본, BOM 역할 경계, catalog-evaluate-batch, preflight, SUPPLIER_MAX_CALLS, supplier-candidate-decision | 13 | 2026-07-27 | active |
| [api-contract](topics/api-contract.md) | @sp/api-contract, Zod 계약, PcbProjectPayload, market·ai·seo·slides·settings 스키마, MARKET_* 코드 사전, AI_USECASES, bom-quote.ts, parts.ts, PartOfferKind, BomQuoteLocalCatalogTrace, apiRoutes | 18 | 2026-07-27 | active |
...
| [infrastructure](topics/infrastructure.md) | nginx, ops/, 라우팅, /market 경로, /rnd, sp-rnd, deploy.sh 케이스 10, DEPLOY_CENTRAFAB, systemd sp-api, sp-engine.service, DB_TUNING, buffer pool, Mailpit, 파일서버, e2e-market | 13 | 2026-07-27 | active |
| [docs-knowledge](topics/docs-knowledge.md) | docs/, 설계 기록, BOM_QUOTE, PARTS_SEARCH, MARKET_FLOW, AI_DIAGRAM, SEO_MANAGEMENT, GERBER_PRICE_MODE, LEGACY_DB_MIGRATION, DB_TUNING, prompts/, 배포 런북, 코드리뷰 기록 | 22 | 2026-07-27 | active |
...
| [snapshot-freeze](concepts/snapshot-freeze.md) | sp-node-api, sp-vue-web, sp-market-web, gnuboard-integration, docs-knowledge | 2026-07-27 |
| [in-memory-async-jobs](concepts/in-memory-async-jobs.md) | sp-node-api, parts-engine, sp-market-web, sp-vue-web | 2026-07-27 |
| [judgment-single-owner](concepts/judgment-single-owner.md) | parts-engine, sp-node-api, sp-vue-web, api-contract, docs-knowledge | 2026-07-27 |

## Recent Changes
- 2026-07-27: 증분 재컴파일 — 지식 파일 13종 변경(신규 5: docs/DB_TUNING.md·walsin README 2종·yeonho Rev2 README·docs/prompts/sp-engine-candidate-decision.md). 6개 토픽 갱신: **제조사 카탈로그 → SamplePCB 자체 취급 편입**(연호 커넥터 Rev2 1,606·Walsin R/C 2,628, `offer_kind` 분리·부품 유형별 로컬 우선 조회·"문의 견적" = 금액 없는 identity 선정)·**엔진 결과 영속 원장 전환**(분석 append-only 정본·공급사 결과 gzip 아티팩트+무인 복구 워커·fingerprint 인제스트 원장·일일 사용량 DB)·**AGENTS.md "BOM 역할 경계" 명문화**(판단=sp-engine 단일 원본)·sp-engine 운영 배포(deploy.sh 케이스 10+systemd 유닛)·InnoDB buffer pool 튜닝(16M→1G, 운영 체크리스트)·카탈로그 파괴 작업 안전 등급 분리(필터 삭제=견적 보호 vs 초기화=견적 강제 삭제). **신규 개념 judgment-single-owner**(판단 단일 소유권, 5토픽). in-memory-async-jobs에 "DB 원장 승격" 방향 전환 기록, snapshot-freeze에 provenance 박제 인스턴스 추가. sp-market-web·shared-packages·spcb-bridge·theme-sp-lite·gnuboard-integration 은 유의미 변경 없어 유지.
